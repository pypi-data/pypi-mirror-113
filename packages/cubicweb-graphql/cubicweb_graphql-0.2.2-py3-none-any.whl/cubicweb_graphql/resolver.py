...
# The attribute name that refers to the CubicWeb entity from the GraphQL object instance
_MODEL = "__model"
...
class Resolver:
    ...
    def _resolve_entity_from_eid(cache, entity_type, connection, eid):
        ...
        try:
            ...
            entity = connection.vreg["etypes"].etype_class(entity_type.__name__)(
                connection
            )
            entity.eid = eid
    ...
    def _build_constraints(
        subject, restrictions, constraints, new_arguments, sub_start=0
    ):
        """
        ...
        for field, value in restrictions.items():
            ...
            if hasattr(value, TYPE_FIELD_META):
                ...
                Resolver._build_constraints(
                    sub_subject, sub_restrictions, constraints, new_arguments
                )
            else:
                # simple value
    ...
    def resolve_entities(self, entity_type, query, info, *args, **kwargs):
        ...
        return [
            self._get_graphql_entity(cache, entity_type, entity)
            for entity in results.entities()
        ]

    def resolve_primitive_field(
        self, instance, relation_name, target_type, info, *args, **kwargs
    ):
        """
    ...
    def resolve_scalar_object_field(
        self, instance, relation_name, target_type, info, *args, **kwargs
    ):
        """
        ...
        :return: The resolved value
        """
        return self._resolve_object_field(
            instance, relation_name, target_type, info, True, *args, **kwargs
        )

    def resolve_vector_object_field(
        self, instance, relation_name, target_type, info, *args, **kwargs
    ):
        """
        Fetch the values for an entity's field when its type is a collection of instances
        of other entities
        :param instance: The entity's instance
        :param relation_name: The name of the relation for this field
        :param target_type: The GraphQL type for this field
        ...
        """
        return self._resolve_object_field(
            instance, relation_name, target_type, info, False, *args, **kwargs
        )

    def _resolve_object_field(
        self, instance, relation_name, target_type, info, scalar, *args, **kwargs
    ):
        """
        ...
        if kwargs is None or len(kwargs) == 0:
            ...
            results = connection.execute(rql, arguments)
            values = [
                self._resolve_entity(cache, entity) for entity in results.entities()
            ]
            if scalar:
    ...
    def on_create_entity(self, root, info, entity_type, **kwargs):
        ...
        results = connection.execute(rql, arguments)
        values = [
            Resolver._get_graphql_entity(cache, entity_type, entity)
            for entity in results.entities()
        ]
        return values[0] if len(values) > 0 else None
    ...
    def _get_update_rql(type_name, arguments):
        ...
        sub_restrictions = {
            k: v
            for k, v in arguments[ARG_UPDATE_OLD].__dict__.items()
            if not k.startswith("_") and v is not None
        }
        ...
        sub_restrictions = {
            k: v
            for k, v in arguments[ARG_UPDATE_NEW].__dict__.items()
            if not k.startswith("_") and v is not None
        }
    ...
    def _get_delete_relation_rql(type_name, arguments):
        ...
        sub_restrictions = {
            k: v
            for k, v in arguments[ARG_DELETE_FROM].__dict__.items()
            if not k.startswith("_") and v is not None
        }
        sub = Resolver._build_constraints(
            _SUBJECT_VAR, sub_restrictions, constraints, new_arguments, sub
    ...
    def on_delete_relation(self, root, info, entity_type, **kwargs):
        ...
        """
        rql, arguments = Resolver._get_delete_relation_rql(
