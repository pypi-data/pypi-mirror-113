...
class ScpiSystem(object):
    ...
    def time_local(self, time: Optional[datetime.time] = None) -> datetime.time:
    ...
    def date_local(self, date: Optional[datetime.date] = None) -> datetime.date:
    ...
    def timezone_posix(self, posix: Optional[str] = None) -> str:
        """Sets or gets the local timezone in POSIX1003.1 format"""
        if posix is not None:
            self.device.execute('SYST:TZONE:POSIX', param=ScpiString(posix))
        else:
            posix = self.device.execute('SYST:TZONE:POSIX?', result=ScpiString).as_string()
