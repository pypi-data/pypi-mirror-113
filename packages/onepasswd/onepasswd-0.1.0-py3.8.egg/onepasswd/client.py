...
class Cmd(object):
    ...
    def list(self):
        log.debug('list')
        for x in self.query_db(self.query):
            print(x)

    def sync(self):
        log.debug('sync')
        git = GitDB(self._config['user'], self._config['token'], self._config['repo'])
        ...
        tmp_file = os.path.join('/tmp', str(time.time()) + '.tmp.db')
