...
def start_webserver(port, host):
    ...
    def post_fork(server, worker):  # pylint: disable=unused-argument
        ...
        if utils.BIOLIB_IS_RUNNING_IN_ENCLAVE:
            res = requests.get(f'{job_worker_config.PARENT_REST_SERVER_URL}/init_enclave/')
            enclave_data = res.json()

            BiolibApiClient.initialize(enclave_data['base_url'])
            ...
            webserver_utils.report_availability(webserver_utils.COMPUTE_NODE_INFO, webserver_utils.DEV_MODE)
