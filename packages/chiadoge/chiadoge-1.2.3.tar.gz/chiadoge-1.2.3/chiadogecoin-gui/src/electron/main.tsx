...
import chiadogeEnvironment from '../util/chiadogeEnvironment';
import chiadogeConfig from '../util/config';
...
if (!handleSquirrelEvent()) {
  ...
  const ensureCorrectEnvironment = () => {
    ...
    if (!chiadogeEnvironment.guessPackaged() && !('VIRTUAL_ENV' in process.env)) {
  ...
  if (ensureSingleInstance() && ensureCorrectEnvironment()) {
    ...
    chiadogeConfig.loadConfig('mainnet');
    ...
    const createWindow = () => {
      ...
      if (!chiadogeConfig.manageDaemonLifetime()) {
      ...
      mainWindow.on('close', (e) => {
        ...
        if (decidedToClose || !chiadogeConfig.manageDaemonLifetime()) {
          return;
        }
        e.preventDefault();
        if (!isClosing) {
    ...
    const appReady = async () => {
      ...
      // if the daemon isn't local we aren't going to try to start/stop it
      if (chiadogeConfig.manageDaemonLifetime()) {
        chiadogeEnvironment.startChiadogeDaemon();
      }
    };
  ...
  const getMenuTemplate = () => {
    const template = [
      ...
      },
      {
        ...
        submenu: [
          {
            label: i18n._(/* i18n */ { id: 'Chiadogecoin Wiki' }),
            click: () => {
              openExternal(
                'https://github.com/Chiadoge-Network/chiadogecoin/wiki',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/Chiadoge-Network/chiadogecoin/wiki/FAQ',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/Chiadoge-Network/chiadogecoin/releases',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/Chiadoge-Network/chiadogecoin/blob/master/CONTRIBUTING.md',
              );
            },
          },
          {
          ...
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/Chiadoge-Network/chiadogecoin/issues',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal('https://discord.gg/invite/qK2x5zFUNx');
            },
          },
          {
            label: i18n._(/* i18n */ { id: 'Follow on Twitter' }),
            click: () => {
              openExternal('https://twitter.com/chiadogecoin');
            },
          },
        ],
      },
    ...
    if (process.platform === 'darwin') {
      // Chiadogecoin menu (Mac)
      template.unshift({
        label: i18n._(/* i18n */ { id: 'Chiadoge' }),
        submenu: [
          {
            label: i18n._(/* i18n */ { id: 'About Chiadogecoin' }),
            click: () => {
              openAbout();
            },
          },
