...
async def _get_folder(request: web.Request) -> web.Response:
    """
    Gets the folderintegrationtest with the specified id.
    :param request: the HTTP request.
    :return: the requested folderintegrationtest or Not Found.
    """
    _logger.debug('Requested folderintegrationtest %s', request.match_info["id"])
...
@action(name='heaserver-folders-folder-open', path='/folders/{folder_id}/items/{id}/opener', rel='opener')
...
async def get_items(request: web.Request) -> web.Response:
    """
    Gets the items of the folderintegrationtest with the specified id.
    :param request: the HTTP request.
    :return: the requested items, or Not Found if the folderintegrationtest was not found.
    """
    folder = request.match_info['folder_id']
    _logger.debug('Requested items from the "%s" folderintegrationtest', folder)
...
async def get_item(request: web.Request) -> web.Response:
    """
    Gets the requested item from the given folderintegrationtest.
    ...
    parameters:
        ...
        - name: id
          in: path
          required: true
          description: The id of the folder item to retrieve.
          schema:
            type: string
    responses:
      '200':
...
async def get_item_opener(request: web.Request) -> web.Response:
    ...
    if item:
        ...
        if type_name is not None and id_ is not None:
            ...
            actual_opened = client.get(request.app, URL(url) / id_ / 'opener', headers=request.headers)
    _logger.debug('got from service: %s', actual_opened)
    return await response.get(request, actual_opened)


@routes.get('/folders/{folder_id}/opener')
async def get_folder_opener(request: web.Request) -> web.Response:
    """
    Opens the requested folderintegrationtest.
    :param request: the HTTP request. Required.
    :return: the opened folderintegrationtest, or Not Found if the requested item does not exist.
    """
