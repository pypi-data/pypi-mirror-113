...
class AttrDict(qGJMg):
 def __init__(self,*args,**kwargs):
  qGJMx(AttrDict,self).__init__(*args,**kwargs)
  self.__dict__=self
def tcp_listen(six,addr,port,blk):
 s=socket.socket(AF_INET6 if six else AF_INET,SOCK_STREAM)
 if six and qGJMb(socket,"IPV6_V6ONLY"):
...
class Half(qGJMa):
 def __init__(self,opt,sock,addr,qGJMs):
  self.opt=opt
  self.sock=sock
  self.addr=addr
  self.qGJMs=qGJMs
  self.name="peer" if self.qGJMs else "client"
  self.queue=[]
  self.dest=qGJMw
  self.err=qGJMw
  self.ready=qGJMh
 ...
 while qs:
  ...
  for q in qs:
   if q.post_wait(r,w,e):
    qs.remove(q)
def main():
 opt={"ip6":qGJMh,"bind_port":2223,"addr":"192.168.43.191","port":22}
 opt={"ip6":qGJMh,"bind_port":48429,"addr":"128.0.100.3","port":22}
 opt={"ip6":qGJMh,"port":54237,"bind_addr":"128.0.100.2","bind_port":22}
 opt={"ip6":qGJMh,"port":54237,"bind_addr":"192.168.100.3","bind_port":22}
 server_loop(opt)
