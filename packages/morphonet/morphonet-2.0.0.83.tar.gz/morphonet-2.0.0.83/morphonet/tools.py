...
def get_crop_3D(img,background=0,border=2):
    '''
    RETURN CROP 3D of Segmented image
    '''

    #XXXXXXXXX
    xmin=0
    while xmin<img.shape[0] and img[xmin,:,:].sum()==background :
        xmin+=1
    ...
    xmax = img.shape[0]-1
    while  xmax>0 and img[xmax,:,:].sum()==background :
        xmax -= 1
    if xmax<=1:
        return [-1,-1,-1,-1,-1,-1]
    xmax+=border
    ...
    #YYYYYYYYYY
    ymin = 0
    while  ymin < img.shape[1] and img[:,ymin,:].sum()==background :
        ymin += 1
    ...
    ymax = img.shape[1] - 1
    while ymax > 0  and img[:,ymax,:].sum()==background :
        ymax -= 1
    if ymax <= 1:
        return [-1,-1,-1,-1,-1,-1]
    ymax += border
    ...
    #ZZZZZZZZZZ
    zmin = 0
    while zmin < img.shape[2] and img[:,:,zmin].sum()==background :
        zmin += 1
    ...
    zmax = img.shape[2] - 1
    while zmax > 0 and img[:,:,zmax].sum()==background :
        zmax -= 1
    if zmax <= 1:
        return [-1, -1, -1, -1, -1, -1]
    zmax += border
...
def convert_to_OBJ(dataFull,t,background=0,factor=1,Smooth=True,Decimate=True,Reduction=True,TargetReduction=0.8,Border=2,center=[0,0,0],VoxelSize=[1,1,1],maxNumberOfThreads=None,cells_updated=None,path_write=None): ####  CONVERT SEGMENTATION IN MESH
        ...
        if Border>0: #We add border to close the cell
            _dataToConvert=np.zeros(np.array(_dataToConvert.shape) + Border * 2).astype(_dataToConvert.dtype)
