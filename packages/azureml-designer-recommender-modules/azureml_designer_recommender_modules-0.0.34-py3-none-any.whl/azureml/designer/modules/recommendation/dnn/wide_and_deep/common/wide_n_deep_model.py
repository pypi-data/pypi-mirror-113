...
from azureml.studio.core.error import UserError, InvalidDirectoryError, PathExistsError
from azureml.studio.internal.error import InvalidModelDirectoryError, ModuleOutOfMemoryError, ErrorMapping
...
from azureml.designer.modules.recommendation.dnn.common.distributed_utils import distributed_env, DistributedStrategy
...
class WideNDeepModelHyperParams:
    ...
    @property
    def use_gpu(self):
        # For now, only below optimizer kernels support sparse update for GPU right now
...
class WideNDeepModel(CoreModel, metaclass=EntryParam):
    ...
    def _init_training_config(self, input_function_builder: InputFunctionBuilder):
        ...
        steps_per_epoch = int(np.ceil(instances_count / input_function_builder.batch_size))
        module_logger.info(f"Get {instances_count} training instances, and {steps_per_epoch} steps per epoch.")

        if distributed_env.distributed_strategy == DistributedStrategy.MultiWorkerMirroredStrategy:
            # this line is a workaround for issue:
            # https://github.com/tensorflow/tensorflow/issues/46556
            tf.compat.v1.disable_eager_execution()
            strategy = tf.distribute.experimental.MultiWorkerMirroredStrategy()
            module_logger.info(f"Would train model with multi worker mirrored strategy.")
            replica_count = strategy.num_replicas_in_sync
        elif distributed_env.distributed_strategy == DistributedStrategy.MultiDeviceMirroredStrategy:
            strategy = tf.distribute.MirroredStrategy()
            module_logger.info(f"Would train model with multi device mirrored strategy.")
            # if the multi device mirrored strategy, each replica consumes one batch, so the steps per epoch will
            # be the 1/N of the original steps per epoch
            if steps_per_epoch < strategy.num_replicas_in_sync:
                module_logger.info(
                    f"Batch count {steps_per_epoch} is less than the replica count {strategy.num_replicas_in_sync}, "
                    f"reduce replica count to {steps_per_epoch}")
                strategy = tf.distribute.MirroredStrategy([f"GPU:{i}" for i in range(steps_per_epoch)])
            steps_per_epoch = steps_per_epoch // strategy.num_replicas_in_sync
            replica_count = strategy.num_replicas_in_sync
        else:
            strategy = None
            module_logger.info(f"Would train model with non-distributed strategy.")
            replica_count = 1

        run_config = tf.estimator.RunConfig(tf_random_seed=self.random_seed,
                                            log_step_count_steps=steps_per_epoch,  # log loss after each epoch
                                            save_checkpoints_steps=steps_per_epoch * self.hyper_params.epochs,
                                            keep_checkpoint_max=1,
                                            train_distribute=strategy)

        return run_config, replica_count

    def train(self, input_function_builder: InputFunctionBuilder):
        run_config, replica_count = self._init_training_config(input_function_builder)

        module_logger.info(f"Build model:\n{self.hyper_params}")
        self.build_model(run_config=run_config)

        input_fn = input_function_builder.get_input_fn()
        metrics_log_hook = LogHook(total_steps=run_config.log_step_count_steps * self.hyper_params.epochs,
                                   log_steps=200,
                                   batch_per_step=input_function_builder.batch_size,
    ...
    def build_optimizer(optimizer_name: OptimizerSelection, learning_rate):
        ...
        if distributed_env.distributed_strategy == DistributedStrategy.MultiDeviceMirroredStrategy:
            # This is a workaround for issue: https://github.com/tensorflow/tensorflow/issues/33358
            # And as our experience, in TensorFlow 2.4.1, this only occurs in MirroredStrategy
            return lambda: optimizer(learning_rate=learning_rate)
        else:
        ...
        with TimeProfile("Saving Wide & Deep recommendation model"):
            ...
            if not overwrite_if_exists:
                if os.path.exists(model_save_path):
                    raise PathExistsError(model_save_path)
                if tf.train.latest_checkpoint(checkpoints_save_dir) is not None:
