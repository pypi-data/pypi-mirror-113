...
SENT_EMAILS=[]
EMAIL_BLACKLIST=SNGoO()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",SNGos(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=SNGou,from_name=SNGou,smtp_host=SNGou,smtp_user=SNGou,smtp_pass=SNGou,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if SNGoU(recipients,SNGoY)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=SNGou):
 ...
 result["From"]=formataddr((SNGos(Header(from_name,"utf-8")),from_email))
 return result
