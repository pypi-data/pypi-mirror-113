...
class SEDMesh(object):
    ...
    def _distributeSea(self, Qs):
        ...
        if MPIrank == 0:
            Qs, ndepo, outVol = distsea(
                Qs, self.gZ, self.sealevel, self.sGrp, self.sPits, self.sVol, self.sOut,
            )
        ...
        if (ndepo > 0).any():
            self._hierarchicalSink(False, True, False, self.gZ)
    ...
    def _distributeContinent(self, Qs, sinkFlx):
        ...
        self._hierarchicalSink(False, False, True, self.gZ)
    ...
    def _distributeOcean(self, hclino, sinkFlx):
        ...
        if MPIrank == 0:
            ...
            vdepth = hclino - self.gZ
            vdepth[vdepth < 0.0] = 0.0
            ndepo = distocean(
                self.flowDir, sid, sinkFlx, sRcv, sWght, vdepth * self.marea, vdepth
            )
        ndepo = MPI.COMM_WORLD.bcast(ndepo, root=0)

        return ndepo

    def _diffuseOcean(self, ndepo, stype):
        ...
        self.depo = h - self.oldh
        self.gZ = h

        return

    def _hierarchicalSink(self, offshore=False, land=False, limited=False, gZ=None):
        """
        ...
        if MPIrank == 0:
            if offshore:
                sFill, self.sPits = fillpit(-9.0e3, gZ, 1.0e6)
            if land:
                lFill, self.lPits = fillpit(self.sealevel, gZ, 1.0e6)
            if limited:
                lFill, self.lPits = fillpit(self.sealevel, gZ, self.sedfill)
        else:
            if offshore:
        ...
        if land or limited:
            ...
            if MPIrank == 0:
                ...
                _, outids, _ = np.intersect1d(
                    self.lPits[:, 0], pitNb, return_indices=True
                )
    ...
    def _distributeSediment(self, stype=0):
        ...
        t0 = process_time()
        self._hierarchicalSink(True, True, False, None)
        ...
        while (Qs > 0).any():
            ...
            if insea[0] > 0:
                ...
                Qs = self._distributeSea(Qs)
                # Remove flux that flow into open sea
                sinkFlx[self.sinkID] += Qs[self.sinkID]
                Qs[self.sinkID] = 0.0
                if MPIrank == 0 and self.verbose:
                    print(
                        "Distribute sea (%0.02f seconds)" % (process_time() - ti),
                        flush=True,
                    )
            ...
            if (Qs > 0).any():
                ti = process_time()
                Qs, sinkFlx = self._distributeContinent(Qs, sinkFlx)
                if MPIrank == 0 and self.verbose:
                    print(
                        "Distribute continent (%0.02f seconds)" % (process_time() - ti),
                        flush=True,
                    )
        ...
        t0 = process_time()
        ndepo = self._distributeOcean(hclino, sinkFlx)
        if MPIrank == 0 and self.verbose:
            print(
        ...
        if (ndepo > 0).any():
            t0 = process_time()
            self._diffuseOcean(ndepo, stype)
            if MPIrank == 0 and self.verbose:
                print(
                    "Diffuse River Sediment in Ocean (%0.02f seconds)"
                    % (process_time() - t0),
                    flush=True,
                )
