...
class Gen2NodeProvider(NodeProvider):
    ...
    def _init(self):
        ...
        self.ibm_vpc_client = _create_vpc_client(endpoint, IAMAuthenticator(iam_api_key))
        self.global_tagging_service = GlobalTaggingV1(IAMAuthenticator(iam_api_key))
        self.global_search_service = GlobalSearchV2(IAMAuthenticator(iam_api_key))
    ...
    def _attach_tags(self, resource_model, _tags):
        ...
        for retry in range(RETRIES):
            logger.info(f'retry #{retry} to tag {resource_model}')
            try:
                ...
                if not tag_results['results'][0]['is_error']:
                    ...
                    logger.error(f"failed to attach tags {_tags} to {resource_model}, {tag_results}")
            except Exception as e:
                logger.error(f"failed to attach tags {_tags} to {resource_model}, {e}")
