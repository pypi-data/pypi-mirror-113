...
from blackdogosint.utils.context import LocalNoarchContext
from blackdogosint.utils.log import getLogger
from blackdogosint.utils.utils import iters
import six
...
"""
    ...
    with context.local(**kwargs):
        ...
        if not isinstance(number, six.integer_types):
            raise ValueError(
                'pack(): number must be of type (int,long) (got %r)' % type(number),
            )
        ...
        if endianness not in ['little', 'big']:
            raise ValueError(
                "pack(): endianness must be either 'little' or 'big' (got %r)" % endianness,
            )
...
arg_doc = {
    'p': 'number (int): Number to convert',
    'u': 'data (bytes): Byte string to convert',
}
rv_doc = {
    'p': 'The packed number as a byte string',
    'u': 'The unpacked number',
}


def make_single(op, size, end, sign):
    name = '_{}{}{}{}'.format(op, size, end, sign)
    fmt = sizes[size]
    end = '>' if end == 'b' else '<'
...
def make_multi(op, size):
    ...
    def routine(number):
        ...
        return {
            ...
            ('big', True): bs,
            ('big', False): bu,
        }[endian, signed](number, 3)

    routine.__name__ = name
    routine.__doc__ = """{}{}(number, sign, endian, ...) -> {}
    {}s an {}-bit integer
    Arguments:
        {}
        endianness (str): Endianness of the converted integer ("little"/"big")
    ...
    Returns:
        {}
    """.format(op, size, return_types[op], op_verbs[op].title(), size, arg_doc[op], rv_doc[op])
...
def _fit(pieces, preprocessor, packer, filler, stacklevel=1):
    ...
    for k, v in pieces.items():
        if isinstance(k, six.integer_types):
            ...
            raise TypeError(
                "flat(): offset must be of type int or str, but got '%s'" % type(k),
            )
        if k in pieces_:
    ...
    negative = {
        k: v for k, v in pieces.items() if isinstance(k, int)
        and k < 0
    }
...
def _flat(args, preprocessor, packer, filler, stacklevel=1):
    ...
    for arg in args:
        ...
        if hasattr(arg, '__flat__'):
            ...
            raise ValueError(
                'flat(): Flat does not support values of type %s' % type(arg),
            )
...
def flat(*args, **kwargs):
    ...
    if length:
        if len(out) > length:
            raise ValueError(
                'flat(): Arguments does not fit within `length` (= %d) bytes' % length,
            )
    ...
    if isinstance(src, six.text_type):
        ...
        for i, b in enumerate(src, seek):
            ...
            if isinstance(b, bytes):
                ...
                raise TypeError(
                    'dd(): Unsupported `src` element type: %r' % type(b),
                )
        src = src_
...
def _need_bytes(s, level=1, min_wrong=0):
    ...
    if worst >= min_wrong:
        warnings.warn(
            'Text is not bytes; assuming {}, no guarantees. See https://docs.pwntools.com/#bytes'
...
def _need_text(s, level=1):
    ...
    warnings.warn(
        'Bytes is not text; assuming {}, no guarantees. See https://docs.pwntools.com/#bytes'
        .format(encoding), BytesWarning, level + 2,
    )
