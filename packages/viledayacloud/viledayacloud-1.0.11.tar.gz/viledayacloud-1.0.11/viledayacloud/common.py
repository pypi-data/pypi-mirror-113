...
class WorkItem:
    ...
    def __init__(self, jsonsrc: Optional[str] = None):
        ...
        """
        if jsonsrc is not None and isinstance(jsonsrc, str):
            _j = loads(jsonsrc)
        else:
            _j = dict()
        self._wi = {"id":         _j["id"] if "id" in _j.keys() and isinstance(_j["id"], str) else shortuuid.uuid(),
                    ...
                    "photo":      _j["photo"] if "photo" in _j.keys() and isinstance(_j["photo"], bool) else None,
                    ...
                    "photo_s3":   _j["photo_s3"] if "photo_s3" in _j.keys() and isinstance(_j["photo_s3"],
                                                                                           str) else None,
                    "voice":      _j["voice"] if "voice" in _j.keys() and isinstance(_j["voice"], bool) else None,
