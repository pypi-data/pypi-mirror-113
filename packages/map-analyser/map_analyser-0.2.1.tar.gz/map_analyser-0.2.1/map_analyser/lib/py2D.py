"""From https://github.com/sseemayer/Py2D"""
...
def intersect_poly_lineseg(poly_points, p1, p2):
    ...
    """
    return intersect_linesegs_lineseg(list(zip(poly_points[0:], poly_points[1:])) + [(poly_points[-1], poly_points[0])],
                                      p1, p2)
...
def intersect_poly_ray(poly_points, p1, p2):
    ...
    @return: The list of intersection points or an empty list
    """
    return intersect_linesegs_ray(list(zip(poly_points[0:], poly_points[1:])) + [(poly_points[-1], poly_points[0])], p1,
                                  p2)
...
def intersect_poly_poly(poly_points1, poly_points2):
    ...
    return intersect_linesegs_linesegs(
        list(zip(poly_points1[0:], poly_points1[1:])) + [(poly_points1[-1], poly_points1[0])],
        list(zip(poly_points2[0:], poly_points2[1:])) + [(poly_points2[-1], poly_points2[0])])
...
def distance_point_line(p, a, b):
    return abs((p.x - a.x) * (b.y - a.y) - (p.y - a.y) * (b.x - a.x)) / math.sqrt(
...
class Polygon(object):
    ...
    def regular(center, radius, points):
        ...
        for i in range(points):
            p.add_point(Vector(center.x + radius * math.cos(i * angular_increment),
                               center.y + radius * math.sin(i * angular_increment)))
    ...
    def convex_decompose(polygon, holes, debug_callback=None):
        ...
        def check_decomp(l, p_minus_l, p):
            ...
            pts = (v for v in p_minus_l if
                   p[v].x <= x_max and p[v].x >= x_min and p[v].y <= y_max and p[v].y >= y_min and is_notch(v))
                ...
                for hole in holes:
                    for a, b in list(zip(hole, hole[1:])) + [(hole[-1], hole[0])]:
                        ...
                        if i and i not in [a, b]:
                            if not closest_intersection or (closest_intersection - d_b).length_squared > (
                                    i - d_b).length_squared:
        ...
        def try_decompose(i_start):
            ...
            i_extend = next((i for i in itertools.chain(range(i_start + 1, len(p)), range(0, i_start + 1)) if
                             not point_orientation(p[i - 1], p[i], p[(i + 1) % len(p)])))

            # build provisional l
            l = list(range(i_start, i_extend + 1)) if i_start < i_extend else list(range(i_start, len(p))) + list(
                range(0, i_extend + 1))
            ...
            try:
                i_extend2 = next((i for i in itertools.chain((i_start, -1, -1), range(len(p) - 1, i_start, -1)) if
                                  not point_orientation(p[i - 1], p[i], p[(i + 1) % len(p)])))

                l2 = list(range(i_extend2, len(p))) + list(range(0, i_start)) if i_extend2 > i_start else list(
                    range(i_extend2, i_start))
            ...
            for v in sorted(l[1:-1], reverse=True):
                dbg(p[v], (255, 0, 255), "del %d" % g.delindex)
                g.delindex += 1
                del p[v]
