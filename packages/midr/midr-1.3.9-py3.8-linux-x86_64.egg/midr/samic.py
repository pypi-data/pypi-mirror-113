...
def loglikelihood_theta(theta, u_values, copula, params_list, dcopula=None):
    ...
    return -np.sum(
        params_list['pi'] + archimedean.lsum(dcopula, is_log=True, axis=1),
        axis=0
    )
...
def local_idr(u_values, params_list, missing=None):
    ...
    """
    return 1 - np.exp(expectation_k(u_values=u_values, params_list=params_list))
...
def minimize_alpha(l_state):
    ...
    1.0
    """
    alpha = archimedean.lsum(l_state, is_log=True, axis=0) - np.log(
        float(l_state.shape[0]))
    assert np.any(~np.isnan(alpha)), "minimize_alpha(), l_state value" + str(
        l_state)
    return alpha
