...
class Vehicle(object):
    ...
    async def vehicle_status(self, vehicleID):
    ...
    async def device_status(self, deviceKey):
        """
        Poll the vehicle for updates
        """
        return await self.sendCommand("DEVICE_STATUS", deviceKey, "2")

    async def start(self, deviceKey):
        """
        Issue a start command to the engine
        """
        return await self.sendCommand("REMOTE_START", deviceKey, "1")

    async def stop(self, deviceKey):
        """
        Issue a stop command to the engine
        """
        return await self.sendCommand("REMOTE_STOP", deviceKey, "1")

    async def lock(self, deviceKey):
        """
        Issue a lock command to the doors
        """
        return await self.sendCommand("ARM", deviceKey, "1")

    async def unlock(self, deviceKey):
        """
        Issue an unlock command to the doors
        """
        return await self.sendCommand("DISARM", deviceKey, "1")

    async def trunk(self, deviceKey):
        """
        Issue a command to open the trunk
        """
        return await self.sendCommand("TRUNK", deviceKey, "1")
    
    async def panic_on(self, deviceKey):
        """
        Issue a panic command to the vehicle
        """
        return await self.sendCommand("PANIC_ON", deviceKey, "1")

    async def panic_off(self, deviceKey):
        """
        Issue a panic command to the vehicle
        """
        return await self.sendCommand("PANIC_OFF", deviceKey, "1")

    async def aux1(self, deviceKey):
        """
        Issue a command to trigger the mapped Aux1 button event
        """
        return await self.sendCommand("REMOTE_AUX1", deviceKey, "1")

    async def aux2(self, deviceKey):
        """
        Issue a command to trigger the mapped Aux1 button event
        """
        return await self.sendCommand("REMOTE_AUX2", deviceKey, "1")

    async def location(self, deviceKey):
        """
        Issue a command to return the vehicle's current location
        """
        return await self.sendCommand("A30", deviceKey, "2")

    async def sendCommand(self, command, deviceKey, deviceType):
