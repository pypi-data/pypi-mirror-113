...
def GetDatasetFolderMixed():
    db_path = "F:\\DATASET\\MIXED\\"
    if platform == "linux" or platform == "linux2":
        db_path = '/home/s1620409/DATASET/MIXED/'
    return db_path
def GetDatasetFolderMixedSyn():
    db_path = "F:\\DATASET\\MIXED\\SYN\\"
    if platform == "linux" or platform == "linux2":
        db_path = '/home/s1620409/DATASET/MIXED/SYN/'
    return db_path
...
def LoadRealDataMixed(dbname):
    db_path = GetDatasetFolderMixed()
    filename_num = db_path + dbname.replace(".csv","") + "_num.csv"
    db = pd.read_csv(filename_num,header=None).replace(np.nan, 'nan', regex=True)._values;
    DB = NormalizeDB(db,True)
    db_des = pd.read_csv(db_path +dbname.replace(".csv",".txt") ,header=None).replace(np.nan, 'nan', regex=True)._values;
    DB['cat'] = db_des[0]
    return DB

def LoadSynthesisDataMixed(n,d,k,range_=8, sigma_rate=0.1):
    if k <=0: k = 8
    db_path = GetDatasetFolderSynMixed()
    name = "SYN_"+str(n) + "_" + str(d) + "_" + str(k)+ "_" + str(range_) +"_"+ str(int(sigma_rate*100))+".csv"
    filename = db_path + name 
    if os.path.isfile(filename)==False:
        GenerateDataset(n,d,k,range_, sigma_rate)
