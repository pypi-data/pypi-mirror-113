...
class QxApiClient(object):
  ...
  def __init__(self, url: str, **kw):
    ...
    self._ak_id = kw.get('ak_id')
    self._ak_secret = kw.get('ak_secret')
  ...
  def _request_raw(self, method: str, path: str, **kw):
    ...
    if self._ak_id and self._ak_secret:
      api_sdk = HtjsApiSdkClient(self._ak_id, self._ak_secret)
      headers = api_sdk.get_signed_headers(method, url, headers)
