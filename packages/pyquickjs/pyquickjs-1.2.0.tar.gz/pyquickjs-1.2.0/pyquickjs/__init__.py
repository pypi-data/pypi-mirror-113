...
dll.eval.argtypes = [c_void_p, c_char_p, c_char_p]
dll.eval.restype = int
...
class QuickJS:
    ...
    def call(self, func_name: str, *args, buffer_length: int = 1024) -> str:
        ...
        _args = f'({",".join(process_args(args))})'
        return self.eval(func_name + _args, buffer_length)

    def eval(self, script: str, buffer_length: int = 1024) -> str:
        """
        eval 脚本
        :param script: 脚本字符串
        ...
        """
        out = create_string_buffer(buffer_length)
        _length = dll.eval(self.ctx, script.encode(), out)
        ret = out[:_length].decode()
        out = None
        return ret        
