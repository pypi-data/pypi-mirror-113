...
SENT_EMAILS=[]
EMAIL_BLACKLIST=rOHzW()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",rOHzb(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=rOHzU,from_name=rOHzU,smtp_host=rOHzU,smtp_user=rOHzU,smtp_pass=rOHzU,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if rOHzw(recipients,rOHzi)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=rOHzU):
 ...
 result["From"]=formataddr((rOHzb(Header(from_name,"utf-8")),from_email))
 return result
