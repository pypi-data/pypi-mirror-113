...
class Fade(Transform):
    ...
    def __init__(self, is_in=True, duration=1e3, curve="linear", *,  degree=2):
        assert curve in ("linear", "polynomial", "exponential")
        self.is_in = is_in
        self.duration = duration
        self.curve = curve
        self.degree = degree
    ...
    def realise(self, audio):
        if self.curve == "linear":
            ...
            if self.degree == 0: # reverts to linear:
                amp = np.linspace(0, 1, self.num_samples(audio.sample_rate))
            else:
                amp = np.linspace(0, 1, self.num_samples(audio.sample_rate))
                d = DB_to_Linear(self.degree) # TODO
                env = d**(1-amp)
                amp = (env - d)/(1-d)
            # TODO eaqsier to use (e**(kx) - 1)/(e**k - 1) for some k
            # then k > 0 is like polynomial of degree > 1
            # k < 0 is like polynomial of degree < 1
