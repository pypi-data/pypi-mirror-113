    ...
    def non_terminated_nodes(self, tag_filters):
        ...
        with self.lock:
            ...
            for retry in range(NT_RETRIES):
                ...
                # check if previously cached node not appears in the list of found nodes
                if self.cached_nodes.keys() - found_nodes.keys():
                    # nodes shouldn't dissapear too frequently, if happened most likely it is a glitch in search, retry
                    logger.warning(f'{self.cached_nodes.keys() - found_nodes.keys()} from perviously cached nodes {self.cached_nodes.keys()} missing in the list of found_nodes {found_nodes.keys()}, retrying search')
                    time.sleep(1)
                    continue
                else:
                    break
