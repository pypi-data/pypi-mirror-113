...
global.daemon_rpc_ws = `wss://${self_hostname}:56690`;
...
function loadConfig(net) {
  try {
    // check if CHIADOGE_ROOT is set. it overrides 'net'
    const config_root_dir =
      'CHIADOGE_ROOT' in process.env
        ? process.env.CHIADOGE_ROOT
        : path.join(os.homedir(), '.chiadoge', net);
    ...
    const daemon_port = lodash.get(config, 'ui.daemon_port', 56690); // jshint ignore:line
