...
from blackdogosint.utils.term import keyconsts as kc
from blackdogosint.utils.term import keymap as km
from blackdogosint.utils.term import term, text
...
def update_search_results():
    ...
    for idx, h in enumerate(history):
        for i in range(len(h) - len(buffer_left) + 1):
...
def readline(_size=-1, prompt='', float=True, priority=10):
    ...
    from blackdogosint.utils.term import term_mode
    if not term_mode:
