...
def get_knative_manifest(bento, registry, selector=None):
    if ':' in bento:
        name, version = bento.split(':')
    else:
        name = bento
    ...
    manifest = gen_knative_manifest(pod_name, version, registry, selector)
    click.echo(manifest)
    return manifest
...
@click.option('--selector', help='comma separated list of key=val pairs')
@click.pass_context
def deploy_to_knative(ctx, bento, registry, selector=None):
    manifest = ctx.invoke(get_knative_manifest, bento=bento, registry=registry, selector=selector)
    from_yaml(manifest)
