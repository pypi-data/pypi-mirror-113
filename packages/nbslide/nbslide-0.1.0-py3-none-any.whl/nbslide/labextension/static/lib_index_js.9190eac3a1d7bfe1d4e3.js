    ...
    constructor(notebook, commands) {
        ...
        this.nextSlide = () => {
            ...
            do {
                let cell = cells[index];
                let slideshow = cell.model.metadata.get('slideshow');
                if ((slideshow != null) && (slideshow.hasOwnProperty('slide_delay'))) {
                    delay = slideshow.slide_delay;
                }
                if (shouldRun && !skipExecution(cells[index])) {
                    let notebook = this.notebook;
                    (function (theindex) {
                        notebook.content.activeCellIndex = theindex;
                        _jupyterlab_notebook__WEBPACK_IMPORTED_MODULE_1__.NotebookActions.run(notebook.content, notebook.sessionContext).then((value) => {
                            if (value) {
                                _lumino_domutils__WEBPACK_IMPORTED_MODULE_2__.ElementExt.scrollIntoViewIfNeeded(notebook.content.node, cells[start].node);
                            }
                        });
                    })(index);
                }
            ...
            if (executed) {
                setTimeout(() => {
                    _lumino_domutils__WEBPACK_IMPORTED_MODULE_2__.ElementExt.scrollIntoViewIfNeeded(this.notebook.content.node, cells[start].node);
                    //this.notebook.content.activeCellIndex = start;
                }, delay);
            }
        };
...
//# sourceMappingURL=lib_index_js.9190eac3a1d7bfe1d4e3.js.map