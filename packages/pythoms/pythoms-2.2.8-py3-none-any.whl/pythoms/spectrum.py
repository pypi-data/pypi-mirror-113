...
class Spectrum(object):
    ...
    def charge(self, charge):
        ...
        # set new bounds
        self._start *= (self.charge / charge)
        self._end *= (self.charge / charge)
        self._charge = charge
    ...
    def trim(self, zeros=False, xbounds=None):
        ...
        for ind, xval in enumerate(self.x):  # iterate over x values
            if xval < xbounds[0]:
                continue
            elif xbounds[1] < xval:
                break
            elif self.y[ind] is not self.filler:
                xout.append(  # rounded to avoid array floating point weirdness
                    round(xval, self.decpl)
                )
