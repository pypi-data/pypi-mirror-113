...
from hestia_earth.validation.utils import (
    update_error_path, _filter_list_errors, _next_error, _same_properties, _value_average, _list_except_item,
    _find_linked_node, _is_before_today, run_model_from_node, run_model, _get_dict_key, _is_same_dict, _dict_without_key
)
...
def validate_nodes_duplicates(node: dict, values: List[dict]):
    node_without_id = _dict_without_key(node, 'id')
    other_values = _list_except_item(values, node)
    duplicates = [
        value for value in other_values if node.get('type') == value.get('type') and
        _is_same_dict(node_without_id, _dict_without_key(value, 'id'))
    ]
    return [next(
        ({
            'level': 'warning',
