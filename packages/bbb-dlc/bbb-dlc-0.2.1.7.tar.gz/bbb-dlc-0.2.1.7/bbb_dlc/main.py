...
class BBBDLC:
    def run(args):
        ...
        try:
            occCommand = "sudo -u adminroot /opt/plesk/php/7.3/bin/php " + args.ncDir + "/occ "
            ncPath = subprocess.check_output(occCommand + "config:system:get datadirectory", shell=True).decode('utf-8').strip()
            ...
            now = datetime.now()
            error = "[" + now.strftime("%d/%m/%Y %H:%M:%S") + "] " + str(e) + "\n"
            print(error)
