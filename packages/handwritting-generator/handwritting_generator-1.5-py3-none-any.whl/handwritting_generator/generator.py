...
class Generator:
    ...
    def select_background(self,string):
        ...
        if wide == True and exp == False:
          while 'wide' not in self.backgrounds[i]:
            i = random.randint(0, self.M - 1)

        elif exp == True and wide == False:
          while 'exp' not in self.backgrounds[i]:
            i = random.randint(0, self.M - 1)
        
        elif exp == True and wide == True:
          while 'exp' not in self.backgrounds[i] or 'wide' not in self.backgrounds[i]:
            i = random.randint(0, self.M - 1)
        else:
          while 'exp' not in self.backgrounds[i] and 'wide' not in self.backgrounds[i]:
            i = random.randint(0, self.M - 1)
