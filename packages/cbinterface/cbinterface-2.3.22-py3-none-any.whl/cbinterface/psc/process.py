...
def get_events_by_type(p: Union[Process, Dict], event_type: str):
    ...
    # change this whole lib to use our own implementation that will yeild events/alerts/everything
    # do not use cbapi except for the legacy response product (make it separete pip install cbinterface:response)
    # maybe use elements of cbc
...
def print_netconns(p: Union[Process, Dict], raw_print=False, return_string: bool = False):
    ...
    txt = "------ NETCONNS ------\n"
    for nc in get_events_by_type(p, "netconn_proxy"):
        if raw_print:
            txt += str(nc)
            continue
        action = (
