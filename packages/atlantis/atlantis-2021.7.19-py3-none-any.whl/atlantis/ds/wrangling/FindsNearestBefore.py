...
class FindsNearestBefore:
	...
	"""
	def __init__(self, time_complexity='O(1)', parse_int_as_date_if_possible=True, n_jobs=8):
		"""
	...
	def _create_map_function(self):
		...
		if values is not None:
			mapping = {x: self._bisect_function(x) for x in values}

			def function(x):
				return mapping[x]
		else:
			mapping = {x: x for x in self._list}
