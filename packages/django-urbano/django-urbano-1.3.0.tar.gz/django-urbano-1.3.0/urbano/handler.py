...
class UrbanoHandler:
    ...
    def get_default_payload(self, data):
        ...
        payload = {
            ...
            'cedibles': [],
            'cod_barra': '',
            'cod_cliente': data.customer_rut,
            'cod_rastreo': data.vtex_reference,
            'correo_elec': '',
            'description': '',
            'dir_entrega': data.address.full_address,
            'fech_emi_vent': data.created_at.strftime('%d/%m/%Y'),
            'fech_pro': data.shipping_date.strftime('%d/%m/%Y'),
            'fech_venc': (data.shipping_date + timedelta(days=14)).strftime('%d/%m/%Y'),
            ...
            'moneda': 'CLP',
            'monto_asegurado': '',
            'nom_autorizado': data.customer_full_name,
            'nom_autorizado_2': '',
            'nom_cliente': data.customer_full_name,
            'nom_empresa': '',
            'nom_urb': '',
            'nro_doc_autorizado': data.customer_rut,
            'nro_doc_autorizado_2': '',
            'nro_factura': '',
            'nro_guia_trans': data.wms_reference,
            'nro_int': data.address.unit,
            'nro_o_compra': data.nbr_reference,
            'nro_telf': '',
            'nro_telf_mobil': data.customer_phone,
            'nro_via': data.address.number,
            'peso_total': '',
