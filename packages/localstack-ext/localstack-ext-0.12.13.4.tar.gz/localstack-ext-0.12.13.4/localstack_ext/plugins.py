...
def register_localstack_plugins():
 ...
 if zcmsj([is_api_enabled(api)for api in EXTERNAL_PORT_APIS]):
...
def load_plugin_files():
 try:
  ...
  if "No module named" not in zcmsS(e):
   print("ERROR: %s"%e)
...
def api_key_configured():
 return zcmsw(os.environ.get("LOCALSTACK_API_KEY"))
