...
var ColumnEditCollection = /** @class */ (function (_super) {
    ...
    ColumnEditCollection.prototype.renderItem = function (col, i, _a) {
        ...
        var _e = this.props, columns = _e.columns, fieldOptions = _e.fieldOptions;
        var _f = this.state, isDragging = _f.isDragging, draggingTargetIndex = _f.draggingTargetIndex, draggingIndex = _f.draggingIndex;
        ...
        <RowContainer className={isGhost ? '' : DRAG_CLASS}>
          {canDelete ? (<button_1.default aria-label={locale_1.t('Drag to reorder')} onMouseDown={function (event) { return _this.startDrag(event, i); }} onTouchStart={function (event) { return _this.startDrag(event, i); }} icon={<icons_1.IconGrabbable size="xs"/>} size="zero" borderless/>) : (<span />)}
          <queryField_1.QueryField fieldOptions={fieldOptions} gridColumns={gridColumns} fieldValue={col} onChange={function (value) { return _this.handleUpdateColumn(i, value); }} takeFocus={i === this.props.columns.length - 1} otherColumns={columns}/>
          {canDelete ? (<button_1.default aria-label={locale_1.t('Remove column')} onClick={function () { return _this.removeColumn(i); }} icon={<icons_1.IconDelete />} borderless/>) : (<span />)}
        </RowContainer>
