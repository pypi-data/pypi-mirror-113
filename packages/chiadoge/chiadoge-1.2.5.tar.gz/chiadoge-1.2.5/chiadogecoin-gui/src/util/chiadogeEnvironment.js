...
const PY_FOLDER = '../chiadoge/daemon';
...
const getChiadogeVersion = () => {
  let version = null;
  const exePath = getExecutablePath('chiadoge');
  // first see if we can get a chiadoge exe in a standard location relative to where we are
  try {
    ...
    // that didn't work, let's try as if we're in the venv or chiadoge is on the path
    try {
      version = child_process
