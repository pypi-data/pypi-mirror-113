...
def assignment_report(assignment_name, subs_filter={}, port=8050):
    ...
    def filter_data(store):
        ...
        store['submissions'] = store['submissions_raw'][store['submissions_raw']['email'].isin(std_emails)]
    ...
    def preprocess_data(store):
        ...
        question_summary['Finished'] = q_result[q_result > 0].sum().values
        question_summary['Struggling'] = q_result[q_result == 0].count().values
    ...
    def build_question_dist(data):
        ...
        question_dist = px.bar(question_summary,
                            x='Questions',
                            y=['Not started yet', 'Struggling', 'Finished'],
                            title='Question Result Distribution',
    ...
    question_dist = html.Div([
        dcc.Graph(id='question-dist', figure=build_histogram(store)),
    ])
    ...
    def refresh_data(n_clicks, include_staffs_value, select_group_value, 
        ...
        # Debug
        # return (*update_layout(), html.Pre(ctx_msg))
