...
__CONST_MAX_COLUMNS = 20 # 최대 columns 개수
__CONST_MAX_ROWS = 10000 # 최대 rows 개수
__CONST_MAX_WIDTH = 1000 # 한 줄에 출력 가능한 최대 데이터 크기
__CONST_URL = 'http://korsair.kisti.re.kr/api/'

pd.set_option('display.max_columns', __CONST_MAX_COLUMNS)
pd.set_option('display.max_rows',__CONST_MAX_ROWS)
pd.set_option('display.width', __CONST_MAX_WIDTH)
# pd.set_option('display.max_colwidth', None) # 셀 안의 데이터 생략 방지. 단,정렬이 안됨

#에러 위치에 따른 예제코드 출력
def __get_error_message__(func):
...
def get_data(requestUrl, func):
    ...
    if(response.status_code != 200):
        __get_error_message__(func)
        return None
    return response.json()

#df에서 정수 및 실수 값을 필터
def extract_data(df, column, num):
    if(__check_empty__(df) == True): return
...
def get_variant(chro, pos, ref, alt):
    ...
    requestUrl = __CONST_URL + "variant/?" + queryString
...
def get_variant_id(rsid):
    requestUrl = __CONST_URL + 'variant/' + str(rsid)
...
def get_gene(gene_id):
    requestUrl = __CONST_URL + 'gene/' + str(gene_id)
...
def get_region(chro, start, end):
    ...
    requestUrl = __CONST_URL + "region/?" + queryString
