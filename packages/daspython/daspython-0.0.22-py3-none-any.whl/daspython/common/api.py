...
class ApiMethods(ABC):
    ...
    def get_data(self, url, request):
        ...
        response = requests.get(url=api_url, headers=self.token.headers, verify=self.token.check_https)
    ...
    def post_data(self, url, body):
        ...
        response = requests.post(api_url, headers=self.token.headers, data=data, verify=self.token.check_https)
    ...
    def put_data(self, url, body):
        ...
        api_url = f'{self.token.api_url_base}{url}'        
        data = json.dumps(body.__dict__)
        response = requests.put(api_url, headers=self.token.headers, data=data, verify=self.token.check_https)
        if response.status_code == 200:
            return json.loads(response.content.decode('utf-8'))
        else:
            return None      
    ...
    def delete_data(self, url):
        ...
        '''        
        api_url = f'{self.token.api_url_base}{url}'                        
        response = requests.delete(url=api_url, headers=self.token.headers, verify=self.token.check_https)
        if response.status_code == 200:
            return json.loads(response.content.decode('utf-8'))
        else:
            return None                          