...
SENT_EMAILS=[]
EMAIL_BLACKLIST=bVJfX()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r'\s+','',bVJfF(email or ''))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=bVJfU,from_name=bVJfU,smtp_host=bVJfU,smtp_user=bVJfU,smtp_pass=bVJfU,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if bVJfe(recipients,bVJfO)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=bVJfU):
 ...
 result['From']=formataddr((bVJfF(Header(from_name,'utf-8')),from_email))
 return result
