...
:LastEditTime: 2021-06-24 16:59:09
    ...
    def check_sign(handler):
        def wrapper(self, **args):
            ...
            if not build_sign or build_sign != sign_params["sign"]:
                print(f"http请求验签不匹配,收到sign:{sign_params['sign']},构建sign:{build_sign} 加密明文信息:{SignHelper.get_sign_params_str(sign_params,sign_key,reverse,is_sign_key)}")
