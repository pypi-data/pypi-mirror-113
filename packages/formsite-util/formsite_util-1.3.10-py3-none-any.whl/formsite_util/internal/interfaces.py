...
class FormsiteInterface:
    ...
    def _perform_api_fetch(self, only_get_headers: bool = False) -> Tuple[dict, List[dict]]:
        """Entrypoint for performing API calls."""
        api_handler = _FormsiteAPI(self, display_progress=self.display_progress, delay=self.api_call_delay)
        items, results = api_handler.Start(only_items=only_get_headers)
