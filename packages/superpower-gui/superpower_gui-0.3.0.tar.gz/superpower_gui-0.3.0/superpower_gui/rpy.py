...
__all__ = ['warnError', 'rPrint', 'R_CONSOLE', 'import_rpy2', 'pkg_name', 'utils', 'lib_loc', 'libPaths', 'SUPERPOWER',
           ...
           'rpy2py_pairlistsexpvector', 'py2rpy_list', 'py2rpy_list', 'py2rpy_none', 'Function', 'RFunction',
           'PyFunction', 'MatrixMixin']
...
class PyFunction(Function):
    ''' When we call a PyFunction, there are both attributes keyword arguments that get used
    when we call the "run" function. The setKwargs function updates both appropriately so we can
    go about our day without thinking about it. If we directly change the model, that's fine too'''

    def __init__(self, name, model, argNames=None, attrNames=None, modelKeys=None):
        super().__init__(name, argNames)
        self.attrNames = attrNames
        self.modelKeys = modelKeys
        self.statmodel = model

    def setKwargs(self, **kwargs):
        self.kwargs.update({k:v for (k, v) in kwargs.items() if k in self.argNames})
        #self.attrs.update({k:v for (k, v) in kwargs.items() if k in self.argNames})
        if self.attrNames:
            for key, value in kwargs.items():
                if key in self.attrNames:
                    setattr(self.statmodel, key, value)
        if self.modelKeys:
            for key, value in kwargs.items():
                if key in self.modelKeys:
                    self.statmodel.model[key] = value

    def run(self, alert=True, **kwargs):
        '''This method runs the python function associated with the model. The run will be
        timed and the runtime and results are added to the results dictionary. You should
        not have to modify this function to run your custom Model
        '''
        if kwargs:
        ...
        try:
            ...
            ret['kwargs'] = self.kwargs
            if self.attrNames:
                ret['kwargs'] = ret['kwargs'] | {key: getattr(self.statmodel, key) for key in self.attrNames if hasattr(self.statmodel, key)}
            if self.modelKeys:
                ret['kwargs'] = ret['kwargs'] | {key: self.statmodel.model[key] for key in self.modelKeys}
            self.logger.info('Running %s with python arguments\n%s' % (self.name, str(self.kwargs)))
            ret['results'] = {}
            ret['results']['power'] = self.statmodel.simulate(**{k:v for (k, v) in self.kwargs.items() if k in self.argNames})
            ret['runtime'] = (time.time() - startTime)
            ret['id'] = int(round(time.time() * 1000))
            ret['date'] = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
            self.logger.info('ID: %i, Date: %s, Runtime: %s' % (ret['id'], str(ret['date']), str(ret['runtime'])))
