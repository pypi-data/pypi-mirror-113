...
from rctaitest.core.android.constant import STFLIB
from rctaitest.utils.logger import get_logger
from rctaitest.utils.nbsp import NonBlockingStreamReader
from rctaitest.utils.safesocket import SafeSocket
from rctaitest.utils.snippet import reg_cleanup, on_method_ready, ready_method, kill_proc
from rctaitest.utils.threadsafe import threadsafe_generator
    ...
    def get_frame(self, projection=None):
        ...
        if self.display_id:
            raw_data = self.adb.raw_shell(
                self.CMD + " -d " + str(self.display_id) + " -n 'rctaitest_minicap' -P %dx%d@%dx%d/%d -s" % params,
                ensure_unicode=False,
            )
        else:
            raw_data = self.adb.raw_shell(
                self.CMD + " -n 'rctaitest_minicap' -P %dx%d@%dx%d/%d -s" % params,
                ensure_unicode=False,
            )
