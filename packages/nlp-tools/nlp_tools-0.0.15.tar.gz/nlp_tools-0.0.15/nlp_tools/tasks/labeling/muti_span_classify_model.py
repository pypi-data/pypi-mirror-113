...
class MutiSpanClassifyModel(ABCLabelingModel):
    ...
    def build_model_arc(self) -> None:
        ...
        tensor = self.refomer_layer(tensor)
    ...
    def compile_model(self,
        ...
        if metrics is None:
            metrics = 'accuracy'
        super(MutiSpanClassifyModel, self).compile_model(loss=loss,
                                                    optimizer=optimizer,
                                                    metrics=metrics,
                                                    **kwargs)
    ...
    def refomer_layer(self,inputs):
        from bert4keras.layers import SinusoidalPositionEmbedding
        from tensorflow.keras import backend as K
        inputs = tf.keras.layers.Dense(128)(inputs)
        pos = SinusoidalPositionEmbedding(128, 'zero')(inputs)
