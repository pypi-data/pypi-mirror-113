...
def random_product(*args, **kwargs):
    ...
    if kwargs != {}:
        raise TypeError(
            'random_product() does not support argument %s' % kwargs.popitem(),
        )
...
def bruteforce(func, alphabet, length, method='upto', start=None, databag=None):
    ...
    for e in iterator:
        ...
        if cur_iteration % 2000 == 0:
            ...
            h.status('Trying "{}", {:0.3f}%'.format(cur, progress))
            if databag:
...
def _mbruteforcewrap(func, alphabet, length, method, start, databag):
    ...
    res = bruteforce(
        func, alphabet, length, method=method,
...
def mbruteforce(func, alphabet, length, method='upto', start=None, threads=None):
    ...
    for i in range(threads):
        ...
        processes[i] = multiprocessing.Process(
            target=_mbruteforcewrap,
            args=(
                func, alphabet, length, method, (chunkid, totalchunks),
                shareddata[i],
            ),
        )
        processes[i].start()
    ...
    while not done:
        ...
        h.status('Trying {} -- {:0.3f}%'.format(current_item_list, progress))
