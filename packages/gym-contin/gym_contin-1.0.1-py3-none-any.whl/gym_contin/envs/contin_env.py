...
class ContinEnv(gym.Env):
  def __init__(self):
    ...
    self.XSA1 = np.add((self.thetas[1]*(df[["Xs"]])).to_numpy(), (self.thetas[2]*(df[["Xa"]])).to_numpy())
    self.XSA1 += self.thetas[0]
    self.Rho=(1/(1+np.exp(-XSA1)))
    self.actmin = (min(Rho))
    self.actmax = (max(Rho))

    self.min_action=np.array([np.float32(actmin)])
    self.max_action=np.array([np.float32(actmax)])
