...
from .enginemanager import AsyncEngineManager
...
ASYNC_ENGINES = AsyncEngineManager()
...
async def destroy_engines(loop=None):
    """Destroys all known engines."""
    ENGINES.destroy()
    await ASYNC_ENGINES.destroy(loop=loop)
...
def get_dsn(name='self', use_async=False):
    """Return the Data Source Name (DSN) for the named connection `name`."""
    global DATABASES
