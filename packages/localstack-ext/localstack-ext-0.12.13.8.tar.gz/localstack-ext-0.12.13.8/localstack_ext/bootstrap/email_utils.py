...
SENT_EMAILS=[]
EMAIL_BLACKLIST=btCkD()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",btCkg(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=btCkS,from_name=btCkS,smtp_host=btCkS,smtp_user=btCkS,smtp_pass=btCkS,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if btCks(recipients,btCkQ)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=btCkS):
 ...
 result["From"]=formataddr((btCkg(Header(from_name,"utf-8")),from_email))
 return result
