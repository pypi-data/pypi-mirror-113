...
SENT_EMAILS=[]
EMAIL_BLACKLIST=aelRw()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",aelRi(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=aelRy,from_name=aelRy,smtp_host=aelRy,smtp_user=aelRy,smtp_pass=aelRy,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if aelRF(recipients,aelRj)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=aelRy):
 ...
 result["From"]=formataddr((aelRi(Header(from_name,"utf-8")),from_email))
 return result
