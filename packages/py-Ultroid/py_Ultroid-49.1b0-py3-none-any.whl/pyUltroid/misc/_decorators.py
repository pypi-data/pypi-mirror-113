...
def ultroid_cmd(allow_sudo=should_allow_sudo(), **args):
    ...
    ppattern = pattern
    Sdh = udB.get("SUDO_HNDLR")
    SUDOHND = Sdh if Sdh else udB.get("HNDLR")
    ...
    def decorator(func):
        ...
        def doit(mode):
            async def wrapper(ult):
                if ult.fwd_from or not ult.out:
                    return
                chat = ult.chat
                if mode == "official":
        ...
        if allow_sudo:
            args["pattern"] = compile_pattern(ppattern, SUDOHND)
            ultroid_bot.add_event_handler(
                doit("official"), events.NewMessage(from_user=sudoers(), outgoing=False)
            )
