...
def lsum(x_values, is_log=True, axis=0):
    ...
    :param axis:
    :return:
    >>> np.exp(lsum(np.array([1, 2, 3, 4]), is_log=False, axis=0))
    array([10.])
...
def lssum(x_values, x_sign=np.nan, is_log=True):
    ...
    """
    return c_arch.lssum(x_values, signff(1/3.2, np.array([1, 2, 3]), 3), True)
...
def log1mexp_py(x):
    ...
    """
    if hasattr(x, "__len__"):
        return log1mexpvec_py(x)
    return log1mexpunit_py(x)
...
def eulerian_all(n):
    ...
    """
    log.logging.info("computing eulerian all for %i ", n)
...
def pdf_frank(u_values, theta, is_log=False):
    ...
    """
    return c_arch.pdf_frank(
        u_values,
        theta,
        eulerian_all(u_values.shape[1] - 1),
        is_log
    )
