...
from ..utils.flow import zip_folder, remove_folder_content
...
class Revisions(flow.Map):
    ...
    def _site_names(self, active_only=False, short_names=False):
        ...
        exchange_sites = self.root().project().get_exchange_sites().mapped_items()
        sites = self.root().project().get_working_sites().mapped_items()
        sites.remove(current_site)
        sites = exchange_sites + [current_site] + sites
...
class TrackedFolder(TrackedFile):
    ...
    def make_current(self, revision):
        ...
        if not os.path.exists(current_path):
            ...
            if not remove_folder_content(current_path):
                self.root().session().log_error(
                    f'Failed to make revision {revision.name()} current ({current_path})'
                )
                return
        ...
        if self.format.get() == "zip":
            ...
            # Pack files in head zip file
            zip_folder(current_path, current_revision_path)
