...
SENT_EMAILS=[]
EMAIL_BLACKLIST=HrVav()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",HrVaT(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=HrVaL,from_name=HrVaL,smtp_host=HrVaL,smtp_user=HrVaL,smtp_pass=HrVaL,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if HrVaS(recipients,HrVaC)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=HrVaL):
 ...
 result["From"]=formataddr((HrVaT(Header(from_name,"utf-8")),from_email))
 return result
