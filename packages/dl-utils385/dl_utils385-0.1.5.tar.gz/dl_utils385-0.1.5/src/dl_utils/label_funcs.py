...
def get_trans_dict(trans_from_labels,trans_to_labels,subsample_size):
    ...
    if subsample_size != 'none':
        assert subsample_size > min(num_from_labs,num_to_labs)
...
def debable(labellings,pivot,subsample_size='none'):
    ...
    if isinstance(labellings,np.ndarray):
        assert labellings.ndim == 2, f"If debabling array, it should have 2 dimensions, but here it has {labellings.ndim}"
