...
from typing import Any, Callable, Dict, List, Optional, Set, Type, BinaryIO
...
class ID:
    ...
    def from_object(cls, obj: Model) -> 'ID':
        return ID(ID.kind_for_model(obj), obj.pk)

    @staticmethod
    def kind_for_model(obj: Model):
        return f'{obj._meta.app_label}:{obj._meta.model_name}'

    @staticmethod
    def model_for_kind(kind: str) -> Type[Model]:
        return registry.apps.get_model(*kind.split(':'))
