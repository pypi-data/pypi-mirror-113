...
from blackdogosint.utils.timeout import Timeout
import six
import socks
...
def _validator(validator):
    """Validator that is tightly coupled to the implementation of the classes
    here.

    This expects that the object has a ._tls property which is of type
    _DictStack.
    """
...
class Thread(threading.Thread):
    ...
    def __bootstrap(self):
        ...
        This only works because the class is named ``Thread``. If its
        name is changed, we have to implement this hook differently.
        """
...
def _longest(d):
    ...
    sorted by the length of the keys, in descending order. This is useful for
    performing substring matching via ``str.startswith``, as it ensures the
    most complete match will be found.
...
class ContextType(object):
    ...
    def __repr__(self):
        v = sorted(
            '{} = {!r}'.format(k, v)
            for k, v in self._tls._current.items()
        )
    ...
    @property
    def verbose(self):
        """Enable all logging within the enclosed scope. This is the opposite
        of :attr:`.quiet` and functionally equivalent to:
    ...
    @_validator
    def arch(self, arch):
        """Target binary architecture. Allowed values are listed in
        :attr:`pwnlib.context.ContextType.architectures`. Side Effects: If an
        architecture is specified which also implies additional attributes
        (e.g. 'amd64' implies 64-bit words, 'powerpc' implies big-endian),
        these attributes will be set on the context if a user has not already
        set a value. The following properties may be modified.
        ...
        try:
            ...
            raise AttributeError(
                'AttributeError: arch must be one of %r' % sorted(
                    self.architectures,
                ),
            )
    ...
    def aslr(self, aslr):
        ...
        If :const:`False`, attempt to disable ASLR in all processes
        which are created via ``personality`` (``setarch -R``) and
        ``setrlimit`` (``ulimit -s unlimited``). The ``setarch`` changes
    ...
    def kernel(self, arch):
        ...
        Usually, this is the same as ``arch``, except when running a
        32-bit binary on a 64-bit kernel (e.g. i386-on-amd64). Even
        then, this doesn't matter much -- only when the the segment
    ...
    def log_level(self, value):
        ...
        raise AttributeError(
            'log_level must be an integer or one of %r' % permitted,
        )
    ...
    def signed(self, signed):
        ...
        if isinstance(signed, str):
            raise AttributeError(
                'signed must be one of %r or a non-string truthy value' % sorted(
                    self.signednesses,
                ),
            )
    ...
    def timeout(self, value=Timeout.default):
        """Default amount of time to wait for a blocking operation before it
        times out, specified in seconds.

        The default value is to have an infinite timeout. See
        :class:`pwnlib.timeout.Timeout` for additional information on
        valid values.
        """
    ...
    def terminal(self, value):
        ...
        :meth:`pwnlib.util.misc.run_in_new_terminal`.

        Can be a string or an iterable of strings.  In the latter case
        the first entry is the terminal and the rest are default
        arguments.
        """
        if isinstance(value, (bytes, six.text_type)):
            return [value]
    ...
    def proxy(self, proxy):
        """Default proxy for all socket connections. Accepts either a string
        (hostname or IP address) for a SOCKS5 proxy on the default port, **or**
        a ``tuple`` passed to ``socks.set_default_proxy``, e.g.
        ``(socks.SOCKS4, 'localhost', 1234)``.
    ...
    def noptrace(self, value):
        ...
        This is useful for switching between local exploitation with a
        debugger, and remote exploitation (without a debugger). This
        option can be set with the ``NOPTRACE`` command-line argument.
        """
        return bool(value)
    ...
    def adb_host(self, value):
        ...
        This is useful for Android exploitation. The default value is
        inherited from ANDROID_ADB_SERVER_HOST, or set to the default
        'localhost'.
        """
        return str(value)
    ...
    def adb_port(self, value):
        ...
        This is useful for Android exploitation. The default value is
        inherited from ANDROID_ADB_SERVER_PORT, or set to the default
        5037.
        """
        return int(value)
    ...
    def adb(self):
        ...
        Unless ``$ADB_PATH`` is set, uses the default ``adb`` binary in
        ``$PATH``.
        """
    ...
    def buffer_size(self, size):
        """Internal buffer size to use for :class:`pwnlib.tubes.tube.tube`
        objects.

        This is not the maximum size of the buffer, but this is the
        amount of data which is passed to each raw ``read`` syscall (or
        equivalent).
        """
        return int(size)
    ...
    def cache_dir(self):
        ...
        cache = os.path.join(
            xdg_cache_home, '.pwntools-cache-%d.%d' % sys.version_info[:2],
        )
    ...
    def delete_corefiles(self, v):
        ...
        :attr:`.process.corefile`. Default value is ``False``.
        """
        return bool(v)

    @_validator
    def rename_corefiles(self, v):
        """Whether pwntools automatically renames corefiles. This is useful for
        two things:
    ...
    def newline(self, v):
        ...
        This configures the newline emitted by e.g. ``sendline`` or that
        is used as a delimiter for e.g. ``recvline``.
        """
        # circular imports
    ...
    def gdbinit(self, value):
        ...
        This is useful if you want pwntools-launched GDB to include some
        additional modules, like PEDA but you do not want to have GDB
        include them by default. The setting will only apply when GDB is
        launched locally since remote hosts may not have the necessary
        requirements for the gdbinit. If set to an empty string, GDB
        will use the default `~/.gdbinit`. Default value is ``""``.
        """
        return str(value)
    ...
    def setter(*a, **kw):
        with context.local(**{k: kw.pop(k) for k, v in tuple(kw.items()) if isinstance(getattr(ContextType, k, None), property)}):
            ...
            if (arch == 'i386' and bits != 32) \
                ...
                raise AttributeError(
                    'Invalid arch/bits combination: {}/{}'.format(arch, bits),
                )

            if arch in ('i386', 'amd64') and endian == 'big':
                raise AttributeError(
                    'Invalid arch/endianness combination: {}/{}'.format(
                        arch, endian,
                    ),
                )
...
def update_context_defaults(section):
    ...
    from blackdogosint.utils.utils import safeeval
