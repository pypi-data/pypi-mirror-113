...
class Assignment(Base): #pylint: disable=[too-few-public-methods]
    ...
    def evaluate_single_submission(self, submission, exec_prep_code=True):
        ...
        score = 0
        for idx, (question, submission) in enumerate(zip(self.questions, answers)):
            try:
                result = self.check(idx, submission['answer'], locals())
                score += question['score'] * int(result)
            except Exception as e:
                print(f'{e}')
        return score
