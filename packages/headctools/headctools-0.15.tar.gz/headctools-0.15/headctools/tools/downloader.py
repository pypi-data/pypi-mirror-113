...
class Downloader:
    ...
    def download(self):
        # Tentative csv file
        csv_file = os.path.join(DPATH, self.asset + '.csv')
        if not os.path.exists(csv_file):
            avaiable = [f.replace('.csv', '') for f in os.listdir(DPATH) if
                        f.endswith('csv')]
            avaiable_str = ', '.join(avaiable)

            raise AttributeError(f"The asset you want to download does not "
                                 f"correspond with a valid file. ({self.asset}"
                                 f").\nAvaiable assets are: {avaiable_str}.")
