...
def find_fastest_path(graph, start, end, max_speed=100, in_parallel=False):
    ...
    dist, path = Pruning(graph, start, end, max_speed, dist).find_best_path(in_parallel=in_parallel)
...
def solve_optimization(graph, areas, return_path=False):
    ...
    if return_path:
        path = [tuple(graph.start)] + [get_point(*e, l) for e, l in zip(common_edges, lambdas)] + [tuple(graph.end)]
        return dist, path
    else:
        return dist
...
class BrutForce:
    ...
    def find_best_path(self, initial=float('inf'), in_parallel=False):
        best = initial
        best_path = [self.graph.start, self.graph.start]
        ...
        if in_parallel:
            ...
            for path in self.iterate_paths_recursive(self.graph):
                value, path = solve_optimization(self.graph, path, return_path=True)
                print(value)
                if value <= best + 0.01:
                    logging.debug(f'>>>>>> better value: {value}')
                    best = value
...
class Pruning(BrutForce):
    ...
    def prune(self, dist, max_speed):
        ...
        logging.info(f"Prunning {len(unwanted)} areas ({len(self.graph.areas) - len(unwanted)} left).")
