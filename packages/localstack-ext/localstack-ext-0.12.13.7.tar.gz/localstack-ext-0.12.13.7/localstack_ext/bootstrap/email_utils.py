...
SENT_EMAILS=[]
EMAIL_BLACKLIST=eIohc()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",eIohz(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=eIohq,from_name=eIohq,smtp_host=eIohq,smtp_user=eIohq,smtp_pass=eIohq,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if eIohE(recipients,eIoht)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=eIohq):
 ...
 result["From"]=formataddr((eIohz(Header(from_name,"utf-8")),from_email))
 return result
