...
A module-specific logger can be imported into the module via::
    from osintlib.log import getLogger
...
A osintlib root logger named 'osintlib' is created and a custom handler and
...
case. In particular there are tight loops inside :mod:`osintlib.tubes.tube`, which
...
inside osintlib, that will not even emit a record without :data:`context.log_level`
being set to `logging.DEBUG` or below.
Log records created by ``Progress`` and ``Logger`` objects will set
``'osintlib_msgtype'`` on the ``extra`` field to signal which kind of message was
...
reference to the ``Progress`` logger as ``'osintlib_progress'``.
If the custom handler determines that :data:`term.term_mode` is enabled, log
records that have a ``'osintlib_progess'`` in their ``extra`` field will not
...
``'osintlib.tubes.ssh'`` is set to ``'DEBUG'`` no additional output will show up
unless :data:`context.log_level` is also set to ``'DEBUG'``.  Other handlers
will however see the extra log records generated by the ``'osintlib.tubes.ssh'``
logger.
"""
...
from blackdogosint.utils.context import Thread
from blackdogosint.utils.exception import OsintlibException
from blackdogosint.utils.term import spinners
...
# module won't add any escape codes if `osintlib.context.log_console.isatty()` is `False`
_msgtype_prefixes = {
...
# in the `osintlib.term.spinners` module
_spinner_style = text.bold_blue


class Progress(object):
    """Progress logger used to generate log records associated with some
    running job.

    Instances can be used as context managers which will automatically
    declare the running job a success upon exit or a failure upon a
    thrown exception.  After :meth:`success` or :meth:`failure` is
    called the status can no longer be updated. This class is intended
    for internal use.  Progress loggers should be created using
    :meth:`Logger.progress`.
    """
    ...
    def status(self, status, *args, **kwargs):
        """status(status, *args, **kwargs) Logs a status update for the running
        job.

        If the progress logger is animated the status line will be
        updated in place. Status updates are throttled at one update per
        100ms.
        """
        now = time.time()
    ...
    def failure(self, status='Failed', *args, **kwargs):
        """failure(message) Logs that the running job failed.

        No further status updates are allowed. If the Logger is
        animated, the animation is stopped.
        """
...
class Logger(object):
    ...
    class. Also adds some ``osintlib`` flavor:
    ...
    * :meth:`warning_once` (alias :meth:`warn_once`)
    Adds ``osintlib``-specific information for coloring, indentation and progress
    ...
    def __init__(self, logger=None):
        if logger is None:
            ...
            # from a tube (which do not actually reside in the osintlib library)
            # to receive logging abilities that behave as they would expect from
            # the rest of the library
            module = self.__module__
            if not module.startswith('osintlib'):
                module = 'osintlib.' + module
            # - end hack -
    ...
    def _log(self, level, msg, args, kwargs, msgtype, progress=None):
        ...
        extra.setdefault('osintlib_msgtype', msgtype)
        extra.setdefault('osintlib_progress', progress)
        kwargs['extra'] = extra
    ...
    def info_once(self, message, *args, **kwargs):
        """info_once(message, *args, **kwargs) Logs an info message.

        The same message is never printed again.
        """
        m = message % args
    ...
    def warning_once(self, message, *args, **kwargs):
        """warning_once(message, *args, **kwargs) Logs a warning message.
    ...
    def error(self, message, *args, **kwargs):
        """error(message, *args, **kwargs) To be called outside an exception
        handler.

        Logs an error message, then raises a ``osintlibException``.
        """
        self._log(logging.ERROR, message, args, kwargs, 'error')
        raise OsintlibException(message % args)

    def exception(self, message, *args, **kwargs):
        """exception(message, *args, **kwargs) To be called from an exception
        handler.
    ...
    def log(self, level, message, *args, **kwargs):
        """log(level, message, *args, **kwargs) Logs a message with log level
        `level`.

        The ``osintlib`` formatter will use the default :mod:`logging`
        formater to format this message.
        """
        self._log(level, message, args, kwargs, None)

    def isEnabledFor(self, level):
        """isEnabledFor(level) -> bool See if the underlying logger is enabled
    ...
    def addHandler(self, handler):
        """addHandler(handler) Add the specified handler to the underlying
        logger."""
        self._logger.addHandler(handler)

    def removeHandler(self, handler):
        """removeHandler(handler) Remove the specified handler from the
        underlying logger."""
...
class Handler(logging.StreamHandler):
    ...
    This class will report whatever
    ...
    created.
    An instance of this handler is added to the ``'osintlib'`` logger.
    """
    ...
    def emit(self, record):
        ...
        # We have set the root 'osintlib' logger to have a logLevel of 1,
        ...
        progress = getattr(record, 'osintlib_progress', None)
        ...
        msgtype = record.osintlib_msgtype
        record.osintlib_msgtype = 'animated'
        msg = '%s\n' % self.format(record)
...
class Formatter(logging.Formatter):
    ...
    containing the ``'osintlib_msgtype'`` attribute.  Other records are
    formatted using the `logging` modules default formatter. If
    ``'osintlib_msgtype'`` is set, it performs the following actions:
    ...
    * If the message spans multiple lines they are split, and all subsequent
      lines are indented.
    This formatter is used by the handler installed on the ``'osintlib'`` logger.
    """
    ...
    def format(self, record):
        ...
        msgtype = getattr(record, 'osintlib_msgtype', None)

        # if 'osintlib_msgtype' is not set (or set to `None`) we just return the
        # message as it is
        if msgtype is None:
            return msg
...
def _need_text(s):
    ...
    global _need_text
    from blackdogosint.utils.utils.packing import _need_text
...
#
# The root 'osintlib' logger is declared here.  To change the target of all
...
def install_default_handler():
    """install_default_handler() Instantiates a :class:`Handler` and
    :class:`Formatter` and installs them for the ``osintlib`` root logger.

    This function is automatically called from when importing
    :mod:`pwn`.
    """
    logger = logging.getLogger('osintlib')
