...
from thegraph_handlers.uniswap_v2.queries import (
    burns_query,
...
class UniswapV2:
    ...
    async def yield_liquidity_pool_shares(
        ...
        try:
            ...
            logger.error("Internal exception for Uniswap v2 fetching - %s", exc)
            raise exc from None
        except Exception as exc:
            logger.error("HTTP exception for Uniswap v2 fetching - %s", exc)
            raise exc from None

        for position in response["result"]:
            yield parse_liquidity_position(position)
    ...
    async def yield_burns(
        ...
        async for d in self._yield(
            burns_query, parse_burn, address, since, until, limit, excluded_txs
        ):
            yield d
    ...
    async def yield_mints(
        ...
        async for d in self._yield(
            mints_query, parse_mint, address, since, until, limit, excluded_txs
        ):
            yield d
    ...
    async def yield_swaps(
        ...
        async for d in self._yield(
            swaps_query, parse_swap, address, since, until, limit, excluded_txs
        ):
            yield d
