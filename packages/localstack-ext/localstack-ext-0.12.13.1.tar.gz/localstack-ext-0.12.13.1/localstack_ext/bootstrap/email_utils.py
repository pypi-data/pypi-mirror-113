...
SENT_EMAILS=[]
EMAIL_BLACKLIST=NJcpL()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r'\s+','',NJcpC(email or ''))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=NJcpz,from_name=NJcpz,smtp_host=NJcpz,smtp_user=NJcpz,smtp_pass=NJcpz,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if NJcpe(recipients,NJcpY)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=NJcpz):
 ...
 result['From']=formataddr((NJcpC(Header(from_name,'utf-8')),from_email))
 return result
