    ...
    def get_product_price_part(self, begin_date, end_date, time_para, product_list: Tuple[str] = (), manager_id=None):
        try:
            ...
            if not _df2.empty > 0:
                ...
                _df = _df1.copy()
            _df3 = derived.get_fof_nav_calc(manager_id=manager_id, fof_id_list=real_ids)
            if not _df3.empty > 0:
                _df3 = _df3.pivot_table(index='datetime',columns='fof_id',values='adjusted_nav')
                if not _df.empty:
                    res = []
                    for fund_id in _df3:
                        if fund_id in _df:
                            b_d = _df[fund_id].dropna().index[0]
                            e_d = _df[fund_id].dropna().index[-1]
                            df_part1 = _df3[[fund_id]].loc[:b_d]
                            df_part2 = _df3[[fund_id]].loc[e_d:]
                            _res = [_df[[fund_id]],df_part1,df_part2]
                            _res = [i for i in _res if not i.empty]
                            _df_ = pd.concat(_res).sort_index()
                            _df_ = _df_[~_df_.index.duplicated(keep='first')]
                            res.append(_df_)
                    res_tmp = [_df[[fund_id]] for fund_id in _df if fund_id not in _df3]
                    res = res+res_tmp
                    res = [i.dropna() for i in res]
                    _df = pd.concat(res,axis=1).sort_index()
                else:
