...
class SimulationManager:
    ...
    def __init__(self, uid: str) -> None:
        ...
        :param uid: Session identifier for this simulation run (e.g. '20210512-145742').
        """
        self.lock = Lock()
        ...
        self.__network_manager = NetworkManager(self.uid)
    ...
    def stop_single_node(self, node: Node, namespace_id: str = "") -> None:
        ...
        try:
            ...
            if not self._check_api_available():
                # Do nothing if the API cannot be reached
                return
    ...
    def cleanup(self, namespace_id: str = "") -> None:
        """
        ...
        for node in self.nodes.values():
            ...
            node.status = NodeStatus.DOWN
        self._remove_k8s_namespace(namespace_id)
    ...
    def _execute_node_pre_deploy_hooks(self, node: Node) -> None:
        ...
        if not self.is_finalized:
            ...
            return None
        try:
            for hook in node.config.hooks["pre-deploy"]:
    ...
    def _execute_node_post_deploy_hooks(self, node: Node) -> None:
        ...
        if not self.is_finalized:
            ...
            return None
        try:
            for hook in node.config.hooks["post-deploy"]:
    ...
    def _execute_node_connect_hooks(self, source_node: Node, target_node: Node, **kwargs: Any) -> None:
        ...
        if not self.is_finalized:
            ...
            return None
        try:
            for hook in source_node.config.hooks["connect"]:
    ...
    def _execute_node_post_connect_hooks(self, node: Node) -> None:
        ...
        if not self.is_finalized:
            ...
            return None
        try:
            for hook in node.config.hooks["post-connect"]:
    ...
    def _execute_node_pre_teardown_hooks(self, node: Node) -> None:
        ...
        if not self.is_finalized:
            ...
            return None
        try:
            for hook in node.config.hooks["pre-teardown"]:
    ...
    def _remove_k8s_namespace(self, namespace_id: str, propagation_policy: str = "Foreground") -> None:
        """
