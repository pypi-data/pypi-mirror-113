...
def extract_polygon_outline(shapely_geometry: shapely.geometry) -> np.ndarray:
    """
    Extract the key outline coordinates of a shapely shape (including multi-shapes like MultiPolygon). The following
    ...
    if shapely_geometry.geom_type == 'Polygon':
        ...
        polygon_outline = max(shapely_geometry, key=lambda a: a.area).exterior.coords.xy

    elif shapely_geometry.geom_type == 'GeometryCollection':
        # extract polygons
        polygons = []
        for geometry in shapely_geometry.geoms:
            if geometry.geom_type == 'Polygon' and not shapely_geometry.is_empty:
                polygons.append(geometry)

        # extract largest polygon
        if polygons:
            polygon_outline = max(polygons, key=lambda a: a.area).exterior.coords.xy
        ...
        raise ValueError("Faced unsupported shape '" + str(shapely_geometry.geom_type) + "'!")

    # convert to numpy array
    if polygon_outline is not None:
        polygon_outline = np.column_stack((polygon_outline[0], polygon_outline[1]))
