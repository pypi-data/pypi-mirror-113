...
from simba.reorganize_animal_bp import display_original_bp_list
...
class loadprojectini:
    def __init__(self,configini):
        ...
        button_unsupervised = Button(lbl_addon,text='Unsupervised',command = self.unsupervisedInterface)
    ...
    def unsupervisedInterface(self):
        window = Toplevel()
        ...
        saveProjectFolder = LabelFrame(tab1, text="1.Save Project Folder", font=("Helvetica", 12, 'bold'), padx=10,
                                       pady=10, fg='black')
        self.folderPath = FolderSelect(saveProjectFolder, "Folder path", lblwidth=12)
        projectname = EntryBox(saveProjectFolder, 'Project Name', '12')
        btnCreateFolder = Button(saveProjectFolder, text="Create folder", command=lambda: self.createFolder(projectname.entry_get))
...
class reorganizeData:
    def __init__(self):
        ...
        self.roToplevel.minsize(300,300)
        self.roToplevel.wm_title('Reorganize Tracking Data')

        roLabelframe = LabelFrame(self.roToplevel,text='Set File Settings',font=('Helvetica',10,'bold'),pady=5,padx=5)
        fFolder = FolderSelect(roLabelframe,'Data Folder',lblwidth='10')
        poseTool = DropDownMenu(roLabelframe,'Tracking tool',['DLC'],'10')
        poseTool.setChoices('DLC')
        fileFormat = DropDownMenu(roLabelframe,'File type',['csv','h5'],'10')
        fileFormat.setChoices('h5')

        btnConfirm = Button(roLabelframe,text='Confirm',command= lambda: self.confirm(fFolder.folder_path,poseTool.getChoices(),fileFormat.getChoices()))
        # btnreorganize = Button(self.roToplevel,text='Reorganize Data',command = )

        #organize
        roLabelframe.grid(row=0,sticky=W)
        fFolder.grid(row=0,sticky=W,columnspan=3)
        poseTool.grid(row=1,sticky=W)
        fileFormat.grid(row=2,sticky=W)
    ...
    def confirm(self,folder,posetool,fileformat):
        ...
        animallist, bplist, headerlist = display_original_bp_list(folder,posetool,fileformat)
        self.frame = Frame(self.roToplevel)
        ...
        idx1, idx2, oldanimallist, oldbplist, self.newbplist = ([0]*len(bplist) for i in range(5)) #create lists
        ...
        if animallist:
            for i in range(len(bplist)):
                ...
                oldbplist[i].grid(row=i,column=2,sticky=W)

                #new order
                idx2[i] = Label(self.table2,text=str(i+1) + '.')
                self.newbplist[i] = DropDownMenu(self.table2,str(animallist[i]),bplist,'10')
                self.newbplist[i].setChoices(bplist[i])

                idx2[i].grid(row=i,column=0,sticky=W)
                self.newbplist[i].grid(row=i,column=1,sticky=W)
        else:
            print(bplist)
