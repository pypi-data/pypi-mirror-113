...
from worker.utils import add_sys_path, get_pid_dir, get_working_dir, is_windows
...
class Task:
    ...
    def load_action(self):
        ...
        # Temporarily add `working_dir` to sys.path
        with add_sys_path(get_working_dir()):
            mod = importlib.import_module(
                f"packs.{self.action_config.pack}.{action_module}"
            )
    ...
    def run_action(self):
        ...
        try:
            ...
            if pidfile.exists() and is_windows():
                pidfile.unlink()
