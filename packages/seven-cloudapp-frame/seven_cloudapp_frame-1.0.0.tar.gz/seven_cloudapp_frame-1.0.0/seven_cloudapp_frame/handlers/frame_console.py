...
@LastEditTime: 2021-07-19 17:18:15
...
class FrameConsole():
    ...
    def console_asset_warn(self):
        ...
        for i in range(mod_count):
            ...
            t = threading.Thread(target=self._process_user_asset_warn, args=[i, mod_count])
            t.start()

            j = multiprocessing.Process(target=self._process_asset_inventory_warn, args=[i, mod_count])
            j.start()

    def _process_user_asset_warn(self,mod_value, mod_count):
        """
        ...
        :last_editors: HuangJianYi
        """
        user_asset_model = UserAssetModel()
