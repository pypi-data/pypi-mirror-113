...
class TestInitCreate(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.token = "ac7a44b7be181774bd088c0099afd449b26bbeb7"
    ...
    def test_init_project(self,):
        clt = client.Client.Project(self.token, self.host)
        self.assertEqual(clt.auth, {"Authorization": "Token " + self.token})
        self.assertIsNone(clt.organization_name)
    ...
    def test_project(self,):
        clt = client.Client.Project(self.token, self.host)
        project = clt.create(project_name="test_project_1")
        ...
        self.assertEqual(resp["description"], "test project description")
...
class TestDataset(unittest.TestCase):
    ...
    def test_fetch_dataset(self,):
        ...
        dataset_infos = dataset.fetch("test_dataset_test")
        self.assertEqual(dataset_infos["dataset_name"], "test_dataset_test")
    ...
    def test_push_dataset(self,):
        ...
        with self.assertRaises(pxl_exceptions.InvalidQueryError):
            datalake.push_dataset("test_dataset_test_3", annotations_path="tests/not_a_file.json", imgdir="tests/test_push_dataset_dir") # test no json
        datalake.push_dataset("test_dataset_test_3", annotations_path="tests/test_annotations.json", imgdir="tests/test_push_dataset_dir", tags=["test_push"]) # test normal
        picture = client.Client.Datalake.Picture(self.token, host=self.host)
        picture.fetch(2, ["test_push"])
        dataset = client.Client.Datalake.Dataset(self.token, host=self.host)
        dataset.create(name="test_push_dataset_test", pictures=picture.fetched_pictures)
        datalake.push_dataset("test_push_dataset_test", annotations_path="tests/test_annotations.json", only_annotations=True) # test annotation only
        with self.assertRaises(pxl_exceptions.InvalidQueryError):
            datalake.push_dataset("test_dataset_test_3", annotations_path="tests/not_a_file.json", only_annotations=True) # test no json
        with self.assertRaises(pxl_exceptions.InvalidQueryError):
            datalake.push_dataset("test_dataset_test_3", annotations_path="tests/test_annotations_wrong.json") # missing images 'key' in json
        with self.assertRaises(pxl_exceptions.ResourceNotFoundError):
            datalake.push_dataset("test_dataset_test_3", annotations_path="tests/test_annotations_wrong.json", imgdir="") # missing images 'key' in json
    ...
    def test_labels_and_annotation(self,):
        dataset = client.Client.Datalake.Dataset(self.token, host=self.host)
        with self.assertRaises(pxl_exceptions.NetworkError):
...
class TestExperimentCrud(unittest.TestCase):
    ...
    def test_experiment(self,):
        ...
        experiment = client.Client.Experiment(self.token, self.host, project.id)
...
class TestExperiment(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        cls.host = "http://127.0.0.1:8000/sdk/v2/"
        cls.token = "ac7a44b7be181774bd088c0099afd449b26bbeb7"
        project = client.Client.Project(cls.token, cls.host)
        project = project.get("test_project_1")
        cls.project_token = project.id
...
class TestDelete(unittest.TestCase):
    ...
    def test_delete_project(self,):
        ...
        project = clt.get("test_project_1")
        resp = clt.delete()
        self.assertEqual(resp["success"], "project deleted")
        picture = client.Client.Datalake.Picture(self.token, host=self.host)
        picture.fetch(6, ["test_test"])
        picture.delete()
        picture.fetch(1, ["test_test_single"])
