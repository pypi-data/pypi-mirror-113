...
import { AlertDialog, Card, Flex } from '@chiadoge/core';
import {
  send_transaction,
...
const IncompleteCard = (props) => {
  ...
  function submit() {
    ...
    const chiadogeper_input = ip_parsed.limit;
    ...
    const chiadogeper_value = Number.parseInt(Number(chiadogeper_input));
        ...
        <Grid item xs={12}>
          <Box display="flex">
            ...
            <Box flexGrow={1}>
              <Typography variant="subtitle1">
                <Trans>
                  Spending Limit (chiadoge per interval):{' '}
                  {mojo_to_chiadoge_string(limit)}
                </Trans>
              </Typography>
            </Box>
          </Box>
...
const SendCard = (props) => {
  ...
  function send() {
    ...
    const fee = chiadoge_to_mojo(fee_input.value);
