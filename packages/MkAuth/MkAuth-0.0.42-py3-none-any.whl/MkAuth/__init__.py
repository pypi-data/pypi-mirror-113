...
def RequestLogin(request, Ltype, LoggedInRedirect, FailureRedirect, UserNonExistingRedirect, CheckVerified=False, Premissions=[], ActiveDuration=15,  AllowUntrustedDevices=False,  NewDeviceActionDef=""):
    ...
    Auth = UserLogin(creds['username'], creds['password'])

    if 'Username' not in Auth:
        try:
            return redirect(UserNonExistingRedirect(creds['username']))
        except:
    ...
    UserSes = UserSession(request, creds['username'], Auth['data'],  Premissions, ActiveDuration)
    try:
        resp = make_response(redirect(LoggedInRedirect(creds['username'])))
    except:
