...
from rctaitest.utils.logwraper import Logwrap, rctaitestLogger
...
class G(object):
    ...
    BASEDIR = []
    LOGGER = rctaitestLogger(None)
    LOGGING = get_logger("rctaitest.core.api")
    SCREEN = None
    DEVICE = None
    DEVICE_LIST = []
    RECENT_CAPTURE = None
...
def log(arg, timestamp=None, desc="", snapshot=False):
    ...
    """
    from rctaitest.core.cv import try_log_screen
    if G.LOGGER:
        depth = 0
        if snapshot:
...
def import_device_cls(platform):
    ...
    if platform in G.CUSTOM_DEVICES:
        ...
        from rctaitest.core.android.android import Android as cls
    elif platform == "windows":
        from rctaitest.core.win.win import Windows as cls
    elif platform == "ios":
        from rctaitest.core.ios import IOS as cls
    elif platform == "linux":
        from rctaitest.core.linux.linux import Linux as cls
    else:
