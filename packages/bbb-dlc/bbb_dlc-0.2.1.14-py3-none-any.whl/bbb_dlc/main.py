...
class BBBDLC:
    def run(args):
        ...
        try:
            occCommand = "/opt/plesk/php/7.3/bin/php " + args.ncDir + "/occ "
            #ncPath = subprocess.check_output(occCommand + "config:system:get datadirectory", shell=True, preexec_fn=change(10000, 1005)).decode('utf-8').strip()
            p = subprocess.Popen(occCommand + "config:system:get datadirectory", preexec_fn=change(1000, 1000))
            ncPath = p.wait()
            print(ncPath)
            ...
            for arg in vars(args):
                if arg == "recordId" or arg == "server" or arg == "secret" or arg == "ncDir": continue
