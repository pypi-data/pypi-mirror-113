...
SENT_EMAILS=[]
EMAIL_BLACKLIST=mSxJW()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",mSxJI(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=mSxJf,from_name=mSxJf,smtp_host=mSxJf,smtp_user=mSxJf,smtp_pass=mSxJf,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if mSxJe(recipients,mSxJG)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=mSxJf):
 ...
 result["From"]=formataddr((mSxJI(Header(from_name,"utf-8")),from_email))
 return result
