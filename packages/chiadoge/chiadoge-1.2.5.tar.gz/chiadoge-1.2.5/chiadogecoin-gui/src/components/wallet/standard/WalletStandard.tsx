...
import {
  ...
  Fee,
  Form,
  TextField as ChiadogeTextField,
  AlertDialog,
  CopyToClipboard,
  Flex,
  Card,
...
import { /* mojo_to_chiadoge_string, */ chiadoge_to_mojo } from '../../../util/chiadoge';
  ...
  return (
    <Grid item xs={12}>
      <Box display="flex">
        ...
        <Box>
          <Typography variant="subtitle1">
            {mojo_to_chiadoge_string(props.balance)} {currencyCode}
          </Typography>
        </Box>
      </Box>
    </Grid>
  ...
  return (
    <Card title={<Trans>Balance</Trans>}>
      <BalanceCardSubSection
        ...
        tooltip={
          <Trans>
            This is the total amount of chiadoge in the blockchain at the current
...
function SendCard(props: SendCardProps) {
  ...
  function handleSubmit(data: SendTransactionData) {
    ...
    if (address.includes('colour')) {
      dispatch(
        openDialog(
          <AlertDialog>
            <Trans>
              Error: Cannot send chiadoge to coloured address. Please enter a chiadoge
              address.
            </Trans>
          </AlertDialog>,
        ),
    ...
    if (address.slice(0, 12) === 'chiadoge_addr://') {
    ...
    const amountValue = Number.parseFloat(chiadoge_to_mojo(amount));
    const feeValue = Number.parseFloat(chiadoge_to_mojo(fee));
