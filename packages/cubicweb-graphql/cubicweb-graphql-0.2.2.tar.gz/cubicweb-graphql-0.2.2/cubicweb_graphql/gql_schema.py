...
class GraphQLSchemaTypes:
    ...
    def _get_union(self, members):
        ...
        union_members = {
            _TYPE_META: type(_TYPE_META, (object,), {"types": member_types})
        }
    ...
    def _create_entity_object_type(self, entity_schema):
        ...
        :return: Nothing
        """
        fields = {_TYPE_FIELD_SCHEMA: entity_schema}  # Data fields
            ...
            if relation.type == "eid":
                ...
                def resolver_lambda(
                    ...
                    **kwargs
                    ...
                    return resolver.resolve_primitive_field(
                        instance, relation_name, graphene.ID, info, *args, **kwargs
                    )

                fields["eid"] = graphene.Field(
                    graphene.ID,
                    name="eid",
                    resolver=resolver_lambda,
                )
            elif relation.final:
                ...
                if field_type in _PRIMITIVES:
                    ...
                    def resolver_lambda(
                        ...
                        **kwargs
                        ...
                        return resolver.resolve_primitive_field(
                            instance, relation_name, target_type, info, *args, **kwargs
                        )
                ...
                definition = entity_schema.rdef(
                    relation, role="subject", targettype=objects[0]
                )
                is_multi = (
                    definition.cardinality[1] == "+" or definition.cardinality[1] == "*"
                )
                if len(objects) > 1:
                    target_type = lambda objects=objects, data=self: data._get_union(
                        objects
                    )
                    arguments = self._get_arguments_union(objects)
                else:
                    target_type = lambda entities=self.entities, n=objects[
                        0
                    ].type: entities[n]
                ...
                if is_multi:
                    ...
                    def resolver_lambda(
                        ...
                        **kwargs
                        ...
                        return resolver.resolve_vector_object_field(
                            instance,
                            relation_name,
                            target_type(),
                            info,
                    ...
                    def resolver_lambda(
                        ...
                        **kwargs
                        ...
                        return resolver.resolve_scalar_object_field(
                            instance,
                            relation_name,
                            target_type(),
                            info,
        ...
        if len(sub_entities) > 0:
            ...
            fields[TYPE_FIELD_META] = type(
                _TYPE_META, (object,), {"name": entity_schema.type + _TYPE_FIELDS}
            )
            if super_entity is not None:
                # Intermediate entity (is a specialization and can be specialized)
                super_type_fields = (
                    self.blobs[super_entity.type + _TYPE_FIELDS]
                    if super_entity is not None
                    else None
                )
                class_fields = type(
                    entity_schema.type + _TYPE_FIELDS, (super_type_fields,), fields
                )
            else:
                # Can be specialized, but is not a specialization
                class_fields = type(
                    entity_schema.type + _TYPE_FIELDS, (object,), fields
                )
            class_entity = type(
                entity_schema.type, (graphene.ObjectType, class_fields), {}
            )
    ...
    def _create_entity_input_type(self, entity_schema):
        ...
        for relation in entity_schema.subject_relations():
            if relation.type == "eid":
                # eid is treated as the identifier
                fields["eid"] = graphene.Field(graphene.ID, name="eid")
            elif relation.final:
                # handle a primitive type
                field_type = relation.objects(entity_schema)[0].type
                if field_type in _PRIMITIVES:
                    target_type = _PRIMITIVES[field_type]
                    fields[relation.type] = graphene.Field(
                        target_type, name=relation.type
                    )
            else:
                # handle a reference to another entity
                objects = relation.objects(entity_schema)
                if len(objects) > 1:
                    for target_schema in objects:
                        ...
                        def target_type(
                            inputs=self.inputs, n=_TYPE_INPUT + target_schema.type
                        ):
                            return inputs[n]

                        name = relation.type + FIELD_SUB_SEPARATOR + target_schema.type
                        fields[name] = graphene.Field(target_type, name=name)
                else:

                    def target_type(
                        inputs=self.inputs, n=_TYPE_INPUT + objects[0].type
                    ):
                        return inputs[n]
        ...
        class_entity = type(
            _TYPE_INPUT + entity_schema.type, (graphene.InputObjectType,), fields
        )
        ...
        for relation in entity_schema.subject_relations():
            if relation.type == "eid":
                ...
                arguments["eid"] = graphene.Argument(
                    graphene.ID, name="eid", required=False
                )
            elif relation.final:
                # handle a primitive type
                field_type = relation.objects(entity_schema)[0].type
                if field_type in _PRIMITIVES:
                    ...
                    arguments[relation.type] = graphene.Argument(
                        target_type, name=relation.type, required=False
                    )
            else:
                # handle a reference to another entity
                objects = relation.objects(entity_schema)
                if len(objects) > 1:
                    for target_schema in objects:
                        ...
                        arguments[name] = graphene.Argument(
                            target_type, name=name, required=False
                        )
                else:
    ...
    def _create_entity_query_field(self, entity_schema):
        ...
        def resolver_lambda(
            ...
            **kwargs
            ...
            return resolver.resolve_entities(entity_type, self, info, *args, **kwargs)
    ...
    def _create_query_type(self):
        ...
        self.query_type = type(
            self.schema.name + _TYPE_QUERY, (graphene.ObjectType,), members
        )
    ...
    def _create_entity_creator_field(self, entity_schema):
        ...
        def resolver_lambda(
            ...
            **kwargs
            ...
            return resolver.on_create_entity(self, info, entity_type, **kwargs)
    ...
    def _create_entity_updater_field(self, entity_schema):
        ...
        def resolver_lambda(
            ...
            **kwargs
            ...
            return resolver.on_update_entity(self, info, entity_type, **kwargs)
    ...
    def _create_entity_deleter_field(self, entity_schema):
        ...
        def resolver_lambda(
            ...
            **kwargs
            ...
            return resolver.on_delete_entity(self, info, entity_type, **kwargs)
    ...
    def _create_entity_relation_deleter_field(self, entity_schema):
        ...
        def resolver_lambda(
            ...
            **kwargs
            ...
            return resolver.on_delete_relation(self, info, entity_type, **kwargs)
    ...
    def _create_mutation_type(self):
        ...
        for entity in self.schema.entities():
            if entity.final:
                continue
            members[_FIELD_CREATE + entity.type] = self._create_entity_creator_field(
                entity
            )
            members[_FIELD_UPDATE + entity.type] = self._create_entity_updater_field(
                entity
            )
            members[_FIELD_DELETE + entity.type] = self._create_entity_deleter_field(
                entity
            )
            members[
                _FIELD_DELETE_FROM + entity.type
            ] = self._create_entity_relation_deleter_field(entity)
        self.mutation_type = type(
            self.schema.name + _TYPE_MUTATION, (graphene.ObjectType,), members
        )
    ...
    def _produce(self):
        ...
        self.graphql_schema = graphene.Schema(
