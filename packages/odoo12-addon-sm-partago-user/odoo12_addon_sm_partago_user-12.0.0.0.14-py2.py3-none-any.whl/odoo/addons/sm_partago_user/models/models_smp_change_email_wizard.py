...
class sm_change_email_wizard(models.TransientModel):
  ...
  change_in_odoo = fields.Boolean(string=_("Change email in Odoo"), default=True)
  ...
  def change_email_action(self):
    ...
    db_utils.firebase_update('persons', member.cs_person_index, {'email' : self.new_email})
    if self.change_in_odoo:
      member.email = self.new_email
    self.env['res.partner'].resend_register_email_action()
    return True
