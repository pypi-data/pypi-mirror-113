...
def batch_fetch(cfgfile,num=10,debug=False) :
    ...
    with open(cfgfile,"r") as f :
        thefirst = True
        for ln in random.shuffle(f.readlines()) :
            ...
            if m :
                if thefirst :
                    ...
                    n = 10+random.choice([i for i in range(30)])
                    print("# sleep : {}".format(n), file=sys.stderr, flush=True)
                    time.sleep(n)
                title = m.group(1)
                tgtdir = m.group(2)
                page = m.group(3)
                ...
                try :
                    report += adhoc_fetch(title,page,num,tgtdir,debug) + "\n\n"
                except :
                    traceback.print_exc()
    return report
