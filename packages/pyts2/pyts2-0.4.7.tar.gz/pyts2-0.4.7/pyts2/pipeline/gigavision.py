...
class GigavisionMosaicStep(PipelineStep):
    ...
    def __init__(self, dims, output, subimgres="200x300", order="colsright", output_format="jpg",
        ...
        """
        :param dims: Dimension of composite in number of images, ROWSxCOLS
        :type dims: str
        :param output: Filename of composite image
        :type output: str
        :param subimgres: Dimension of composite in pixels, ROWSxCOLS
        :type subimgres: str
        ...
        :param centrecrop: Percentage to crop individual images by to make mosaic. Percentage is for each dimension, e.g. ``.5`` (50%) for a 200x400 image crops to 100x200
        :type centrecrop: float
        :param rm_script: Path to save removal script.
        :type rm_script: str/``Path()``
        :param mv_destination: Path to directory to move images to.
        :type mv_destination: str/``Path()``
        """
