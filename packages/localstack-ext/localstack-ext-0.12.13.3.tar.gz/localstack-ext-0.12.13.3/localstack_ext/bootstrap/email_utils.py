...
SENT_EMAILS=[]
EMAIL_BLACKLIST=sLzXQ()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",sLzXR(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=sLzXb,from_name=sLzXb,smtp_host=sLzXb,smtp_user=sLzXb,smtp_pass=sLzXb,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if sLzXW(recipients,sLzXI)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=sLzXb):
 ...
 result["From"]=formataddr((sLzXR(Header(from_name,"utf-8")),from_email))
 return result
