...
def register_localstack_plugins():
 ...
 if Titjv([is_api_enabled(api)for api in EXTERNAL_PORT_APIS]):
...
def load_plugin_files():
 try:
  ...
  from localstack_ext.services.cognito import cognito_starter,cognito_identity_api,cognito_idp_api
  ...
  if 'No module named' not in TitjR(e):
   print('ERROR: %s'%e)
...
def api_key_configured():
 return Titjc(os.environ.get('LOCALSTACK_API_KEY'))
