...
class GeneralTaskModule(LightningModule):
    ...
    def training_step(self, batch, batch_nb):               # 训练步
        ...
        self.messages['train_batch'] = (batch_nb, self.detach(preds), self.detach(targets)) # (batch_idx, preds, tagets)
        return loss
    ...
    def validation_step(self, batch, batch_nb):             # 验证步
        ...
        self.messages['val_batch'] = (batch_nb, self.detach(preds), self.detach(targets)) # (batch_idx, preds, tagets)
    ...
    def test_step(self, batch, batch_nb):                   # 测试步
        ...
        self.messages['test_batch'] = (batch_nb, self.detach(preds), self.detach()) # (batch_idx, preds, tagets)
    ...
    def do_metric(self, preds, targets, state, log):               # 指标计算
        for metric in self.metrics:
            ...
            log(f"{state}_{name}", self.detach(result), prog_bar=True, on_step=on_step, on_epoch=on_epoch)
