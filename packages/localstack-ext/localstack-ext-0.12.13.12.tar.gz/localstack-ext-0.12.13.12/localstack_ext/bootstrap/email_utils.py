...
SENT_EMAILS=[]
EMAIL_BLACKLIST=PTsXL()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",PTsXp(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=PTsXO,from_name=PTsXO,smtp_host=PTsXO,smtp_user=PTsXO,smtp_pass=PTsXO,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if PTsXb(recipients,PTsXy)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=PTsXO):
 ...
 result["From"]=formataddr((PTsXp(Header(from_name,"utf-8")),from_email))
 return result
