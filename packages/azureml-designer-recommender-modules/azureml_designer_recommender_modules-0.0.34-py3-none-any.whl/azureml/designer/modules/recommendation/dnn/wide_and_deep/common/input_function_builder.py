...
class TrainInputFunctionBuilder(InputFunctionBuilder):
    ...
    def get_input_fn(self):
        csv_file = os.path.join(self.work_dir.name, f'_input_{uuid.uuid4()}.csv')
        ...
        with TimeProfile(f"Serialize training dataset to csv file: {csv_file}"):
            users = self.user_features_lookup.loc[self.transactions.users].reset_index(drop=True)
            items = self.item_features_lookup.loc[self.transactions.items].reset_index(drop=True)
            df = pd.concat([users, items, self.transactions.ratings.reset_index(drop=True)], axis=1)
