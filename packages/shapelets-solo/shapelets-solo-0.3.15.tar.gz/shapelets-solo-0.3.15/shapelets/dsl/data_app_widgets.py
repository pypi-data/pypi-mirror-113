...
class LineChart(Widget):
    def __init__(self,
        ...
        if self.sequence:
            widget_type = AttributeNames.SEQUENCE.value.capitalize()
        if self.y_axis:
            widget_type = AttributeNames.NDARRAY.value
        super().__init__(f"{self.__class__.__name__}:{widget_type}", "LineChart", **additional)
