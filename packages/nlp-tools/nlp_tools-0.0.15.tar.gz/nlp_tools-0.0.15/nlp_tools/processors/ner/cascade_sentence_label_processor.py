...
from nlp_tools.utils.segment_utils import get_words
...
class CascadeSentenceLabelProcessor(ABCLabelProcessor):
    ...
    def trans_label(self,segment_labels_id, attr_labels_id):
        ...
        # way two
        std_labels = []
        for segments,attrs in zip(segment_labels_id,attr_labels_id):
            words = get_words(segments)
            labels = ['O'] * len(attrs)
            for word in words:
                word_label = attrs[word[1]]
                labels[word[0]] = "B-" +word_label
                for idx in range(word[0]+1,word[1]+1):
                    labels[idx] = "I-" + word_label
            std_labels.append(labels)
        return std_labels
