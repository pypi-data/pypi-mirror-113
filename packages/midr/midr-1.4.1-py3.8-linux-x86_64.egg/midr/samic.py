...
def expectation_k(u_values, params_list, dcopula=None):
    ...
    return (
        ...
        np.log(
            np.exp(params_list['pi']) + (1 - np.exp(params_list['pi'])) *
            np.exp(k_state)
        )
    )
...
def minimize_theta(u_values, copula, params_list, dcopula=None,
    ...
    :return:
    >>> minimize_theta(
    ... compute_empirical_marginal_cdf(
    ...    compute_rank(np.random.rand(10000, 30))),
    ...    copula="frank",
    ...    params_list={
    ...        'pi': np.log(0.01),
...
def samic_minimize(u_values, params_list):
    """
