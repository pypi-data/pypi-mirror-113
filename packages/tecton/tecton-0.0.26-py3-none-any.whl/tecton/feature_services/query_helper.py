...
class _QueryHelper:
    ...
    def get_feature_vector(
        self,
        join_keys: Dict[str, Union[int, str, bool]],
        include_join_keys_in_response: bool,
        request_context_map: Dict[str, Union[int, str, float, bool]],
    ...
    def _response_to_feature_vector(
        self, response: GetFeaturesResponse, include_join_keys: bool, join_keys: Dict
    ) -> FeatureVector:
        features = {}
        ...
        return FeatureVector(names=list(features.keys()), values=list(features.values()))
    ...
    def _query_response_to_pandas(self, response: QueryFeaturesResponse, join_keys: Dict[str, Union[int, str, bytes]]):
    ...
    def _python_to_pb_value(self, key: str, api_value: "Value", python_value: Union[int, str, bool], schema_type=None):
    ...
    def _request_context_to_pb_value(
        self, key: str, api_value: "Value", python_value: Union[int, str, bool, float], schema_type
    ):
        if schema_type == BooleanType():
            ...
            if not isinstance(python_value, int):
                raise TypeError(f"Invalid type for {key}: expected int, got {type(python_value).__name__}")
            api_value.string_value = str(python_value)
            ...
            if not (isinstance(python_value, int) or isinstance(python_value, float)):
