...
export default function Block() {
  ...
  const poolReward = mojo_to_chiadoge(calculatePoolReward(blockRecord.height));
  const baseFarmerReward = mojo_to_chiadoge(
    calculateBaseFarmerReward(blockRecord.height),
  );

  const chiadogeFees = blockRecord.fees
    ? mojo_to_chiadoge(BigInt(blockRecord.fees))
    : '';
  ...
  const rows = [
    ...
    },
    {
      ...
      value: (
        <Link
          target="_blank"
          href={`https://www.chiadogeexplorer.com/blockchain/puzzlehash/${blockRecord.farmer_puzzle_hash}`}
        >
          {currencyCode
            ? toBech32m(
    ...
    },
    {
      ...
      value: (
        <Link
          target="_blank"
          href={`https://www.chiadogeexplorer.com/blockchain/puzzlehash/${blockRecord.pool_puzzle_hash}`}
        >
          {currencyCode
            ? toBech32m(
    ...
    },
    {
      ...
      value: chiadogeFees ? `${chiadogeFees} ${currencyCode}` : '',
      tooltip: (
        <Trans>
    ...
    <LayoutMain title={<Trans>Block</Trans>}>
      <Card
        title={
          <BlockTitle>
            <Trans>
              Block at height {blockRecord.height} in the Chiadogecoin
            </Trans>
          </BlockTitle>
        }
        action={
