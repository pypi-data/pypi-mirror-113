...
@patch(f"{class_path}._fetch_default_country", return_value={'@id': 'global'})
...
def test_aggregate(mock_aggregate, *args):
    ...
    mock_aggregate.assert_called_once_with(node_type, {'@id': 'single'})

    mock_aggregate.reset_mock()

    # aggregate global
    aggregate(node_type)
    mock_aggregate.assert_called_once_with(node_type, {'@id': 'global'})
...
@patch(f"{class_path}._get_source", return_value={})
...
def test_aggregate_by_timerange(mock_call_aggregate, *args):
    ...
    _aggregate_by_timerange(node_type, country={})(time_range)
