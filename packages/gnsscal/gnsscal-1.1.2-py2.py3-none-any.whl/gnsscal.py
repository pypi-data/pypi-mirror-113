...
def gpsws2date(weeks, seconds):
    """Convert GPS week and seconds of week to date, return datetime.datetime.
    ...
    """
    if weeks < 0 or seconds < 0 or seconds > 86400*7:
        raise ValueError('Invalid week or seconds: %s, %s' %(weeks, seconds))
    start = datetime.datetime(year=1980, month=1, day=6)
    totals = datetime.timedelta(seconds=weeks*7*24*3600 + seconds)
    day = start + totals
    return day
