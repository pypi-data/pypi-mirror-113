...
class PkgManagerBom:
    ...
    def __init__(self, pkg_manager, build_log, cov_home, cov_output, os_dist, bld_dir, unresolved_files_list,
    ...
    def run_log_parser(self):
        ...
        self.linker_list = log_parser.parse_build_log(self.build_log)
    ...
    def get_files(self, list_paths):
        ...
        for basefile in basefiles_dict.keys():
            basefiles_dict[basefile] = sorted(
                set(map(lambda path_1: path_1.strip(), filter(lambda path: path.endswith("/" + basefile), list_paths))),
                key=len,
                reverse=True)
