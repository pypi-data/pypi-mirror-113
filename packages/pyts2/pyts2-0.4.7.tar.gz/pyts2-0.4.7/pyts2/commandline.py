...
@click.option("--truncate-time", type=int, default=None, metavar="MINUTES",
              help="Truncate time to N-minute intervals.")
...
@click.option("--bundle-output", "--bo", type=str, default=None,
              help="Output timestream for verbatim import images")
...
@click.argument("input")
def gvmosaic(input, informat, dims, order, ncpus, audit_output, composite_bundling,
    ...
    pipe = TSPipeline(multi_file_pipeline=True)
    ...
    if bundle_output is not None:
        ...
        pipe.add_step(WriteFileStep(verbatim_ts))
    ...
    if recoded_output is not None:
        ...
        pipe.add_step(TeeStep(recoded_pipe))
