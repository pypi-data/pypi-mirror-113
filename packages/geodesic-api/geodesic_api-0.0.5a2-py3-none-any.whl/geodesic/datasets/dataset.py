...
class Bands(dict):
    ...
    def _repr_html_(self, add_style=True):
        ...
        bnds = ""
        for band_name in self:
            band = self[band_name]
            eo = band.get('eo', None)
            if eo:
                bt = eo_band_template.format(
                    title=band['title'],
                    key=band_name,
                    GSD=eo['bands'][0]['gsd']
                )
            else:
                bt = band_template.format(
                    title=band['title'],
                    key=band_name
                )
            bnds += bt

        return st + band_html_template.format(
            name=self._ds_name,
            bands=bnds
        )
...
<div class="card">
  <div class="inner scroll-style">
    ...
    <div class="content scroll-style">
      <div class="bands">
        <span class="card-text text-bands"><b>Bands:</b><ul>{bands}</ul></span>
      </div>
    </div>
  </div>
</div>
