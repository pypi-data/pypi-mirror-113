...
class TypeTemplate(
    ...
    def _get_type_value(self, type=None):
        ...
        print(10,type)
        print(11,type.name)
        print(11,type.sequence)
        print(11,type.sequence)
        print('rec', getattr(type, 'receivable'))
        print('dep', getattr(type, 'deposit'))
        ...
        res['name'] = type.name
        res['sequence'] = type.sequence
        res['statement'] = type.statement
        res['assets'] = type.assets
        for boolean in [
            ...
            res[boolean] = getattr(type, boolean)
        if not type or type.template != self:
            res['template'] = self.id
        res['template'] = type.template
        #if 'erhalten' in type.name:
        #    print(res)
        return res
