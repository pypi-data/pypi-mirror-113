...
class HardwareAPICLI:
    ...
    def readline(self, targets=[]):
        ...
        while True:
            ...
            if not new_data:
                break
            # After the first character shows up, we assume the rest will be quick
            if len(data) == 0:
    ...
    def run_command(self, command, timeout=None, check_return_code=True):
        self.absorb_outstanding()
    ...
    def linux_command(self, command, timeout=None):
        res = self.run_command(command, timeout=timeout, check_return_code=True)
