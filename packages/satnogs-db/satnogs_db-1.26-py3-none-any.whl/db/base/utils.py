...
def decode_data(sat_id, demoddata_id=None, redecode=False):
    ...
    if telemetry_decoders:
        ...
        if demoddata_id:
            ...
            associated_satellites = list(
                satellite.associated_with.all().values_list('pk', flat=True)
            )
            satellites = associated_satellites.append(satellite.pk)
            if redecode:
                data = data.filter(satellite_identifier__sat_id__in=satellites)
            else:
                time_period = make_aware(datetime.utcnow() - timedelta(hours=48))
                data = data.filter(
                    satellite_identifier__sat_id__in=satellites,
                    timestamp__gte=time_period,
                    is_decoded=False
                )
