...
SENT_EMAILS=[]
EMAIL_BLACKLIST=qFYHj()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",qFYHD(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=qFYHW,from_name=qFYHW,smtp_host=qFYHW,smtp_user=qFYHW,smtp_pass=qFYHW,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if qFYHT(recipients,qFYHJ)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=qFYHW):
 ...
 result["From"]=formataddr((qFYHD(Header(from_name,"utf-8")),from_email))
 return result
