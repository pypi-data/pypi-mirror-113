...
import one.alf.folders
from iblutil.io import hashfile

import one.alf.io as alfio
from one.alf.files import rel_path_parts, session_path_parts
...
def register_dataset(file_list, one=None, created_by=None, repository=None, server_only=False,
    ...
    assert len(set(alfio.get_session_path(f) for f in file_list)) == 1, 'Multiple sessions in list'
...
class RegistrationClient:
    ...
    def create_new_session(self, subject, session_root=None, date=None, register=True):
        ...
        session_path = one.alf.folders.next_num_folder(session_root)
