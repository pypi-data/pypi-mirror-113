...
class R2plus1dEncoder(nn.Module):
    def __init__(self, obs_shape, feature_dim, hidden_dim=64):
        super().__init__()
        ...
        self.r2plus1d = r2plus1d_9(num_classes=hidden_dim)
    ...
    def forward_conv(self, obs):
        ...
        T = self.n_frames
        C = TC // T
        obs = obs.view(N, T, C, H, W).transpose(1, 2)
        # resnet2+1D requires (N, C, T, H, W)
        return self.r2plus1d(obs)
