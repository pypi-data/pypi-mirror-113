...
var SpanTree = /** @class */ (function (_super) {
    ...
    SpanTree.prototype.render = function () {
        ...
        var _b = spans.reduce(function (acc, payload, index) {
            ...
            var key = utils_2.getSpanID(span, "span-" + index);
            var isLast = payload.isLastSibling;
            var isRoot = type === 'root_span';
            var spanBarColour = utils_1.pickBarColour(utils_2.getSpanOperation(span));
            var spanNumber = index + 1;
            var numOfSpanChildren = payload.numOfSpanChildren;
            var treeDepth = payload.treeDepth;
            var continuingTreeDepths = payload.continuingTreeDepths;
            acc.numOfFilteredSpansAbove = 0;
            acc.numOfSpansOutOfViewAbove = 0;
            acc.spanTree.push(<spanBar_1.default key={key} organization={organization} event={waterfallModel.event} spanBarColour={spanBarColour} spanBarHatch={type === 'gap'} span={span} showSpanTree={!waterfallModel.hiddenSpanGroups.has(utils_2.getSpanID(span))} numOfSpanChildren={numOfSpanChildren} trace={waterfallModel.parsedTrace} generateBounds={generateBounds} toggleSpanTree={_this.toggleSpanTree(utils_2.getSpanID(span))} treeDepth={treeDepth} continuingTreeDepths={continuingTreeDepths} spanNumber={spanNumber} isLast={isLast} isRoot={isRoot} isCurrentSpanFilteredOut={false} showEmbeddedChildren={payload.showEmbeddedChildren} toggleEmbeddedChildren={payload.toggleEmbeddedChildren} fetchEmbeddedChildrenState={payload.fetchEmbeddedChildrenState}/>);
            return acc;
        }, {
