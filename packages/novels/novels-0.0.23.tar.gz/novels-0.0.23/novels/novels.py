...
def build_index(dirbook) :
    ...
    if not os.path.isdir(dirbook) :
        return
    with open(os.path.join(dirbook,"index.html"),"w") as fnew :
        fnew.write("<html>\n")
        fnew.write("<head>\n")
        fnew.write("  <meta name=\"viewport\" content=\"width=device-width, initial-scale=1.0\">\n")
        fnew.write("  <title>Novels</title>\n")
        fnew.write("</head>\n")
        fnew.write("<body style=\"background-color: #EAF2F8;color: #000000;\">\n")
        fnew.write("<table border=1 style=\"border-collapse:collapse;\">\n")
        tmstr = strftime("%H:%M",localtime())
        fnew.write("<tr style=\"background-color:{};\"><td><a href=\".\">{}({})</a></td><td></td></tr>\n".format("#C9D08E","Index",tmstr))
        colors = ["#EFD3FD","#FDEFD3","#F6FDD3"]
        ic=0
        for d in sorted(os.listdir(dirbook),reverse=True,key=lambda x:os.path.getmtime(x)) :
            ...
            with open(fidx,"r") as f :
                ...
                for ln in reversed(f.readlines()) :
                    if cnt < 2 and re.search(r"td.*href.*html",ln) :
                        ...
                        if cnt == 0 :
                            fnew.write("<tr style=\"background-color:{};\">\n<td><a href=\"{}\">{}</a></td>\n".format(bgcolor,d,d))
                        else :
                            tmstr = strftime("%D %H:%M",localtime(os.path.getmtime(rd))) 
                            fnew.write("<tr style=\"color:{};background-color:{};\">\n<td align=\"right\">{}</td>\n".format("#00BB00",bgcolor,tmstr))
                        fnew.write(ln+"</tr>\n")
                        cnt += 1
        fnew.write("</table>\n")
        fnew.write("</html>")
...
"""
    ...
    for ix, m in enumerate(chapters) :
        ...
        if len(context) < 200 :
            ...
            break
        context = re.sub(r"</br>","<br/>\n",context)
        context = re.sub(r"<br\s*/>","<br/>\n",context)
