...
SENT_EMAILS=[]
EMAIL_BLACKLIST=QmFWj()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",QmFWM(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=QmFWv,from_name=QmFWv,smtp_host=QmFWv,smtp_user=QmFWv,smtp_pass=QmFWv,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if QmFWX(recipients,QmFWn)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=QmFWv):
 ...
 result["From"]=formataddr((QmFWM(Header(from_name,"utf-8")),from_email))
 return result
