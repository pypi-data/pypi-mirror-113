...
class PlateRecording(FileManagerPlateRecording):
    ...
    def from_directory(cls, dir_to_load_files_from: str) -> "PlateRecording":
        first_item = [f for f in os.listdir(dir_to_load_files_from) if not f.startswith(".")][0]
        if first_item.endswith(".zip"):
            ...
            if members[0].endswith(".xlsx"):
                optical_well_files = [
                    ExcelWellFile(os.path.join(dir_to_load_files_from, member)) for member in members
                ]
