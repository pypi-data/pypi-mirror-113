...
class TechnicalAnalysisAC(object):
    def __init__(self):
        pass
...
class RelativeStrengthIndex(TechnicalAnalysisAC):
    def __init__(self, int_period):
        pass
    
    def fit(self, X):
        diff = np.diff(X)
        gains = diff[diff>0]*(diff>0)
        losses = np.absolute(diff[diff<0]*(diff<0))
        gains = np.convolve(gains, np.ones(self.period), 'valid') / self.period
        losses = np.convolve(losses, np.ones(self.period), 'valid') / self.period
