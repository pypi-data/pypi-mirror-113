...
class BankingImportLine(ModelSQL, ModelView):
    ...
    def _apply_rules(self, rules):
        for rule in rules:
            print('_apply', rule.rec_name)
            keywords = rule.match(self)
            print('_apply kw', keywords)
            if keywords:
...
class RunImport(Wizard):
    ...
    def transition_next_(self):
        ...
        def next_line():
            ...
            print('actual_line', actual_line.balance, actual_line.amount)
            line_balance = actual_line.balance - actual_line.amount
            print('before_check line_balance journal_balance', line_balance, self.start.running_balance)
            ...
            print('next_line bl', batch_lines)
            print('next_line l', len(lines))
            if batch_lines:
        ...
        print('yyyyy', getattr(self.show, 'lines', None))
        ...
        print('xxxx', getattr(self.show, 'lines', None))
        if not next_line():
            return 'open_batch'
        return 'show'
        #while next_line():
    ...
    def transition_save_lines(self):
        if self.show.batch_lines:
            for line in self.show.batch_lines:
                ...
                print('_save_lines', line.amount, self.start.running_balance)
        return 'next_'
