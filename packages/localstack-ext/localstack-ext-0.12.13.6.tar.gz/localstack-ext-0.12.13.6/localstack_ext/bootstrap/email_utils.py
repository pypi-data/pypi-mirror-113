...
SENT_EMAILS=[]
EMAIL_BLACKLIST=efJOd()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",efJOm(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=efJOp,from_name=efJOp,smtp_host=efJOp,smtp_user=efJOp,smtp_pass=efJOp,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if efJOS(recipients,efJOE)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=efJOp):
 ...
 result["From"]=formataddr((efJOm(Header(from_name,"utf-8")),from_email))
 return result
