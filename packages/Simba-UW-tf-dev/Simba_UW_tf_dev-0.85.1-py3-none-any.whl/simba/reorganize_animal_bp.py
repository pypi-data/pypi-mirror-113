    ...
    if POSE_TOOL == 'DLC':
        ...
        if FILE_FORMAT == 'csv':
            ...
            if first_df.scorer.loc[2] == 'coords':
                ...
                return None, bodyparts, header_list
...
def reorganize_bp_order(IN_FOLDER, POSE_TOOL, FILE_FORMAT, HEADER_LIST, NEW_ANIMAL_LIST, NEW_BP_LIST):
    date_time = datetime.now().strftime('%Y%m%d%H%M%S')
    new_folder_name = 'Reorganized_bp_' + str(date_time)
    new_directory = os.path.join(IN_FOLDER, new_folder_name)
    if not os.path.exists(new_directory):
        os.makedirs(new_directory)
    print('Saving re-organized pose=estimation files in ' + str(new_directory) + '...')
    if POSE_TOOL == 'DLC':
        found_files = glob.glob(IN_FOLDER + '/*.' + FILE_FORMAT)
        new_header_tuples = []
        ...
        for file in found_files:
            ...
            in_test = pd.read_hdf(df_save_path)
            print(in_test)
            #curr_df_reorganized.to_hdf(df_save_path)
            print(str(df_save_path) + ' saved...')
        print('All pose-estimation files with re-organized body-part order saved in ' + new_directory)


# IN_FOLDER = r'/Volumes/GoogleDrive/My Drive/GitHub/SimBA_troubleshooting/re_organize_bps/input'
# POSE_TOOL = 'DLC'
# FILE_FORMAT = 'h5' #H5 or CSV
#
# in_animal_list, in_bp_list, header_list = display_original_bp_list(IN_FOLDER, POSE_TOOL, FILE_FORMAT)
#
# NEW_ANIMAL_LIST = list(reversed(in_animal_list))
# NEW_BP_LIST = list(reversed(in_bp_list))
#
# #CHECK THAT LISTS ARE EQUAL
# in_list_animal_sorted = sorted(in_animal_list)
# in_list_bp_sorted = sorted(in_bp_list)
# new_list_animal_sorted = sorted(NEW_ANIMAL_LIST)
# new_list_bp_sorted = sorted(NEW_ANIMAL_LIST)
# if in_list_animal_sorted != new_list_animal_sorted:
#     print('ERROR: Some animal names in the output column does not exist in the input column, please check that your typing is correct')
#
# elif in_list_bp_sorted != new_list_bp_sorted:
#     print('ERROR: Some animal body-part names in the output column does not exist in the input column, please check that your typing is correct')
# else:
#     reorganize_bp_order(IN_FOLDER, POSE_TOOL, FILE_FORMAT, header_list, NEW_ANIMAL_LIST, NEW_BP_LIST)
