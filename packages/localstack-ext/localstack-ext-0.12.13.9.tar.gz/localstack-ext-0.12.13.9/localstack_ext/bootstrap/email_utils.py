...
SENT_EMAILS=[]
EMAIL_BLACKLIST=Bpafe()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",BpafS(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=Bpafh,from_name=Bpafh,smtp_host=Bpafh,smtp_user=Bpafh,smtp_pass=Bpafh,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if Bpafw(recipients,BpafP)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=Bpafh):
 ...
 result["From"]=formataddr((BpafS(Header(from_name,"utf-8")),from_email))
 return result
