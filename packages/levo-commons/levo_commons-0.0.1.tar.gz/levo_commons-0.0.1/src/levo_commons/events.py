...
@attr.s(slots=True)
class Initialized(BaseEvent, Generic[P]):
...
@attr.s(slots=True)
class BeforeTestCaseExecution(BaseEvent, Generic[P]):
    """Just before a test case."""


@attr.s(slots=True)
class BeforeTestStepExecution(BaseEvent, Generic[P]):
    """Before each step inside a test case."""


@attr.s(slots=True)
class AfterTestStepExecution(BaseEvent, Generic[P]):
    """After each step in a test case."""


@attr.s(slots=True)
class AfterTestCaseExecution(BaseEvent, Generic[P]):
    """After a test case."""


@attr.s(slots=True)
class Interrupted(BaseEvent):
    """If execution was interrupted by Ctrl-C, or a received SIGTERM."""


@attr.s(slots=True)
class InternalError(BaseEvent):
...
@attr.s(slots=True)
class Finished(BaseEvent, Generic[P]):
