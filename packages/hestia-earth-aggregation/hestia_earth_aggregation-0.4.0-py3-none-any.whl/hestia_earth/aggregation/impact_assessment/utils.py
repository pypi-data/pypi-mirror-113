...
from hestia_earth.aggregation.utils import _aggregated_version, _aggregated_node
...
def _format_aggregate(aggregate: dict):
    ...
    if sd:
        node['sd'] = sd
    return _aggregated_version(node, 'value', 'min', 'max', 'sd', 'observations')


def _format_terms_results(results: tuple):
    aggregates, data = results
    return {
        **_create_impact_assessment(data.get('nodes', [])[0]),
