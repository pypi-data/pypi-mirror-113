...
class BBBDLC:
    def run(args):
        ...
        try:
            ...
            while (serverDir != "/" and os.path.basename(serverDir) != serverName):
                serverDir = os.path.dirname(serverDir)
            
            if serverDir == "/": raise NameError('Could not find nextcloud folder!')
            serverDir += "/nextcloud"
