...
def job_titles() -> dict:
    JOB_TITLES = {}
    try:
        with open('./src/persontitles/german_jobtitles.txt', mode='r', encoding='utf-8') as fin:  # noqa
            german_jobtitles = fin.read().split('\n')
            JOB_TITLES["German"] = german_jobtitles
        with open('./src/persontitles/english_jobtitles.txt', mode='r', encoding='utf-8') as fin:  # noqa
        ...
        with open('./src/persontitles/german_jobtitles.txt', mode='a', encoding='utf-8') as fout:  # noqa
            fout.write('\n'.join(item for item in german_jobtitles))
        with open('./src/persontitles/english_jobtitles.txt', mode='a', encoding='utf-8') as fout:  # noqa
