...
def start_course_trace(qsb, course: str, spare_time=4, waiting_time=60,
    ...
    general_functions.send_round_status(stage='weighing',
                                        info='start_photocell_trace_{}'.format(
                                            photocell_name),
                                        timer=waiting_time)
...
def photocell_tracer(qsb, photocell_name: str, spare_time=4, waiting_time=60,
    ...
    while waiting_time != 0:                           # Запускаем цикл
        ...
        if lock_state_last_time > start_datetime:
            ...
            general_functions.send_round_status(stage='weighing',
                                                info='photocell_was_blocked_{}'.format(photocell_name),
                                                timer=waiting_time)
            return result
        else:
            time.sleep(1)
            waiting_time -= 1
            general_functions.send_round_status(stage='weighing',
                                                info='wait_photocell_block_{}'.format(photocell_name),
                                                timer=waiting_time)
    return False
...
def spare_waiting_cycle(qsb, photocell_name, spare_time=4):
    ...
    while spare_timer != 0:
        ...
        if normal_state_info['current']:
            general_functions.send_round_status(stage='weighing',
                                                info='photocell_was_relieved_{}'.format(
                                                    photocell_name),
                                                timer=spare_timer)
            spare_timer -= 1
        else:
            ...
            general_functions.send_round_status(stage='weighing',
                                                info='photocell_was_blocked_again_{}'.format(
                                                    photocell_name),
                                                timer=spare_timer)
