...
class Plugin(E2EEPlugin):
    ...
    def get_omemo_sid_message(self) -> str:
        return 'xmpp:{bare}?omemo-sid-{did}={fp}'.format(
            bare=self.core.xmpp.boundjid.bare,
            did=self.my_device_id(),
            key=slixmpp_omemo.fp_from_ik(self.my_identity_key()),
        )
