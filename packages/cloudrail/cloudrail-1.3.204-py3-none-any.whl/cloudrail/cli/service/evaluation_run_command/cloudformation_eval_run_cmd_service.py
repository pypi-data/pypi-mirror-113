...
class CloudformationEvalRunCmdService(EvaluationRunCommandService):
    ...
    def _validate_input_paths(self) -> None:
        ...
        cfn_stack_region: str = os.getenv('AWS_REGION', self.command_parameters.cfn_stack_region)
        ...
        if cfn_params_file and not os.path.exists(cfn_filtered_template_file):
        ...
        if self.command_parameters.no_cloud_account and self.command_parameters.cloud_provider is None:
            echo_error('Enter "--cloud-provider" parameter when "--no-cloud-account" flag is on')
            exit_with_code(ExitCode.INVALID_INPUT)
    ...
    def _upload_iac_file(self, customer_id: str, account_config: AccountConfigDTO,
        ...
        filtered_cfn_template: str = ''
        if self.command_parameters.cfn_template_file:
            filtered_cfn_template = self._create_filtered_cfn_template()
        elif self.command_parameters.cfn_filtered_template_file:
            filtered_cfn_template = str(CloudformationHelper.load_cfn_template(self.command_parameters.cfn_filtered_template_file))
        else:
        ...
        cfn_template_content_as_dict: dict = json.loads(filtered_cfn_template)
