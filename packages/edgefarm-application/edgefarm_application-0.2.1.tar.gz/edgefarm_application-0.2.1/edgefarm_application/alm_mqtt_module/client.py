...
class AlmMqttModuleClient:
    ...
    def __init__(self):
        ...
        self._reqres_request_codec = schema_load_builtin(__file__, "reqResRequest")
    ...
    async def close(self):
        ...
        try:
            ...
            _logger.exception(f"Exception {e} while unsubscribing subjects (ignored)")
    ...
    async def subscribe(self, topic, handler):
        ...
        _logger.debug(f"subscribed to mqtt {topic}, nats subject {nats_subject}")
        return nats_subject
    ...
    async def request(self, mqtt_topic, payload, timeout: int):
        ...
        :raise: RuntimeError if ALM MQTT module reports an error
        """
        request_msg = {"topic": mqtt_topic, "payload": payload, "timeout": timeout}
        ...
        resp_msg = await self._nc.request(
            "alm-mqtt-module.request-response", data, timeout=timeout / 1000 + 5
        )
        resp = schema_decode(resp_msg.data)
        if len(resp["error"]) > 0:
            raise RuntimeError(
    ...
    async def _register_mqtt_topic(self, target, topic):
        ...
        if len(resp["error"]) > 0:
            raise RuntimeError(f"Can't register MQTT Topic {topic}: {resp['error']}")
