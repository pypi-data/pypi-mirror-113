...
class Manager:
    ...
    def show_status(self):
        ...
        log.info(
            f"######\nRunning sourcing workers {sourcing_worker_count} with {sourcing_core_count} cores\n"
...
class ManagerWorkersLoading(ManagerWorkersBase):
    ...
    def manage_nodeset_loading(self, assigned_no_of_cores):
        ...
        log.debug(
            f"NODESET progess loading, initial : {self._get_workers(progress=('initial', 'loading'), tag=self.worker_tag_nodesets)}"
        )
        log.debug(
            f"NODESET process running : {self._get_workers(status='running', tag=self.worker_tag_nodesets)}"
        )
        log.debug(
            f"RELSET loading : {self._get_workers(progress=('loading'), tag=self.worker_tag_relsets)}"
        )
    ...
    def manage_relation_loading(self, assigned_no_of_cores):
        ...
        log.debug(
            f"DRAINING WAIRT: {self._get_workers(progress='drain_ordered', tag=self.worker_tag_relsets)}"
        )
    ...
    def _check_for_drains_ready(self, assigned_no_of_cores):
        ...
        for drain_wait_worker in self._get_workers(
            ...
            if not self.parent.cache.is_drain_done(
                ...
                continue
            #### possible removable code?
            # if type(drain_wait_worker.set_meta) == SetsMetaBase:
            #    log.debug("###THIS PATH IS USED")
            #    relset_meta_details: RelationSetMeta = self.parent.cache.get_RelSetMeta(
            #        drain_wait_worker.set_meta
            #    )
            # else:
            #   relset_meta_details: RelationSetMeta = drain_wait_worker.set_meta
