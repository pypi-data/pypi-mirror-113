...
class Coordinates:
	...
	def get_decimal(self) -> DecimalStruct:
		...
		Snippet: value: DecimalStruct = driver.source.fsimulator.dsSimulation.user.tx.trajectory.tdf.enu.coordinates.get_decimal() \n
		No command help available \n
			:return: structure: for return value, see the help for DecimalStruct structure arguments.
		"""
		return self._core.io.query_struct('SOURce<HwInstance>:FSIMulator:DSSimulation:USER:TX:TRAJectory:TDF:ENU:COORdinates:DECimal?', self.__class__.DecimalStruct())

	def set_decimal(self, value: DecimalStruct) -> None:
		"""SCPI: [SOURce<HW>]:FSIMulator:DSSimulation:USER:TX:TRAJectory:TDF:ENU:COORdinates:[DECimal] \n
		Snippet: driver.source.fsimulator.dsSimulation.user.tx.trajectory.tdf.enu.coordinates.set_decimal(value = DecimalStruct()) \n
		No command help available \n
			:param value: see the help for DecimalStruct structure arguments.
		"""
		self._core.io.write_struct('SOURce<HwInstance>:FSIMulator:DSSimulation:USER:TX:TRAJectory:TDF:ENU:COORdinates:DECimal', value)
