...
class ConfigManager:
    ...
    def get_config_overrides(self, context, config_overrides_string):
        ...
        if context == 'global':
            ...
            Logger.warn("Switched from the application context to the global context.")
        elif context == 'project':
            _config['application'] = 'default'
            Logger.warn(f"Switched from the application context to the project context '{self.project}'.")
    ...
    def application(self):
        ...
        if self.project == 'default':
            if not result == 'default':
                # Logger.warn(f"Switched to project context '{self.project}' from the application context '{result}'.")
    ...
    @property
    def allow_stage_templates(self):
        return self.merged_config['template']['allowStageTemplates'] in [True, 'yes', 'Yes', 'true', 'True']
