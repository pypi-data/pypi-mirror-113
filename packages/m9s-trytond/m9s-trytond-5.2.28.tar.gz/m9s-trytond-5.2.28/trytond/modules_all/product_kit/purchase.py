    ...
    def compute_supply_date(self, date=None):
        ...
        if product.type == 'kit':
            ...
            for component in product.components:
                lead_time = max(lead_time, component.product.lead_time)
            if not date:
                date = Date.today()
            return date + lead_time
        return super(ProductSupplier, self).compute_supply_date(date=date)
    ...
    def compute_purchase_date(self, date):
        ...
        if product.type == 'kit':
            ...
            if not date:
                date = Date.today()
            return date - lead_time
        return super(ProductSupplier, self).compute_purchase_date(date=date)
