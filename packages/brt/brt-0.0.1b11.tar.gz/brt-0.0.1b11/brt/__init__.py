...
def split(data:pd.DataFrame, labels:list, weights:list = [], label_col = 'label', groupby:str=None, shuffle = False):
    '''
    Assign labels to the data with given weights.    
    ...
    data : pd.DataFrame
        Input data.
    labels : list
        List of labels applied to the data.
    weights : list, optional
        Weights of labels. If the length of weights is not same as the length of labels, equal weights will be used. The default is [].
    label_col : TYPE, optional
        The name of label column. The default is 'label'.
    groupby : str, optional
         If set, sample will be splited within each subsample grouped by 'groupby' column. The default is None.
    shuffle : TYPE, optional
        Whether the data is shuffled before labels are assigned. The default is False.
    ...
    if(groupby==None):
        ...
        for i in range(len(labels)):
            to = int(l*tos[i])
            data.loc[fm:to, label_col] = labels[i]
            fm = to
        ...
        for group in data[groupby].unique():
            output.append(split(data[data[groupby] == group], label_col = label_col, labels = labels, weights = weights))
        return pd.concat(output)  


def split_train_valid_test(data:pd.DataFrame, train:float=20, validation:float=2, test:float=1, label_col:str='label', groupby:str=None, shuffle = False):
    '''
    ...
    '''
    return split(data, ['train', 'valid', 'test'], weights = [train, validation, test], label_col = label_col, groupby = groupby, shuffle = shuffle)
