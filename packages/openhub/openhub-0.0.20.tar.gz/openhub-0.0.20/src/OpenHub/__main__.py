...
def check_if_api_has_hub_serial(ip):
    from ip_resolver import get_ip
    ip = get_ip()
    response = requests.get('http://' + str(ip) + ':8000/hubs/'+str(hub_serial_no))
    if not response.ok:
        hub_dict = {}
        ...
        hub_dict['aid'] = 1
        response = requests.post('http://' + str(ip) + ':8000/hub/',json=hub_dict)
