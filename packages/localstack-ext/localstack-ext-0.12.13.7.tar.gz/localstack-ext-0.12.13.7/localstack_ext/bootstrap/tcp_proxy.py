...
class AttrDict(RGufv):
 def __init__(self,*args,**kwargs):
  RGufS(AttrDict,self).__init__(*args,**kwargs)
  self.__dict__=self
def tcp_listen(six,addr,port,blk):
 s=socket.socket(AF_INET6 if six else AF_INET,SOCK_STREAM)
 if six and RGufV(socket,"IPV6_V6ONLY"):
...
class Half(RGufw):
 def __init__(self,opt,sock,addr,RGufJ):
  self.opt=opt
  self.sock=sock
  self.addr=addr
  self.RGufJ=RGufJ
  self.name="peer" if self.RGufJ else "client"
  self.queue=[]
  self.dest=RGufD
  self.err=RGufD
  self.ready=RGufx
 ...
 while qs:
  ...
  for q in qs:
   if q.post_wait(r,w,e):
    qs.remove(q)
def main():
 opt={"ip6":RGufx,"bind_port":2223,"addr":"192.168.43.191","port":22}
 opt={"ip6":RGufx,"bind_port":48429,"addr":"128.0.100.3","port":22}
 opt={"ip6":RGufx,"port":54237,"bind_addr":"128.0.100.2","bind_port":22}
 opt={"ip6":RGufx,"port":54237,"bind_addr":"192.168.100.3","bind_port":22}
 server_loop(opt)
