...
class CacheInterface:
    ...
    * To pass through log messages from the workers to the manager process, which handle the logs centralistic
    The implemented cache class need to have a high perfomance backend.
    ...
    * Mongo DB - https://www.mongodb.com/de
    ...
    def __init__(self, cache_backend_params: Dict = None):
...
class RedisCache(CacheInterface):
    ...
    class DrainOrder:
        ...
        @classmethod
        def list_orders(
            cls, cache, status=None, nodeset_meta: SetsMetaBase = None
            ...
            for res in results:
                for drain_id, raw_drain_order in res.items():
                    ...
                    if nodeset_meta is None or (
                        nodeset_meta is not None
                        and nodeset_meta.key in [ns.key for ns in order.nodesets_meta]
                    ):
                        orders.append(order)
        ...
        @classmethod
        def list_open_nodesets_meta(cls, cache: "RedisCache"):
            open_nodesets_meta = []
        ...
        def is_empty(self, cache):
            ...
            for bucket in self.nodesets_buckets_keys:
                if (
                    bucket != "DRAIN_EMPTY_PLACEHOLDER_KEY"
                    and bucket not in all_drained_buckets_keys
                ):
                    return False
        ...
        def set_graphset_drained(
            ...
            if isinstance(graphset, graphio.NodeSet):
                # DevSanityCheck: To be removed later
    ...
    def fetch_NodeSets(self, nodeset_meta: NodeSetMeta) -> graphio.NodeSet:
        ...
        drain_orders = RedisCache.DrainOrder.list_orders(
            self, status="OPEN", nodeset_meta=nodeset_meta
        )
        if drain_orders:
    ...
    def fetch_RelSets(
        ...
        if not drain_order.are_nodesets_drained(self):
            log.exception(
