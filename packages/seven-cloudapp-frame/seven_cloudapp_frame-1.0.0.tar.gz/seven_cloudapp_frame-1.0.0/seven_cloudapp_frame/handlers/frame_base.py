...
@LastEditTime: 2021-07-19 17:57:58
...
class FrameBaseHandler(BaseApiHandler):
    ...
    def add_request_user(self, app_id, object_id):
        """
        :description: 每小时流量UV计数,用于登录接口限制登录
        :param app_id: 应用标识
        ...
        redis_key = "request_user_list:" + app_id + "_" + str(SevenHelper.get_now_hour_int())
        if not redis_init.hexists(redis_key, object_id):
            redis_init.hset(redis_key, object_id, 1)
            redis_init.expire(redis_key, 3600)

    def check_request_user(self, app_id, current_limit_count):
        """
        :description: 每小时流量UV校验
        :param app_id: 应用标识
        ...
        redis_init = SevenHelper.redis_init()
        redis_key = "request_user_list:" + app_id + "_" + str(SevenHelper.get_now_hour_int())
        if int(redis_init.hlen(redis_key)) >= current_limit_count:
            return True
        else:
            return False
...
class TaoBaseHandler(FrameBaseHandler):
    ...
    def prepare(self):
        ...
        try:
            ...
            # 频繁请求校验
            is_continue_request, error_message = self.check_continue_request(self.__class__.__name__, str(dict_par["app_id"]), str(dict_par["open_id"]))
            if is_continue_request:
                return self.reponse_json_error("error", error_message)

            #每小时流量UV计数,用于登录接口限制登录
...
class WeChatBaseHandler(FrameBaseHandler):
    ...
    def prepare_ext(self):
        try:
            ...
            if clientheaderinfo_string:
                self.device_dict = self.get_device_info(clientheaderinfo_string)

            # 频繁请求校验
            is_continue_request, error_message = self.check_continue_request(self.__class__.__name__, str(self.device_dict["pid"]), str(self.get_param("user_id")))
            if is_continue_request:
                return self.reponse_json_error("error", error_message)
    ...
    def get_device_info(self, device_str):
        ...
        device_dict["pid"] = int(info_model["PID"][0])
...
def filter_check_head(is_check=True, sign_key=""):
    """
