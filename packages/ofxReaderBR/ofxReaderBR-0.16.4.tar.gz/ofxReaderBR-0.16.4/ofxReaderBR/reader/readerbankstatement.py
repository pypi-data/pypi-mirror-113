    ...
    def read(self):
        ...
        for row in ws.values:
            ...
            if cs.is_valid():
                ...
                bs.errors.append(f"Transação com dados inválidos")
            ...
            if used_origin:
                if origin['type'] == AccountType.CREDITCARD:
                    if len(str(origin['number'])) != 4:
                        errors.append(f"Origem do Tipo Cartão de Crédito deve possuir apenas "
                                      f"os últimos 4 dígitos do Cartão")
                        read_status = BankStatement.ERROR
                    if not str(origin['number']).isdigit():
                        errors.append(f"Origem do Tipo Cartão de Crédito deve possuir apenas números")
                        read_status = BankStatement.ERROR
                elif origin['type'] == AccountType.BANKACCOUNT:
                    account_split = str(origin['number']).split('-')
                    if len(account_split) != 2:
                        errors.append(f"Origem do Tipo Conta Corrente deve possuir dígito verificador")
                        read_status = BankStatement.ERROR
                    else:
                        if not str(account_split[0]).isdigit() or not str(account_split[1]).isdigit():
                            errors.append(f"Origem do Tipo Conta Corrente deve possuir "
                                             f"apenas números e o dígito verificador")
                            read_status = BankStatement.ERROR
                        if len(str(account_split[1])) != 1:
                            errors.append(f"Origem do Tipo Conta Corrente deve possuir apenas um dígito verificador")
                            read_status = BankStatement.ERROR
    ...
    def _read_origins(values):
        ...
        for row in values:
            ...
            if len(cell_values) < 10:
                errors.append(f"É necessário preencher todos campos do arquivo")
                read_status = BankStatement.ERROR
            ...
            if cell_values[0] is None:
                errors.append(f"Banco não pode estar em Branco")
                read_status = BankStatement.ERROR
            if cell_values[1] is None:
                errors.append(f"Tipo não pode estar em Branco")
                read_status = BankStatement.ERROR
            if cell_values[2] is None:
                errors.append(f"Origem não pode estar em Branco")
                read_status = BankStatement.ERROR
