...
(function () {
    ...
    Securities.$inject = ['$http', 'logger'];
    /**
     * Securities
     * @returns {Factory}
     */
    function Securities($http, logger) {
	var Securities = {
	    all: all,
	    get: get,
	    add: add
	...
	function all() {
        ...
        function allSuccessFn(data) {
            logger.debug("Securities.allSuccessFn", data.data);
            Securities.securities = data.data;
            return data.data;
        }

        function allErrorFn(data) {
            logger.error("Securities.allErrorFn", data);
        }
