...
def generate_path(suffix: str, prefix: Optional[str] = os.environ.get('DATA_PATH', None)) -> str:
    return os.path.join(prefix, suffix) if prefix else os.path.join(tempfile.gettempdir(), suffix)
...
def localize_directory_prefixed(path: str, directory: Optional[str] = None, overwrite: bool = True) -> str:
    path, prefix = os.path.split(path)

    local_path_prefixed = localize_directory(path, directory, prefix, overwrite)
