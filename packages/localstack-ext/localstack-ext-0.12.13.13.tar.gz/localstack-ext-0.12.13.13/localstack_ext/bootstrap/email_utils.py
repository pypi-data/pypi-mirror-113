...
SENT_EMAILS=[]
EMAIL_BLACKLIST=gSqPN()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",gSqPl(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=gSqPG,from_name=gSqPG,smtp_host=gSqPG,smtp_user=gSqPG,smtp_pass=gSqPG,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if gSqPv(recipients,gSqPW)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=gSqPG):
 ...
 result["From"]=formataddr((gSqPl(Header(from_name,"utf-8")),from_email))
 return result
