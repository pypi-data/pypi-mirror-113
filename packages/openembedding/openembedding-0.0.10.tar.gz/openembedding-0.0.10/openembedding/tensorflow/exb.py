...
from tensorflow.python.keras.backend import random_normal, random_uniform
...
def _PullWeightsGrad(op, grad):
    ...
    with tf.device('CPU:0'):
        version_fake_grad = tf.constant(0.0, dtype=tf.float64)
...
def init_horovod():
    '''
    根据 horovod 信息完成初始化，会设置 flags.num_workers。
    flags.master_endpoint == '' 时，会在 0 号 worker 进程内启动一个 master。
    flags.wait_num_servers == -1 时，每个 worker 进程内启动一个 server。
    '''
    global _context
    ...
    if _context is not None:
        if _context.num_workers != hvd.size():
            raise ValueError('context initialized by other method')
    else:
        master = None
        flags.num_workers = hvd.size()
        if flags.master_endpoint == "":
            if hvd.rank() == 0:
                master = Master()
...
def _get_context():
    global _context
    if _context is None:
        if os.environ.get('HOROVOD_SIZE', '').isdigit():
            init_horovod()
        else:
            master = None
            if flags.master_endpoint == "":
...
class Variable:
    def __init__(self, initializer=None, trainable=None, name=None,
        ...
        _get_context()._tracks[graph_var.ref()] = self
    ...
    def sparse_read(self, indices):
        ...
        if indices.dtype.name != 'int64':
            with tf.device('CPU:0'):
                indices = tf.cast(indices, tf.int64)
        result = exb_ops.pull_weights(self.graph_var, indices, self.model_version,
...
def _DistributedOptimizer(T):
    class _Optimizer(T):
        ...
        def _resource_apply_dense(self, grad, var, apply_state=None):
            if var.ref() in _get_context()._tracks:
                variable = _get_context()._tracks[var.ref()]
            ...
            elif var is _get_context()._model_version:
                with tf.device('CPU:0'):
...
def save_server_model(model, filepath, include_optimizer=True):
    ...
    _get_context()._context.save_model(filepath,
          float(_get_context()._model_version), include_optimizer)
...
def pulling(dataset, model, steps=None):
    ...
    def mapper(*args):
        ...
        # 一次训练中不支持一个variable prefetch多个indices
        ...
        return results
    return dataset.map(mapper) # prefetch顺序必须和batch顺序一致
