...
#                    logger.info(f'query result: {result}')
                ...
                logger.info(f'self.cached_by_query: {self.cached_by_query.keys()}')
                _prev_nodes = self.cached_by_query.get(query, {})

                logger.info(f'_prev_nodes: {_prev_nodes.keys()} vs found_nodes {found_nodes.keys()}')
                if _prev_nodes.keys() - found_nodes.keys():
                    # nodes shouldn't dissapear too frequently, if happened most likely it is a glitch in search, retry
                    logger.warning(f'{_prev_nodes.keys() - found_nodes.keys()} from perviously cached nodes {_prev_nodes.keys()} is missing in the list of found_nodes {found_nodes.keys()}, retrying search')
                    time.sleep(1)
                    continue
                else:
                    logger.info("new nodes dont miss any perviously cached nodes")
                    self.cached_by_query[query] = {node["resource_id"]: node for node in nodes}
                    break

                self.cached_by_query[query] = {node["resource_id"]: node for node in nodes}

            # update cached_nodes only for whole cluster query
            if CLUSTER_NAME_QUERY == query:
                self.cached_nodes = found_nodes

            for key in found_nodes.keys() - self.cached_nodes.keys():
    ...
    def _get_node(self, node_id):
        ...
        with self.lock:
            ...
            try:
                ...
                print(f'Error listing tags for node {node}: {e}')
