...
def get_ctg_mean_depth_by_samtools_coverage(index_ref, ref_seq, reads_r1, reads_r2, reads_unpaired, subsample_rate, num_threads):
    ...
    # sort mapping
    cmd_samtools_sort = 'samtools sort %s -o %s' % (sam_file_one_end_clp_no_middle_best_match_low_mismatch, sam_file_sorted)
...
def get_ctg_mean_depth_by_samtools_coverage_global(index_ref, ref_seq, reads_r1, reads_r2, reads_unpaired, subsample_rate, num_threads):
    ...
    # sort mapping
    cmd_samtools_sort = 'samtools sort %s -o %s' % (sam_file_best_match_low_mismatch, sam_file_sorted)
    os.system(cmd_samtools_sort)
...
def parse_rd2_sam_gnm_worker(arguments_list):
    ...
    free_living_ctg_ref_file_with_pos_cigar      = arguments_list[3]
    min_M_len_ctg                                = arguments_list[4]
    mismatch_cutoff                              = arguments_list[5]
    round_2_ctg_end_seq_len_dict                 = arguments_list[6]
    rd2_with_both_mates                          = arguments_list[7]
    ctg_ignore_region_dict_2rd                   = arguments_list[8]
...
def link_16s(args):
    ...
    for each_link in marker_to_ctg_LinkingRecord_dict:
        ...
        if len(linking_reads) >= 3:
            ...
            linked_to_16s_end_cigar_num, linked_to_16s_end_cigar_pct = get_linked_to_ref_end_cigar_num_and_pct(
                min_dist_list_to_16s_end)
            linked_to_ctg_end_cigar_num, linked_to_ctg_end_cigar_pct = get_linked_to_ref_end_cigar_num_and_pct(
                min_dist_list_to_ctg_end)
            ...
            if (short_cigar_pct_16s < max_short_cigar_pct_cutoff) and (short_cigar_pct_ctg < max_short_cigar_pct_cutoff) and (linked_to_16s_end is True) and (linked_to_ctg_end is True):
    ...
    for marker_to_ctg in marker_to_ctg_linkage_num_dict_after_qc:
        ...
        for each_link in mini_to_mag_LinkingRecord_dict.copy():
            ...
            if len(current_linkage_linking_read_base) < 3:
                ...
                if (short_cigar_pct_mini < max_short_cigar_pct_cutoff) and (short_cigar_pct_ctg < max_short_cigar_pct_cutoff) and (linked_to_ctg_end is True):
                    ...
                    stats_mini_assembly_to_ctg_handle.write(
