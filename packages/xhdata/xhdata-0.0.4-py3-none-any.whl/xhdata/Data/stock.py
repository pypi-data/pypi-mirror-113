...
def stock_zh_a_daily(
    ...
    try:
        ...
        if adjust in ['qfq', 'hfq']:
            ...
            if qfq is not None:
                df['open'] = df['open'].transform(lambda x: x * float(qfq['factor']))
                df['close'] = df['close'].transform(lambda x: x * float(qfq['factor']))
                df['high'] = df['high'].transform(lambda x: x * float(qfq['factor']))
                df['low'] = df['low'].transform(lambda x: x * float(qfq['factor']))
