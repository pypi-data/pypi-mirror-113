...
def finalize_and_clean_up_compute_job(job_id: str):
    ...
    if RUNNING_IN_ENCLAVE and not DEV_MODE:
        logger.debug('Deregistering and shutting down...')
        deregister_and_shutdown()
...
def deregister_and_shutdown():
    payload = {
        'auth_token': COMPUTE_NODE_INFO['auth_token'],
        'public_id': COMPUTE_NODE_INFO['public_id'],
    }
    # TODO: deregister directly with backend instead
    # requests.post(url=f'{BASE_URL}/api/jobs/deregister/', json=payload)
    requests.post(url=f'{compute_process_config.PARENT_REST_SERVER_URL}/deregister/', json=payload)
