...
def run():
    ...
    bld_dir = shlex.quote(my_args.args.build_dir)
    bld_cmd = my_args.args.build_cmd
    cov_dir = shlex.quote(my_args.args.cov_dir) if my_args.args.cov_dir else None
    ...
    codelocation_name = shlex.quote(my_args.args.codelocation_name) if my_args.args.codelocation_name else "{}/{}".format(
    ...
    if cov_dir:
        ...
        cov_output = os.path.join(cov_home, "output", hub_project_name)
    ...
    logging.basicConfig(format="%(filename)s:%(funcName)s:%(lineno)d:%(asctime)s:%(levelname)s: %(message)s",
                        level=logging.DEBUG, filename=os.path.join(cov_output, LOG_FILENAME), filemode='w')
    ...
    logging.info("output log is written to file {}".format(os.path.join(cov_output, LOG_FILENAME)))

    if offline_mode:
        logging.debug("Storing config file to location : {}".format(os.path.join(cov_output, "offline_config.yaml")))
        with open(os.path.join(cov_output, "offline_config.yaml"), 'w') as f:
    ...
    if use_offline_files:
        logging.warning(
            "Please make sure skip_* parameters are kept exactly same as in offline mode from offline configuration file {}".format(
                os.path.join(cov_output, "offline_config.yaml")))
    cov_bin = os.path.join(cov_home, "bin")
    sig_scanner_path = os.path.join(cov_home, "output")
    build_log = os.path.join(cov_output, "build-log.txt")
    binary_files_list = os.path.join(cov_output, "all_binary_paths.txt")
    json_grep_files_list = os.path.join(cov_output, "json_grep_files_diff.txt")
    unresolved_files_list = os.path.join(cov_output, "unresolved_file_paths.txt")
    resolved_files_list = os.path.join(cov_output, "resolved_file_paths.txt")
    resolved_cov_files_list = os.path.join(cov_output, "resolved_cov_file_paths.txt")
    unresolved_cov_files_list = os.path.join(cov_output, "unresolved_cov_file_paths.txt")
    cov_json_file_path = os.path.join(cov_output, "cov_emit_links.json")
    ...
    cov_conf = os.path.join(cov_home, "conf")
    ...
    if not use_offline_files:
        blackduck_emit_wrapper = EmitWrapper(cov_home, cov_output, platform_name, skip_build=my_args.args.skip_build,
                                             build_log=build_log)
        ...
        if os.path.exists(os.path.join(cov_output, 'cov_emit_output_files')):
            logging.info("Attempting to use offline files for cov-manage-emit at location: {}".format(
                os.path.join(cov_output, 'cov_emit_output_files')))
        else:
            logging.error(
    ...
    if os.path.exists(cov_emit_link_path):
        ...
        if not use_offline_files:
            full_cmd = "{} --dir {} list-capture-invocations > cov_emit_links.json".format(cov_manage_emit_path,
                                                                                           cov_output)
            ...
            if os.path.exists(os.path.join(cov_output, 'cov_emit_links.json')):
                logging.info("Attempting to use offline files for cov-emit-link at location: {}".format(
                    os.path.join(cov_output, 'cov_emit_links.json')))
            else:
                logging.error(
    ...
    if (BDBA_MODE in run_modes or ALL_MODE in run_modes):
        if not use_offline_files:
            logging.info(
                "The bdba tar file to scan will be written to {}".format(os.path.join(cov_output, "bdba_ready.tar")))
        else:
            if os.path.exists(os.path.join(cov_output, "bdba_ready.tar")):
                logging.info("Attempting to use offline files for BDBA at location: {}".format(
                    os.path.join(cov_output, 'bdba_ready.tar')))
            else:
                logging.error(
    ...
    if not offline_mode and (PKG_MGR_MODE in run_modes or ALL_MODE in run_modes):
        bdio2_python_wrapper = BDIO2Transformer(cov_output, hub_api)
        json_file_path = os.path.join(cov_output, "raw_bdio.json")
        ...
        try:
            ...
            logging.debug("no components present in json {}".format(os.path.join(cov_output, "raw_bdio.json")))
            skip_bdio2 = True
        if not skip_bdio2:
