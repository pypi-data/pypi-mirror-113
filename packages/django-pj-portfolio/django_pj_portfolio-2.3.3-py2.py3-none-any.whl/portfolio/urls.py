...
from portfolio.accounts.views import (AccountListView,
                                      AccountEditView, AccountDeleteView,
                                      AccountCreateView, AccountViewSet)
...
urlpatterns = [
    url(r'^$', PortfolioIndexView.as_view(), name='portfolio-index'),
    ...
    url(r'^api/v1/exchange/', ExchangeRatesView.as_view(),
        name='exchange-rates'),
    # All the rest to be handled by Angular
    url(r'^.*$', PortfolioIndexView.as_view(), name='portfolio-index'),
]
