...
```
q = litcorpt.Query()
search = corpus_db.search(q.creator.any(q.name == 'Eça de Queirós'))
...
most_common_names = {lastname: count for name, count in names.items() if count >= 5}
...
for name, count in names.items():
  if count >= 5:
    most_common_surnames[name] = count
```
