...
from cubicweb.__pkginfo__ import numversion as version_cubicweb
...
class GraphQLSecurityTC(CubicWebTC):
    ...
    def test_empty_when_cannot_read_with_args(self):
        query = (
            "query getAffaire($sujet: String) " "{ Affaire(sujet: $sujet) { sujet } }"
        )
        sujet = self.repo.schema["Affaire"].rdef("sujet")
        with self.temporary_permissions((sujet, {"read": ("users", "managers")})):
            with self.admin_access.repo_cnx() as cnx:
    ...
    def test_read_erqlexpr(self):
        ...
        with self.new_access(u"iaminusersgrouponly").repo_cnx() as cnx:
            ...
            cnx.commit()
            result = self.gql_nok(
                cnx, "query getStuff($x: ID) { Affaire(eid: $x) {eid, sujet} }", x=aff1
            )
            self.assertIsNone(result.data["Affaire"])
            result = self.gql_ok(
                cnx, "query getStuff($x: ID) { Affaire(eid: $x) {eid, sujet} }", x=aff2
            )
            self.assertEqual(len(result.data["Affaire"]), 1)
            self.assertEqual(result.data["Affaire"][0]["eid"], str(aff2))
            result = self.gql_ok(
                cnx, "query getStuff($x: ID) { Card(eid: $x) {eid, title} }", x=card1
            )
    ...
    def test_insert_security(self):
        with self.new_access(u"anon").repo_cnx() as cnx:
            self.gql_ok(
                cnx,
                "mutation m($nom: String) " "{ createPersonne(nom: $nom) {  nom  } }",
                nom=u"bidule",
            )
            self.assertRaises(Unauthorized, cnx.commit)
            self.assertEqual(
                len(self.gql_ok(cnx, "{ Personne { nom } }").data["Personne"]), 1
            )
    ...
    def test_insert_security_2(self):
        with self.new_access(u"anon").repo_cnx() as cnx:
            ...
            self.assertEqual(
                len(self.gql_ok(cnx, "{ Affaire { sujet } }").data["Affaire"]), 0
            )
    ...
    def test_insert_rql_permission(self):
        ...
        with self.admin_access.repo_cnx() as cnx:
            self.assertEqual(
                len(self.gql_ok(cnx, "{ Affaire { sujet } }").data["Affaire"]), 1
            )
        with self.new_access(u"iaminusersgrouponly").repo_cnx() as cnx:
            self.gql_ok(
                cnx,
    ...
    def test_delete_security(self):
        ...
        with self.new_access(u"iaminusersgrouponly").repo_cnx() as cnx:
            self.gql_nok(
                cnx, "mutation m($n:String) { deleteCWGroup(name: $n) }", n=u"staff"
            )
    ...
    def test_delete_relation_rql_permission(self):
        ...
        with self.new_access(u"iaminusersgrouponly").repo_cnx() as cnx:
            self.gql_nok(
                cnx,
                "mutation { deleteFromAffaire(from: {}," " concerne_as_Societe: {}) }",
            )
            self.assertRaises(QueryError, cnx.commit)  # can't commit anymore
            cnx.rollback()
            self.gql_ok(
...
class GraphQLFeatureTC(GraphQLBaseTC):
    ...
    def test_simple_query_with_eid_arg(self):
        ...
        """
        query = (
            "query getAffaire($affaire_eid: ID) "
    ...
    def test_simple_query_with_eid_arg_double(self):
        ...
        """
        query = (
            "query getAffaire($eid1: ID, $eid2: ID) "
    ...
    def test_nested_query_arguments(self):
        ...
        """
        query = (
            "query getPersonne($pn: String, $sn: String) "
            "{ Personne(nom: $pn) "
        ...
        with self.admin_access.repo_cnx() as cnx:
            ...
            cnx.execute(
                "SET P travaille S WHERE " "P nom 'personne1', S nom 'societe2'"
            )
            ...
            self.assertEqual(
                result.data["Personne"][0]["travaille"][0]["nom"], "societe1"
            )
    ...
    def test_mutation_insert_simple(self):
        ...
        """
        query = (
            "mutation myMutation($sujet: String) "
            "{ createAffaire(sujet: $sujet) {  eid, sujet  } }"
        )
        with self.admin_access.repo_cnx() as cnx:
    ...
    def test_mutation_insert_with_where(self):
        ...
        with self.admin_access.repo_cnx() as cnx:
            ...
            self.assertEqual(
                result.data["createAffaire"]["concerne"][0]["nom"], u"societe1"
            )
            self.assertEqual(
                result.data["createAffaire"]["concerne"][0]["eid"], societe1
            )
    ...
    def test_mutation_update_object_field(self):
        ...
        with self.admin_access.repo_cnx() as cnx:
            ...
            self.assertEqual(
                result.data["updateAffaire"][0]["concerne"][0]["eid"], societe1
            )
            self.assertEqual(
                result.data["updateAffaire"][0]["concerne"][0]["nom"], u"societe1"
            )
            ...
            self.assertEqual(len(result.data["updateAffaire"][0]["concerne"]), 1)
            self.assertEqual(
                result.data["updateAffaire"][0]["concerne"][0]["eid"], societe2
            )
            self.assertEqual(
                result.data["updateAffaire"][0]["concerne"][0]["nom"], u"societe2"
            )
    ...
    def test_mutation_delete_relation(self):
        ...
        with self.admin_access.repo_cnx() as cnx:
            ...
            self.assertEqual(result.data["updateAffaire"][0]["eid"], str(self.affaire1))
            self.assertEqual(
                result.data["updateAffaire"][0]["concerne"][0]["eid"], str(societe1)
            )
            result = GraphQLQuerier.execute(
                cnx, "{ Affaire { eid, concerne { ... on Societe { eid } } } }"
            )
            self.assertEqual(result.data["Affaire"][0]["eid"], str(self.affaire1))
            self.assertEqual(
                result.data["Affaire"][0]["concerne"][0]["eid"], str(societe1)
            )
            result = GraphQLQuerier.execute(
                cnx,
                "mutation m($s: ID, $a: ID) "
...
class GraphQLReflectionTC(GraphQLBaseTC):
    ...
    def test_graphql_based_reflection_list_entities(self):
        ...
        with self.admin_access.repo_cnx() as cnx:
            ...
            self.assertIsNone(result.errors)
            self.assertIsNotNone(result.data)
            if version_cubicweb < (3, 27, 0):
                self.assertEqual(
                    len(result.data["__schema"]["queryType"]["fields"]), 44
                )
            else:  # If CW is >= 3.27 then CWCache entity type is dropped
                self.assertEqual(
    ...
    def test_cw_based_reflection_list_entities(self):
        ...
        with self.admin_access.repo_cnx() as cnx:
            ...
            self.assertIsNotNone(result.data)
            if version_cubicweb < (3, 27, 0):
                self.assertEqual(len(result.data["CWEType"]), 44)
            else:  # If CW is >= 3.27 then CWCache entity type is dropped
