...
class BqConnector(GoogleConnector):
    ...
    def dry_run_sql(self, sql:str = None):
        ...
        info = query_job.total_bytes_processed/1024/1024
        log.info("COST: This query will process {} MiB.".format(info))
        return info
...
class BqConnect(BqConnector):
    ...
    def __init__(self,
        ...
        self.sql = self.__file_sub(sql, sql_file)


    def __file_sub(self, sql, sql_file):
        if sql_file:
            if sql:
                log.warning('you should not provide sql and a sql file... ignoring file')
                return sql
            with open(sql_file) as f:
                sql = f.read()
                if bool(re.search('\{.*\}', sql)):
    ...
    def _accessor(self, write=False, **kwargs):
        ...
        if self.data_type == 'bq':
            log.warning('you are only able to write with this configuration')
