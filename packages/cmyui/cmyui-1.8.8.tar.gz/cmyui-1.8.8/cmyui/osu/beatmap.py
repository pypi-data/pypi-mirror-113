    ...
    def from_str(cls, s: str) -> 'TimingPoint':
        ...
        if split_len == 8:
            if all(map(is_atleast_negative_float, tp_split)):
                return cls(
                    time=int(float(tp_split[0])),#losing info
    ...
    def from_str(cls, s: str, **kwargs):
        ...
        if _extra:
            if len(_extra) == 3:
                ...
                #print('slider extras wrong')
                pass

        # support sliders without any points; this is rare but i've
        # seen it in some aspire maps, e.g. https://old.ppy.sh/osu/2573493
        curve_points = []
        if '|' in _curve:
...
class Event:
    ...
    @staticmethod
    def from_str(s: str) -> Optional['Event']:
        ...
        transformations:
            ...
            F: Fade
            V: VectorScale
            T: TriggerGroup/TriggerLoop
            L: TransformationLoop
            R: Rotation
            S: Scale
            P: Parameter
                H: Horizontal flip
        ...
        if split[1].isdecimal():
            ...
            # TODO: Colour, Sprite, Sample, Animation
            if _type.isdecimal():
                ...
                _type = _type.lstrip(' _')

                if _type not in ev_map:
                    return
...
class Beatmap:
    ...
    @classmethod
    def from_data(cls, data: str) -> 'Beatmap':
        b = cls(data)
        b._parse()
    ...
    def _parse(self) -> None:
        # osu sometimes  leaves \ufeff at start?
        if self.data.startswith('\ufeff'):
            #print('\x1b[0;92mosu funny ufeff\x1b[0m')
            self._offset += 1

        # fix for my own stupid programs
        if self.data.startswith('//'):
            x = self._data.find('\n') + 1
            print('yep',x)
            self._offset += x

        sec_start = self._data.find('\n\n')
