...
class ExchangeHistory:
    ...
    def state_transitions(self, walker='replica'):
        ...
        Language here is somewhat different than the rest of the class. We have a "walker" that randomly walks across
        some sort of "state" space.
        ...
        References
        ----------
        Kim, J., Keyes, T., & Straub, J. E. (2009) Replica exchange statistical temperature Monte Carlo. *J. Chem.
        Phys.* 130, 124112.
        ...
        n_steps, n_replicas = data.shape
        steps = {}
        for k in range(n_replicas):  # This is the walker
            k_data = data[k].to_frame('state')
            for i in range(n_replicas):  # First state
                for j in range(n_replicas):  # Second state
                    if i == j:
                        continue
                    r = int(np.abs(i - j))  # Difference between states
                    ijk_data = k_data.query(f'state in [{i}, {j}]').reset_index()
                    mask = ijk_data['state'].diff() != 0  # remove repeated rows
                    ijk_data = ijk_data[mask].copy()
                    ijk_data['n_steps'] = ijk_data['step'].shift(-1) - ijk_data['step']
                    trans = ijk_data.query(f'state == {i}')['n_steps'].dropna().to_numpy()  #
                    steps[r] = np.append(steps.get(r, []), trans)
