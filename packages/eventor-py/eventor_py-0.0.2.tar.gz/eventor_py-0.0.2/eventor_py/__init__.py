...
class Eventor():
    ...
    # Save .xml file 
    def _saveXML(self, fileName, content):
        try:
    ...
    # Build url with querys
    def _url_builder(self, apiurl, querys):
        if str(apiurl)[-1] == str('/'):
            try:
    ...
    # Gets data from api call, returns content
    def fetch_data(self, url, querys):
        logging.info(url)
        logging.info(querys)

        url = self._url_builder(url, querys)
        logging.info(url)
        response = ''
        ...
        if self.save_xml == True:
            self._saveXML(self.filepath, response.content)
    ...
    def parse_results(self, **kwargs):
        logging.info("function: parse_results")
        try:
            if ".xml" in kwargs["openfilepath"]:
                self.open_file = True
                self.openfilepath = kwargs["openfilepath"]
                logging.info(f"openfilepath: {self.openfilepath}")
        except Exception:
            logging.info("wrong openfilepath")
            

        rootData = False
        if self.open_file:
            if os.path.exists(self.openfilepath):
                logging.info(f"file exists: {self.openfilepath}")
                tree = ET.parse(self.openfilepath)
                root = tree.getroot()
                rootData = True
                ...
                for c in classes:
                    name = c.find("ClassResult").find("EventClass").find("Name").tex
                    print(name)

            else:
                logging.info(f"could not find file: {self.openfilepath}")
        ...
        if rootData:
            ...
            for child in root:
                ...
                for c in child:
                    print("     " + c.tag)
            event_name = root.find("Event").find("Name").text
...
#response = eventor.get_event(eventId="35217", filepath="D:\PROGRAMMERING\Python\EventorPy2\downloads\event.xml")
#response = eventor.get_results(eventId="35217", filepath="D:\PROGRAMMERING\Python\EventorPy2\downloads\eventres.xml", includeSplitTimes="true")

print(eventor.parse_results(openfilepath="D:\PROGRAMMERING\Python\EventorPy2\downloads\eventres.xml"))