...
class Loader {
  ...
  _processConfigObject(rootDir) {
    ...
    if (this._config.globalSetup) this._config.globalSetup = path.resolve(rootDir, this._config.globalSetup);
    if (this._config.globalTeardown) this._config.globalTeardown = path.resolve(rootDir, this._config.globalTeardown);
    ...
    if ('testDir' in this._config && this._config.testDir !== undefined && !path.isAbsolute(this._config.testDir)) this._config.testDir = path.resolve(rootDir, this._config.testDir);
...
function validateConfig(file, config) {
  ...
  if ('reporter' in config && config.reporter !== undefined) {
    if (Array.isArray(config.reporter)) {
      ...
      if (typeof config.reporter !== 'string' || !builtinReporters.includes(config.reporter)) throw (0, _util.errorWithFile)(file, `config.reporter must be one of ${builtinReporters.map(name => `"${name}"`).join(', ')}`);
    }
  }
