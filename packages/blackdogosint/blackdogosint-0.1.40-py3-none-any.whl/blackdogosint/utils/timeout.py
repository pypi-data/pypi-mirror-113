...
from blackdogosint.utils import context
...
class TimeoutDefault(object):
    def __repr__(self): return 'timeout.Timeout.default'
...
class Timeout(object):
    """Implements a basic class which has a timeout, and support for scoped
    timeout countdowns. Valid timeout values are:
    ...
    @property
    def timeout(self):
        """Timeout for obj operations.

        By default, uses ``context.timeout``.
        """
    ...
    def countdown(self, timeout=default):
        """Scoped timeout setter.

        Sets the timeout within the scope, and restores it when leaving
        the scope. When accessing :attr:`timeout` within the scope, it
        will be calculated against the time when the scope was entered,
        in a countdown fashion. If :const:`None` is specified for
        ``timeout``, then the current timeout is used is made.  This
        allows :const:`None` to be specified as a default argument with
        less complexity.
        """
