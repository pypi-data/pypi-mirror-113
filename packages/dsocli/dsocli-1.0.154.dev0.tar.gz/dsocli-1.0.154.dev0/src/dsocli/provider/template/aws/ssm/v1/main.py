...
class AwsSsmTemplateProvider(TemplateProvider):
    ...
    def add(self, project, application, stage, key, contents, render_path=None):
        ...
        if not Stages.is_default(stage) and not ALLOW_STAGE_TEMPLATES:
            raise DSOException(f"Templates may not be added to stage scopes, as the feature is currently disabled. It may be enabled using 'ALLOW_STAGE_TEMPLATES' in the DSO global settings.")
        ...
        response = ssm.put_parameter(Name=path, Value=escape_curly_brackets(contents), Type='StringList', Overwrite=True)
        return {
                'Key': key,
    ...
    def get(self, project, application, stage, key, revision=None):
        ...
        response = ssm.get_parameter_history(Name=found['Name'])
    ...
    def delete(self, project, application, stage, key):
        ...
        response = ssm.delete_parameter(Name=found['Name'])
        return {
                'Key': key, 
                'Scope': found['Scope'],
                'Origin': found['Origin'],
