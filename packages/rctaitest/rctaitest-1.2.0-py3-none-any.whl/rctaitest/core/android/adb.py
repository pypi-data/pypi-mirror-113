...
from rctaitest.core.android.constant import (DEFAULT_ADB_PATH, IP_PATTERN,
                                           SDK_VERISON_ANDROID7)
from rctaitest.core.error import (AdbError, AdbShellError, rctaitestError,
                                DeviceConnectionError)
from rctaitest.utils.compat import decode_path, raisefrom, proc_communicate_timeout, SUBPROCESS_FLAG
from rctaitest.utils.logger import get_logger
from rctaitest.utils.nbsp import NonBlockingStreamReader
from rctaitest.utils.retry import retries
from rctaitest.utils.snippet import get_std_encoding, reg_cleanup, split_cmd, make_file_executable
...
class ADB(object):
    ...
    def swipe(self, tuple_x0y0, tuple_x1y1, duration=500):
        ...
        Raises:
            rctaitestError: if SDK version is not supported
        ...
        if version <= 15:
            raise rctaitestError('swipe: API <= 15 not supported (version=%d)' % version)
        elif version <= 17:
    ...
    def get_top_activity(self):
        ...
        Raises:
            rctaitestError: if top activity cannot be obtained
        ...
        if m:
            ...
            raise rctaitestError("Can not get top activity, output:%s" % dat)
    ...
    def is_screenon(self):
        ...
        Raises:
            rctaitestError: if screen state can't be detected
        ...
        raise rctaitestError("Couldn't determine screen ON state")
    ...
    def is_locked(self):
        ...
        Raises:
            rctaitestError: if lock screen can't be detected
        ...
        if not m:
            raise rctaitestError("Couldn't determine screen lock state")
        return (m.group(1) == 'true')
    ...
    def path_app(self, package):
        ...
        Raises:
            ...
            rctaitestError: if package is not found on the device
        ...
        if 'package:' not in output:
            raise rctaitestError('package not found, output:[%s]' % output)
    ...
    def check_app(self, package):
        ...
        Raises:
            rctaitestError: if package is not found
        ...
        if match is None:
            raise rctaitestError('package "{}" not found'.format(package))
        return True
    ...
    def start_app_timing(self, package, activity):
        ...
        if not re.search(r"Status:\s*ok", out):
            raise rctaitestError("Starting App: %s/%s Failed!" % (package, activity))
