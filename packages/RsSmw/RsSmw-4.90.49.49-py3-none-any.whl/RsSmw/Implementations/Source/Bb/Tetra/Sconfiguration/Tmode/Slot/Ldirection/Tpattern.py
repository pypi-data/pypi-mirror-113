...
class Tpattern:
	...
	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("tpattern", core, parent)
	...
	def set(self, structure: TpatternStruct, testMode=repcap.TestMode.Default, slot=repcap.Slot.Default, channel=repcap.Channel.Default) -> None:
		"""SCPI: [SOURce<HW>]:BB:TETRa:SCONfiguration:TMODe<DI>:SLOT<ST>:LDIRection<CH>:TPATtern \n
		Snippet: driver.source.bb.tetra.sconfiguration.tmode.slot.ldirection.tpattern.set(value = [PROPERTY_STRUCT_NAME](), testMode = repcap.TestMode.Default, slot = repcap.Slot.Default, channel = repcap.Channel.Default) \n
		...
		equivalent to the first bit of the training sequence. All further will be inserted successively. \n
			:param structure: for set value, see the help for TpatternStruct structure arguments.
		...
		self._core.io.write_struct(f'SOURce<HwInstance>:BB:TETRa:SCONfiguration:TMODe{testMode_cmd_val}:SLOT{slot_cmd_val}:LDIRection{channel_cmd_val}:TPATtern', structure)
