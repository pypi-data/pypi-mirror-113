...
class _BaseRateSimulator(Generic[RATE_MODEL_TYPE], _BaseSimulator[RATE_MODEL_TYPE]):
    ...
    def _scan_threading(self, parameter_values: list[float], worker: Callable) -> list:
    ...
    def _scan_multiprocessing(self, parameter_values: list[float], worker: Callable) -> list:
        with tqdm(total=len(parameter_values)) as pbar:
    ...
    def parameter_scan(
        ...
        **integrator_kwargs: Dict[str, Any],
        ...
        if sys.platform in ["win32", "cygwin"] or not multiprocessing:
            results = self._scan_threading(parameter_values, worker)
        else:
            results = self._scan_multiprocessing(parameter_values, worker)
        concentrations = {}
    ...
    def parameter_scan_with_fluxes(
        ...
        **integrator_kwargs: Dict[str, Any],
        ...
        if sys.platform in ["win32", "cygwin"] or not multiprocessing:
            ...
            Executor = futures.ThreadPoolExecutor  # type: ignore
        else:
            Executor = futures.ProcessPoolExecutor  # type: ignore
        with tqdm(total=len(parameter_values)) as pbar:
            with Executor() as executor:
                tasks = [executor.submit(worker, i) for i in parameter_values]
                results = []
