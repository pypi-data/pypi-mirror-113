...
def basic_post():
   ...
   '''
    ...
    url = current_app.config['EXTERNAL_URL'] + url_for('host.hostlist', q=dMsg.get('Content'), fromuser=dMsg.get(
        'FromUserName'), state=request.args['msg_signature'])
    ...
    dRsp = {'xml': {
        ...
        'MsgType': 'text',
        'Content': f'收到\"{dMsg.get("Content")}\"查询请求，<a href=\"{url}\">点击链接查看查询结果</a>'
    }}
