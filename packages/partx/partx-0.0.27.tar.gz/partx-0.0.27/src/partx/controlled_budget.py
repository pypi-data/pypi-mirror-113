...
def model_construction(s: np.array, Y: list, v_s: int, sub_r: list, lower, upper, level: list, z: int, q: int, i_dim: int, fal_num:int, n_model:int,alpha:float, list_star:list, root_path) -> Tuple[np.array,np.array]:   ##Gaussian process 
    '''
...
def replicate(data):
    ...
    q, sub_u, i_dim, i_B, alpha, n ,level, iteration, min_volume, max_budget, fal_num, n_model, n_bo, n_b, sample_method, part_num, root_path = data
    ...
    for z in range(iteration):  ##change for iteration
        ...
        if min(v_min) > min_volume*v_s and D + i_B*(n_bo+n)*len(sub_u) < max_budget: #0.01: #v(sub_u)
            ...
            model_lower, model_upper,level_quantile = model_construction(s, Y, v_s, sub_r, lower, upper, level, z, q, i_dim, fal_num, n_model,alpha,list_star, root_path)
...
#             print(S)
            ...
            lower=[]
            upper=[]
            model_lower, model_upper,level_quantile = model_construction(s_c, Y_c, v_s, sub_r, lower, upper, level, z, q, i_dim, fal_num, n_model,alpha, list_star, root_path)
            p_quantile = [[] for i in range(len(level))]
            p_quantile = part_percent(level, sub_r, v_s,level_quantile, p_quantile,i_dim)
            theta_undefined = []
            tpn = []
...
def _part_classify(sub_u: list, i_dim: int, i_B: int, 
    ...
    for q in range(replication):
        data = (q, sub_u, i_dim, i_B, alpha, n , level, iteration, min_volume, max_budget, fal_num, n_model, n_bo, n_b, sample_method, part_num, root_path)
        inputs.append(data)
