...
from platrock.Common.OneDTreeGenerator import TreeGenerator
		...
		if(self.enable_forest):
			...
			while(True):
				...
				if(trees_density<=1e-5 or dhp<0.01): # avoid zero division
					...
					treeGenerator=TreeGenerator(treesDensity=trees_density/10000,trees_dhp=dhp_mean/100,dRock=r.radius*2.,sim=self)
