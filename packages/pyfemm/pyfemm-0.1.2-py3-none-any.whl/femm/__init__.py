# Adapted for Linux and Mac, Melina Merkel, June 2021
import os
import re
...
def openfemm(winepath='',femmpath='',*arg):
	...
	# find wine path
		if winepath != '':
			if os.path.isdir(winepath):
				...
				raise Exception("Given path for wine does not exist. Please check 'winepath'.")
		else:
			# check all environment paths for wine
			envpath = os.getenv('PATH').split(":") 
			# add good guesses 
			envpath.extend(['/usr/bin','/usr/local/bin','/opt/bin','/opt/local/bin','/Applications/CrossOver.app/Contents/SharedSupport/CrossOver/bin'])
			for k in envpath:
				if os.path.exists(k+'/wine'):
					winedir = k
					break
			if 'winedir' not in locals():
				raise Exception("Wine binary not found in default path. Please define a path as openfemm(winepath='path/to/wine/',femmpath='path/to/femm/').")
		...
		if femmpath != '':
			if os.path.isdir(femmpath):
				...
				raise Exception("Given path for FEMM does not exist. Please check 'femmpath'.")
		else:
			# check if femm.exe exists in user's wine directory
			rootpath = [expanduser("~")+'/.wine/drive_c/femm42/bin',
			...
			if 'femmdir' not in locals():
				raise Exception("FEMM binary not found in default path. Please define a path as openfemm(femmpath='path/to/wine/',femmpath='path/to/femm/').")
				

		ifile=fixpath(femmdir + '/ifile.txt');
		ofile=fixpath(femmdir + '/ofile.txt');
		...
		try:
			...
			os.unlink(ifile);
			os.system(fixpath(winedir + '/wine ') + fixpath(femmdir + '/femm.exe') + ' -filelink &');	
		except OSError as e:
...
def callfemm(myString):
	...
	if windowsOS:
		...
		while (x == False):
			try:
				f = open(ofile, mode='rt')
				x = f.readline().replace("[ ","[").replace(" ]","]").replace(" ",",").replace("I","1j");
				f.close()
			except FileNotFoundError as e:
				time.sleep(sleeptime)
				notfound = 1
...
def ci_setnodeprop(nodeprop,groupno,incond):
	callfemm('ci_setnodeprop(' + quotec(nodeprop) + numc(groupno) + quote(incond) + ')');
...
def mi_setcomment(fn):
	callfemm('mi_setcomment(' + quote(fn) + ')' );
	
def ei_setcomment(fn):
	callfemm('ei_setcomment(' + quote(fn) + ')' );

def hi_setcomment(fn):
	callfemm('hi_setcomment(' + quote(fn) + ')' );

def ci_setcomment(fn):
	callfemm('ci_setcomment(' + quote(fn) + ')' );
