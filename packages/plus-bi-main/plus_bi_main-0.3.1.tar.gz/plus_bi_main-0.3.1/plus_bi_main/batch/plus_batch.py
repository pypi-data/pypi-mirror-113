...
class PlusBatch():
    ...
    def file_was_processed(self):
        ...
        res = self.call_batchsummary_api(parameters=parameters)

        if res['message'] != 'File not yet processed.':
    ...
    def set_report_message(self, action):
        ...
        return self.report
    def call_batchsummary_api(self, parameters):
    ...
    def send_report(self, destination_type, api_url=''):
        ...
        if destination_type == 'web_service':
            if self.api_url == '' and api_url=='':
                logging.error('API URL for web service not set...')
