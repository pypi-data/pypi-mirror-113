...
from map_analyser.app.presenters import Viewer, TestViewer
...
def main(viewer, params):
    ...
    yield 'Offsetting polygons'
    areas = ma.offset_areas(areas)

    yield 'Cleaning polygons'
    tmp = ma.clean_areas(areas)
    areas = ma.create_default_area(areas) if params["default_area"] else []
    areas.extend(tmp)
    ...
    viewer.add_path(path, color="#FF0000")
...
def run():
    ...
    vw = Viewer() if not DEBUG else TestViewer()
