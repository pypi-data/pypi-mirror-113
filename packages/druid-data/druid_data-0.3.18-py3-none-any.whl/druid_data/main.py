        ...
        if not isclass(python_type):  # Try to get the class type
            if hasattr(python_type, "__supertype__"):  # Try to get the class type from typings.NewType
                ...
                if (hasattr(python_type, "__args__") and issubclass(python_type.__origin__, (set, frozenset))
                    ...
                    if issubclass(python_type.__args__[0], (bytes, bytearray)):
                        return "BS"
                    if issubclass(python_type.__args__[0], str):
                        return "SS"
                    if issubclass(python_type.__args__[0], (int, float, Decimal)):
                        return "NS"
            ...
            for fl in fls:
                ...
                if fl.name not in self.entity_type._dynamo_ignore():
                    if ConstantString.ITEM_ATTR_TYPE.value not in self.map_dict[fl.name]:
                        ...
                        if ConstantString.OBJ2ITEM_CONV.value not in self.map_dict[fl.name]:
                            if attrib_type == "SS":
                                self.map_dict[fl.name][
                                    ConstantString.OBJ2ITEM_CONV.value] = DynamoConversions.string_set_to_dynamo
                                conv_back = DynamoConversions.identity
                                self.map_dict[fl.name][ConstantString.ITEM2OBJ_CONV.value] = conv_back
                            elif attrib_type == "BS":
                                self.map_dict[fl.name][
                                    ConstantString.OBJ2ITEM_CONV.value] = DynamoConversions.bytes_set_to_dynamo
                                if issubclass(fl.type.__args__[0], bytearray):
                                    conv_back = DynamoConversions.bytearray_set_from_dynamo
                                    self.map_dict[fl.name][ConstantString.ITEM2OBJ_CONV.value] = conv_back
                                else:  # Assumes a bytes set
                                    conv_back = DynamoConversions.bytes_set_from_dynamo
                                    self.map_dict[fl.name][ConstantString.ITEM2OBJ_CONV.value] = conv_back
                            elif attrib_type == "NS":
                                self.map_dict[fl.name][
                                    ConstantString.OBJ2ITEM_CONV.value] = DynamoConversions.number_set_to_dynamo
                                if issubclass(fl.type.__args__[0], int):
                                    conv_back = DynamoConversions.int_set_from_dynamo
                                    self.map_dict[fl.name][ConstantString.ITEM2OBJ_CONV.value] = conv_back
                                elif issubclass(fl.type.__args__[0], float):
                                    conv_back = DynamoConversions.float_set_from_dynamo
                                    self.map_dict[fl.name][ConstantString.ITEM2OBJ_CONV.value] = conv_back
                                else:  # Assumes a Decimal set
                                    conv_back = DynamoConversions.identity
                                    self.map_dict[fl.name][ConstantString.ITEM2OBJ_CONV.value] = conv_back
