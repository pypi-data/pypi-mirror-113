...
class TestSpectrum(unittest.TestCase):
    ...
    def test_element(self):
        ...
        mol.charge = 2
        self.assertEqual(mol.charge, 2, 'ensure charge was changed')
        self.assertEqual(mol.x[0], 68.436231, 'ensure minimum value was scaled')
        self.assertEqual(
            mol.trim()[0],
        ...
        self.assertEqual(mol.x[0], 136.872462, 'ensure minimum value was scaled')
        self.assertEqual(
            mol.trim()[0],
            [171.843, 173.84, 173.841, 174.842, 175.837, 175.838, 175.841, 176.839, 177.835, 177.838, 177.841, 178.836,
             179.835, 179.838, 179.843, 181.835, 181.84, 183.837]
    ...
    def test_trim_indexing(self):
        """tests trim indexing to correctly return the expected spectrum"""
        spec = Spectrum(1, start=200., end=600.)
        spec.add_value(200., 10.)
        spec.add_value(456.2, 20.)
        spec.add_value(599.9999992, 10.)
        masses, intensity = spec.trim()
        self.assertEqual(
            masses,
            [200.0, 456.2, 600.0],
        )
