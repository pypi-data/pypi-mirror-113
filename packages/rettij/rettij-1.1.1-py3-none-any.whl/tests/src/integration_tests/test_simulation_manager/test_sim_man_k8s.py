...
class TestSimulationManagerKubernetes(unittest.TestCase):
    ...
    def _copy(self, cont_file_dir: str) -> None:
        """
        Copy a file to and from the Node.

        :param cont_file_dir: File base directory.
        """
        # Test file path
        ...
        self.assertTrue(
            self.test_node.executor.copy_file_to_node(test_file_path, dst_dir=cont_file_dir),
            msg=f"Testing copying a file to container {self.test_node.executor.name} failed.",
        )

        with tempfile.TemporaryDirectory() as dst_dir:
            # Get file from container
            self.test_node.executor.copy_file_from_node(f"{cont_file_dir}{test_file_name}", dst_dir=dst_dir)
