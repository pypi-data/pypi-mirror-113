...
class PlusSubbatch():
    ...
    def set_web_service(self, api_url):
        if api_url =='':
...
WHERE SubBatchId = '{self.cds_subbatchid}'"""
    ...
    AND state = '{self.state}'"""
            ...
            if not self.report:
                self.report = self.set_report_message(action='select')
            else:
                self.report['Action'] = 'insert'
            subbatch_report = json.dumps(self.report['SubBatch'])
            
            parameters={ "key": f"{self.report['Action']}", "value": f"{subbatch_report}" }
        ...
        if not result:
            ...
            if not self.web_service:
                ...
                self.cds_batchid=result.CDS_BatchId
                self.number_of_records = result.NumberOfRecords
                self.destination_account = result.Accountname
                self.destination_container = result.Containername
                self.destination_folder = result.Foldername
                self.destination_filename = result.Filename
