...
class Data2D:
    ...
    def resample_data2d(data2d_time, data2d, new_time, tolkey, tolval):
        ...
        for i in range(0, nvariables):
            ...
            if len(rt1_arr) != len(new_time):
                print('[resample_on_data2d] len(rt1_arr) != len(new_time)')
                print('[resample_on_data2d] len(rt1_arr) %s, len(new_time) %s'
                      % (len(rt1_arr), len(new_time)))
                return None
