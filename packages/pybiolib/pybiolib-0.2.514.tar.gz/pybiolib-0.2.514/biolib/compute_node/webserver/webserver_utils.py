...
def report_availability(compute_node_info, dev_mode):
    try:
        ...
        logger.debug(f'Registering with {data} to host {api_client.base_url}')
        logger.debug(f'Registering at {datetime.datetime.now()}')

        max_retries = 5
        for retry in range(max_retries):
            try:
                req = requests.post(f'{api_client.base_url}/api/jobs/report_available/', json=data)
