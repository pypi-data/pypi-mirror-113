...
class CloudformationHelper:
    ...
    def update_cfn_template_with_extra_parameters(cls, cfn_template_content: dict, cfn_stack_name: str, iac_type: IacType,
                                                  cloud_provider: CloudProviderDTO, cfn_stack_region: str,
        ...
        if account_config:
            cfn_template_content[cls.EXTRA_PARAMETERS_KEY]['account_id'] = account_config.cloud_account_id
    ...
    @staticmethod
    def load_cfn_template(cfn_template_file: str) -> dict:
        body = read_file(cfn_template_file)
        if StringUtils.is_json(body):
            return load_json(body)
        elif StringUtils.is_yaml(body):
