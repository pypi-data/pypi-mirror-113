...
SENT_EMAILS=[]
EMAIL_BLACKLIST=VOiaE()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",VOias(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=VOiaz,from_name=VOiaz,smtp_host=VOiaz,smtp_user=VOiaz,smtp_pass=VOiaz,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if VOiaw(recipients,VOiaA)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=VOiaz):
 ...
 result["From"]=formataddr((VOias(Header(from_name,"utf-8")),from_email))
 return result
