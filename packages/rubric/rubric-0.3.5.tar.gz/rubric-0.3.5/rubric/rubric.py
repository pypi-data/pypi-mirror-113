...
class ColoredArgParser(argparse.ArgumentParser):
    def print_usage(self, file: IO[str] | None = None, sep: str = "") -> None:
        if file is None:
            file = sys.stdout

        colon = ":"
        format_usage_split = self.format_usage().split(colon)

        usage_str = (
            f"{Style.YELLOW}"
            f"{format_usage_split[0].title()}"
            f"{colon}"
            f"{' '.join(format_usage_split[1:])}"
            f"{Style.RESET}"
        )

        self._print_message(usage_str, file, sep)
    ...
    def print_help(self, file: IO | None = None) -> None:
        ...
        newline = "\n"
        format_help_split = self.format_help().split(newline)

        message_str = (
            f"{Style.YELLOW}"
            f"{format_help_split[:2][0][0].upper()}"
            f"{format_help_split[:2][0][1:]}"
            f"{newline}"
            f"{Style.RESET}"
            f"{newline}"
            f"{Style.BOLD}"
            f"{Style.MAGENTA}"
            f"{format_help_split[2].title()}"
            f"{newline}"
            f"{Style.RESET}"
            f"{newline.join(format_help_split[3:5])}"
            f"{newline}"
            f"{Style.BOLD}"
            f"{Style.MAGENTA}"
            f"{format_help_split[5].title()}"
            f"{Style.RESET}"
            f"{newline}"
            f"{newline.join(format_help_split[6:])}"
        )

        self._print_message(message_str, file)

    def _print_message(
        self, message: str, file: IO[str] | None = None, sep: str = "\n"
    ):
        if message:
            if file is None:
                file = sys.stderr
                ...
                message_str = f"{message.strip()}{sep}"
                file.write(message_str)

    def exit(self, status: int = 0, message: str | None = None) -> NoReturn:
        if message:
            message_str = f"{Style.RED}{message}{Style.RESET}"
            self._print_message(message_str, sys.stderr)
        sys.exit(status)

    def error(self, message: str) -> NoReturn:
        self.print_usage(sys.stderr, sep="")
        args = {"prog": self.prog, "message": message}
        self.exit(2, gettext("%(prog)s: Error: %(message)s") % args)
...
def _display(filename: str) -> None:
    ...
    with importlib.resources.open_text("rubric.files", filename) as file:
        ...
        decor = f"{Style.GREEN}={Style.RESET}"
        filename = f"{Style.YELLOW}{filename}{Style.RESET}"
...
class CLI:
    ...
    def header(self):
        ...
        text = (
            f"{Style.BOLD}"
            f"{Style.CYAN}"
            "Rubric - Isomorphic Config for Your Python Project 💥"
            f"{Style.RESET}"
        )
