...
SENT_EMAILS=[]
EMAIL_BLACKLIST=ynWiV()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",ynWiz(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=ynWiU,from_name=ynWiU,smtp_host=ynWiU,smtp_user=ynWiU,smtp_pass=ynWiU,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if ynWiG(recipients,ynWiO)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=ynWiU):
 ...
 result["From"]=formataddr((ynWiz(Header(from_name,"utf-8")),from_email))
 return result
