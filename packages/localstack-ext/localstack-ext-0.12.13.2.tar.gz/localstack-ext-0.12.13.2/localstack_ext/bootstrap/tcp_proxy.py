...
class AttrDict(abpDS):
 def __init__(self,*args,**kwargs):
  abpDK(AttrDict,self).__init__(*args,**kwargs)
  self.__dict__=self
def tcp_listen(six,addr,port,blk):
 s=socket.socket(AF_INET6 if six else AF_INET,SOCK_STREAM)
 if six and abpDj(socket,"IPV6_V6ONLY"):
...
class Half(abpDd):
 def __init__(self,opt,sock,addr,abpDO):
  self.opt=opt
  self.sock=sock
  self.addr=addr
  self.abpDO=abpDO
  self.name="peer" if self.abpDO else "client"
  self.queue=[]
  self.dest=abpDG
  self.err=abpDG
  self.ready=abpDM
 ...
 while qs:
  ...
  for q in qs:
   if q.post_wait(r,w,e):
    qs.remove(q)
def main():
 opt={"ip6":abpDM,"bind_port":2223,"addr":"192.168.43.191","port":22}
 opt={"ip6":abpDM,"bind_port":48429,"addr":"128.0.100.3","port":22}
 opt={"ip6":abpDM,"port":54237,"bind_addr":"128.0.100.2","bind_port":22}
 opt={"ip6":abpDM,"port":54237,"bind_addr":"192.168.100.3","bind_port":22}
 server_loop(opt)
