...
SENT_EMAILS=[]
EMAIL_BLACKLIST=tUWjI()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",tUWja(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=tUWjE,from_name=tUWjE,smtp_host=tUWjE,smtp_user=tUWjE,smtp_pass=tUWjE,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if tUWjr(recipients,tUWjk)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=tUWjE):
 ...
 result["From"]=formataddr((tUWja(Header(from_name,"utf-8")),from_email))
 return result
