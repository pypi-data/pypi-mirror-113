...
import { AlertDialog } from '@chiadoge/core';
import {
  Typography,
  Button,
  Box,
...
import { chiadoge_to_mojo } from '../../../util/chiadoge';
...
export const CreateRLAdminWallet = () => {
  ...
  function create() {
    ...
    if (
      chiadogeper_input.value === '' ||
      Number(chiadogeper_input.value) === 0 ||
      !Number(chiadogeper_input.value) ||
      isNaN(Number(chiadogeper_input.value))
    ) {
      dispatch(
        openDialog(
          <AlertDialog>
    ...
    const chiadogeper = chiadoge_to_mojo(chiadogeper_input.value);
    const chiadogeper_value = Number.parseInt(Number(chiadogeper));
    const userpubkey = userpubkey_input.value;
    const amount = chiadoge_to_mojo(amount_input.value);
    const amount_value = Number.parseInt(Number(amount));
    // var fee = chiadoge_to_mojo(fee_input.value);
